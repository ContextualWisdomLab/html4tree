...
When the CLI crawl (`go` → `crawl_directories`) sees a null `listFiles()`
snapshot, it forwards an empty name array and an empty `File` array so
`process_ignore_file` does not call `File.list()` and `process_dir` does
not call `File.listFiles()` again.
...
file appears as `href="./minutes.txt"` with a `title` that wraps
`minutes.txt` in First Strong Isolate marks and the Korean type label
`파일`. After this listing-metadata branch, the same row also shows the
byte size and a UTC `<time>`.

An unreadable directory must stay an empty-directory status page. Do not
publish a later successful listing that ignore never saw.

## Threat and failure model

Two separate `File.list()` or `File.listFiles()` calls can observe
different directory contents (Bishop & Dilger, 1996). That TOCTOU gap can
hide a newly created secret from one filter while exposing it to the
other, or the reverse. One snapshot makes both filters agree. This is not
a substitute for symlink refusal, ignore-file size limits, HTML escaping,
or bidirectional-control neutralization.
