...
// ⚡ Bolt Performance Optimization: Extract static CSS string and expensive SHA-256 computation to top-level properties.
// This prevents redundant string allocation and cryptographic hashing on every recursive directory processing call (process_dir).
val globalCssContent = """
              body {
...
val globalStyleHash = "sha256-" + Base64.getEncoder().encodeToString(MessageDigest.getInstance("SHA-256").digest(globalCssContent.toByteArray(Charsets.UTF_8)))

val globalCss = """
              <style>
${globalCssContent}              </style>
              """
...
<html lang="ko">
     <head>
        ...
        <meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src '${globalStyleHash}'; base-uri 'none'; form-action 'none';">
