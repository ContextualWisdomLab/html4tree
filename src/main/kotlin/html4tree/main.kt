...
fun process_ignore_file(curr_dir: File, dirFilesNames: Array<String>? = null): Set<String> {
    ...
    val defaultSensitiveFiles = listOf(".git", ".env", ".ssh", ".htpasswd", ".htaccess", "id_rsa", "id_ed25519", "id_ecdsa", "id_dsa", "secrets.yml", "secrets.json", ".html4ignore", ".DS_Store", ".aws", ".kube", ".npmrc", ".gnupg", "config.json", "credentials.json")
    files_to_exclude.addAll(defaultSensitiveFiles)

    // 보안 향상: 숨김 파일 및 흔히 쓰이는 인증키(.pem, .key), 데이터베이스(.sqlite, .db) 확장자를 동적으로 제외하여 중요 정보 노출 방지
