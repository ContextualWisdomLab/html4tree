...
private object StaticAssets {
    // ⚡ Bolt Performance Optimization: Extract static assets
    // Prevents redundant string allocations and expensive SHA-256 hash recalculations
    // on every process_dir loop iteration.
    // Using const val and @JvmField avoids JaCoCo coverage drops from implicit getters.
    ...
    @JvmField val styleHash = "sha256-" + Base64.getEncoder().encodeToString(MessageDigest.getInstance("SHA-256").digest(cssContent.toByteArray(Charsets.UTF_8)))
...
""" 
   ...
   try {
       write_index_file(curr_dir, index_top+index_middle()+StaticAssets.index_bottom)
   } catch (e: Exception) {
