...
<html lang="en">
     ...
     <body>
       ...
       <ul>
          <li><a style="display:block; width:100%" href="./.." aria-label="Parent directory"><span aria-hidden="true">&#x21B0;</span> ..</a></li>
""" 
    ...
    val index_middle = fun():String{ 
        ...
        dir_files.forEach {
           if((it.getName() !in exclude) && (it != curr_dir)) {
              l += """          <li><a style="display:block; width:100%" href="${if (it.isDirectory()) { "./${it.getName().urlEncodePath()}/" } else { "./${it.getName().urlEncodePath()}" }}"><span aria-hidden="true">${if (it.isDirectory()) { "&#128193;" } else { "&rtrif;" }}</span> ${it.getName().escapeHtml()}</a></li>"""+"\n"
           }
        }
