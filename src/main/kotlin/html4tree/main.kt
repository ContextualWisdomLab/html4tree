...
private const val MAX_IGNORE_FILE_BYTES = 1024 * 1024
...
/**
 * Thrown when a security policy file (e.g., .html4ignore) exists but cannot be read securely,
 * ensuring fail-closed behavior to prevent unauthorized directory traversal and information exposure.
 * This also mitigates TOCTOU (Time-of-Check to Time-of-Use) attacks.
 */
...
internal fun read_ignore_policy_bytes(
    ...
    while (true) {
        ...
        if (read == 0) continue
        total += read
        if (total > MAX_IGNORE_FILE_BYTES) {
            throw IgnoreFileReadException("Policy file exceeds the byte limit")
        }
...
internal fun process_ignore_file_with_validation_hook(
    ...
    val ignore_file = curr_dir.toPath().resolve(ignore_filename)
    val files_to_exclude = mutableSetOf<String>()

    if (Files.exists(ignore_file, LinkOption.NOFOLLOW_LINKS)) {
        val attributes = try {
            Files.readAttributes(ignore_file, BasicFileAttributes::class.java, LinkOption.NOFOLLOW_LINKS)
        } catch (_: Exception) {
            throw IgnoreFileReadException("Policy file metadata cannot be read safely")
        }
        if (!attributes.isRegularFile || attributes.isSymbolicLink) {
            throw IgnoreFileReadException("Policy file is not a regular no-follow file")
        }
        ...
        val policyText = try {
            Files.newByteChannel(
                ...
                afterValidation()
                val bytes = read_ignore_policy_bytes(channel)
            ...
            throw IgnoreFileReadException("Policy file cannot be opened, read, or decoded safely")
        }

        for ((lineIndex, line) in policyText.lineSequence().withIndex()) {
