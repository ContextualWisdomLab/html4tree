...
/**
 * Exception thrown when a security policy file (e.g., .html4ignore) cannot be read.
 * This ensures fail-closed behavior to prevent TOCTOU vulnerabilities and accidental publication of sensitive files.
 */
...
/**
 * Processes the ignore file in a directory to determine which files should be excluded.
 * Enforces fail-closed behavior: if an ignore file is present in the directory snapshot
 * but cannot be read (e.g., due to permissions, removal after listing, or read errors),
 * it throws [IgnoreFileReadException] to suppress publication and mitigate TOCTOU vulnerabilities.
 */
fun process_ignore_file(
    curr_dir: File,
    dirFilesNames: Array<String>? = null,
    useLines: (File, (Sequence<String>) -> Unit) -> Unit = { file, block -> file.useLines { block(it) } }
): Set<String> {

    val ignore_filename = ".html4ignore"
 
    val ignore_file = File(curr_dir, ignore_filename)

    val files_to_exclude = mutableSetOf<String>()

    val hasIgnoreFile = dirFilesNames?.contains(ignore_filename) ?: ignore_file.exists()

    if (hasIgnoreFile) {
        if (!ignore_file.isFile || Files.isSymbolicLink(ignore_file.toPath()) || !ignore_file.canRead() || ignore_file.length() > 1048576) {
            throw IgnoreFileReadException("Ignore file present but inaccessible or invalid")
        }

        val ignored_matchers = mutableListOf<java.nio.file.PathMatcher>()
        try {
            useLines(ignore_file) { lines ->
            ...
            throw IgnoreFileReadException("Failed to read ignore file due to I/O error", e)
        }
...
fun process_dir(curr_dir: File, excludeSet: Set<String>? = null, dirFiles: Array<File>? = null){
    ...
    val exclude: Set<String> = try {
        excludeSet ?: process_ignore_file(curr_dir)
    } catch (e: IgnoreFileReadException) {
        return
    }
