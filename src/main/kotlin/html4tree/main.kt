...
private object HtmlEscapeHelper {
    @JvmField
    val HTML_ESCAPE_MAP = Array<String?>(128) { null }.apply {
        this['&'.toInt()] = "&amp;"
        this['<'.toInt()] = "&lt;"
        this['>'.toInt()] = "&gt;"
        this['"'.toInt()] = "&quot;"
...
// Replaces the `when` conditional jump table with a direct array lookup for characters < 128.
// This significantly speeds up character mapping in the hot path.
fun String.escapeHtml(): String {
    var sb: StringBuilder? = null
    for (i in 0 until this.length) {
        val c = this[i]
        val cInt = c.toInt()
        val replacement = if (cInt < 128) HtmlEscapeHelper.HTML_ESCAPE_MAP[cInt] else null
        if (replacement != null) {
            if (sb == null) {
