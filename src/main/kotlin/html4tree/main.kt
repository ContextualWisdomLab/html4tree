    ...
    val index_middle = fun():String{ 
        ...
        dir_files.forEach {
           ...
           if (!fileName.isHiddenFile() && fileName !in exclude) {
               ...
               if (!isSymbolicLink) {
                  // ⚡ Bolt Performance Optimization: Cache expensive escaping and encoding results
                  val escapedFileName = fileName.escapeHtml()
                  val encodedName = fileName.urlEncodePath()
                  val encodedHref = if (isLinkedDirectory) { "./${encodedName}/" } else { "./${encodedName}" }
                  val ariaLabel = "${escapedFileName} ${if (isLinkedDirectory) { "디렉토리" } else { "파일" }}"
                  val icon = if (isLinkedDirectory) { "&#128193;" } else { "&#128196;" }
                  l.append("""          <li><a class="dir-link" href="${encodedHref}" aria-label="${ariaLabel}" title="${ariaLabel}"><span class="icon" aria-hidden="true">${icon}</span> <span>${escapedFileName}</span></a></li>""")
                  l.append('\n')
               }
           }
        }
