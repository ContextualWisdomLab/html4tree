...
/**
 * Exception thrown when an ignore file exists in the directory snapshot but becomes
 * inaccessible or is invalid at the time of reading, enforcing a fail-closed security boundary
 * against TOCTOU race conditions.
 */
...
fun process_ignore_file(curr_dir: File, dirFilesNames: Array<String>? = null): Set<String> {
    ...
    val isIgnoreFilePresent = ignore_file.exists()
    if (isIgnoreFilePresent) {
        if (!ignore_file.isFile || Files.isSymbolicLink(ignore_file.toPath()) || !ignore_file.canRead() || ignore_file.length() > 1048576) {
            throw IgnoreFileReadException("Ignore file $ignore_filename is present but inaccessible, invalid, or too large.")
        }
