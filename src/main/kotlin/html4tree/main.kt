...
fun write_index_file(
    curr_dir: File,
    content: String,
    moveFile: (java.nio.file.Path, java.nio.file.Path) -> java.nio.file.Path = { src, dest -> Files.move(src, dest, StandardCopyOption.ATOMIC_MOVE, StandardCopyOption.REPLACE_EXISTING) },
    fallbackMoveFile: (java.nio.file.Path, java.nio.file.Path) -> java.nio.file.Path = { src, dest -> Files.move(src, dest, StandardCopyOption.REPLACE_EXISTING) }
) {
    ...
    try {
        ...
        try {
            ...
            fallbackMoveFile(tempPath, indexPath)
        }
    } finally {
