...
fun write_index_file(
    curr_dir: File,
    content: String,
    filesMove: (java.nio.file.Path, java.nio.file.Path, Array<out java.nio.file.CopyOption>) -> java.nio.file.Path = Files::move
) {
    ...
    try {
        ...
        try {
            filesMove(tempPath, indexPath, arrayOf(StandardCopyOption.ATOMIC_MOVE, StandardCopyOption.REPLACE_EXISTING))
        } catch (e: java.nio.file.AtomicMoveNotSupportedException) {
            filesMove(tempPath, indexPath, arrayOf(StandardCopyOption.REPLACE_EXISTING))
        }
    } finally {
