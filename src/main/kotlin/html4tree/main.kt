...
fun process_ignore_file(curr_dir: File, dirFilesNames: Array<String>? = null): Set<String> {
    ...
    (dirFilesNames ?: curr_dir.list())?.forEach {
        if (it.isHiddenFile() || it.endsWith("~")) {
            ...
            if (normalizedName in Constants.defaultSensitiveFileNamesLowercase) {
                files_to_exclude.add(it)
            } else {
                val dotIndex = normalizedName.lastIndexOf('.')
                if (dotIndex != -1 && normalizedName.substring(dotIndex) in Constants.defaultSensitiveExtensionsSet) {
                    files_to_exclude.add(it)
                }
            }
        }
