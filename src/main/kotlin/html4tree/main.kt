...
a:hover .name, a:focus-visible .name {
...
<html lang="ko">
     ...
     <body>
       <main>
         ...
         <ul role="list">
            <li><a class="dir-link" href="./.." title="상위 디렉토리로 이동"><span class="icon" aria-hidden="true">&#x21B0;</span> <span class="name" aria-hidden="true">..</span> <span class="visually-hidden">상위 디렉토리로 이동</span></a></li>
""" 
        ...
        dir_files.forEach {
           ...
           if (!fileName.isHiddenFile() && fileName !in exclude) {
               ...
               if (!isSymbolicLink) {
                  ...
                  l.append("""          <li><a class="dir-link" href="${encodedHref}" title="${ariaLabel}"><span class="icon" aria-hidden="true">${icon}</span> <span class="name">${fileName.escapeHtml()}</span> <span class="visually-hidden">${typeLabel}</span></a></li>""")
                  l.append('\n')
               }
           }
        }
