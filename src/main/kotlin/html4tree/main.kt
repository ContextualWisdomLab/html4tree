...
<html lang="ko">
     <head>
        ...
        <title>${if (curr_dir.name.isEmpty()) curr_dir.absolutePath.escapeHtml() else curr_dir.name.escapeHtml()}</title>
        ${css}
     </head>
     <body>
       <main>
         <h1>${if (curr_dir.name.isEmpty()) curr_dir.absolutePath.escapeHtml() else curr_dir.name.escapeHtml()}</h1>
