...
fun process_ignore_file(curr_dir: File, dirFilesNames: Array<String>? = null): Set<String> {
    ...
    // ⚡ Bolt Performance Optimization: 디렉토리 목록 조회 I/O 캐싱
    // `dirFilesNames`가 null일 때 `curr_dir.list()`가 중복으로 호출되는 것을 방지합니다.
    val directory_files = dirFilesNames ?: curr_dir.list()
