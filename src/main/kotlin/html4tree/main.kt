...
/**
 * Exception thrown when a security policy file exists but cannot be read securely,
 * enforcing fail-closed behavior to mitigate TOCTOU vulnerabilities.
 */
...
fun process_ignore_file(
    ...
    readLines: (File, (Sequence<String>) -> Unit) -> Unit = { f, action -> f.useLines(Charsets.UTF_8, action) }
): Set<String> {
    ...
    if (ignore_file.exists()) {
        if (!ignore_file.isFile || Files.isSymbolicLink(ignore_file.toPath()) || !ignore_file.canRead()) {
            throw IgnoreFileReadException("Policy file exists but is unreadable or invalid: $ignore_file_path")
        }
        if (ignore_file.length() > 1048576) {
            throw IgnoreFileReadException("Policy file exceeds maximum size limit: $ignore_file_path")
        }

        val ignored_matchers = mutableListOf<java.nio.file.PathMatcher>()

        try {
            readLines(ignore_file) { lines ->
            ...
            throw IgnoreFileReadException("Failed to read policy file due to IO error: $ignore_file_path")
        }
