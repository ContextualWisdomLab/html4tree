...
<html lang="ko">
     <head>
        ...
        <title>${(curr_dir.getName().takeIf { it.isNotEmpty() } ?: curr_dir.absolutePath).escapeHtml()}</title>
        ${css}
     </head>
     <body>
       <main>
         <h1>${(curr_dir.getName().takeIf { it.isNotEmpty() } ?: curr_dir.absolutePath).escapeHtml()}</h1>
