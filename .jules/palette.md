...
## 2024-08-18 - [BiDi (Bidirectional) Text 지원 강화]
**Learning:** 디렉토리 또는 파일 이름이 RTL(Right-To-Left) 문자로 작성되었을 때, 전체 페이지가 LTR(Left-To-Right) 레이아웃인 경우 이름이 잘못 표시될 수 있습니다.
**Action:** 양방향 텍스트(BiDi)의 정확한 렌더링을 위해 동적 텍스트를 포함하는 요소(예: `<h1>`, `<span>`)에 `dir="auto"` 속성을 추가하여 브라우저가 해당 콘텐츠의 방향을 스스로 판단하고 올바르게 표시하도록 구성하십시오.
