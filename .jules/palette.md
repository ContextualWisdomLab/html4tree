...
## 2026-08-19 - 빈 디렉토리 상태의 접근성 개선 (role="status" 안티패턴 제거)
**Learning:** 초기 로드 시 렌더링되는 정적 요소(예: 빈 디렉토리 메시지)에 `role="status"`와 같은 ARIA live region 역할을 부여하는 것은 접근성 안티패턴입니다. 스크린 리더가 콘텐츠를 중복해서 읽거나 페이지 읽기 흐름을 방해할 수 있습니다.
**Action:** 정적인 UI 상태 요소에는 불필요한 ARIA 역할을 지양하고 의미에 맞는 HTML 태그와 CSS 구조에 의존합니다. ARIA live region은 동적으로 발생하는 상태 변화를 알릴 때만 사용합니다.
