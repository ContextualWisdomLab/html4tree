...
## 2026-07-29 - 파일 시스템 루트의 빈 이름 방지
**Learning:** 파일 시스템 루트(예: `/`)에 대해 `File.getName()`을 호출하면 빈 문자열이 반환되어, 생성된 HTML의 `<title>` 및 `<h1>` 요소가 비워지는 접근성 문제가 발생합니다.
**Action:** 항상 파일 이름이 빈 문자열인지 확인하고(예: `ifEmpty { absolutePath }`), 비어 있는 경우 절대 경로를 폴백으로 제공하여 빈 제목 태그 생성을 방지하십시오.
