...
## 2024-08-18 - 파일 및 디렉토리 이름 양방향(BiDi) 텍스트 렌더링 지원
**학습:** 아랍어, 히브리어 등 RTL(우에서 좌로) 언어로 작성된 파일이나 디렉토리 이름이 기본 LTR(좌에서 우로) 환경에서 렌더링될 때 문자 순서나 텍스트 정렬이 깨져 혼란을 주거나 읽을 수 없게 되는 문제가 발생할 수 있습니다.
**조치:** 텍스트를 포함하는 요소(예: 파일/디렉토리 이름이 표시되는 `<span>` 및 `<h1>`)에 항상 `dir="auto"` 속성을 추가하여, 브라우저가 콘텐츠 자체의 언어 방향을 감지하고 BiDi(양방향) 텍스트를 올바르게 렌더링하도록 보장하십시오.
