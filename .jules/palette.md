...
## 2026-08-17 - nav 랜드마크의 번역 호환성 개선
**학습:** `aria-label` 속성으로 제공된 `<nav>`의 접근성 이름은 많은 브라우저 자동 번역 도구에서 무시되어 번역되지 않은 채로 스크린 리더에게 읽히는 다국어 문제가 있습니다. 기존의 span 처리와 유사하나, 랜드마크 영역의 경우 `aria-labelledby`를 사용하여 제목을 연결하는 것이 시맨틱하게 더 적합합니다.
**조치:** 랜드마크 등 주요 요소에 이름을 부여할 때는 `aria-labelledby`를 사용하여, `.visually-hidden` 클래스가 적용된 숨겨진 일반 텍스트 헤딩 요소(예: `<h2>`)와 연결함으로써 브라우저 번역기 및 스크린 리더와 완벽히 호환되게 구성하십시오.
