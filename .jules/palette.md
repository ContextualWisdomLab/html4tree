...
## 2026-08-18 - 랜드마크의 다국어 번역 호환성 개선
**학습:** `aria-label` 속성으로 지정된 네비게이션 랜드마크의 이름은 브라우저 기본 번역기(예: Chrome 번역)에 의해 번역되지 않아, 언어가 변환되어도 화면 판독기에서는 원문으로 읽히는 문제가 발생할 수 있습니다.
**조치:** 랜드마크(예: `<nav>`)에 접근 가능한 이름을 제공할 때 `aria-label` 대신 `<h2 id="..." class="visually-hidden">`와 같은 시각적으로 숨겨진 요소를 만들고 `aria-labelledby`를 사용하여 연결함으로써, 브라우저 번역 도구가 텍스트를 정상적으로 인식하고 번역할 수 있도록 하십시오.
