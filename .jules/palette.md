...
## 2024-09-03 - 정적 빈 상태 콘텐츠의 불필요한 ARIA Live 속성 방지
**Learning:** 초기 페이지 로드 시 렌더링되는 정적인 빈 디렉토리 메시지에 `role="status"`(ARIA live region)를 부여하면, 스크린 리더가 불필요하게 해당 내용을 반복해서 읽거나 페이지 전체의 정상적인 읽기 흐름을 방해하는 안티패턴이 됩니다.
**Action:** 비동기적으로 추가되는 콘텐츠가 아닌 정적으로 렌더링되는 빈 상태 메시지에는 `role="status"` 또는 `aria-live` 속성을 사용하지 마십시오.
