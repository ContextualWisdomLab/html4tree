...
**학습:** 명시적인 flexbox 줄바꿈이 없는 유동적인 레이아웃에서 매우 긴 파일 이름은 앞의 인라인 아이콘 아래로 텍스트가 줄바꿈되어 시각적 계층 구조와 정렬을 깨뜨릴 수 있습니다.
**조치:** 파일/디렉토리 목록을 항상 `align-items: flex-start` 속성을 가진 flex 컨테이너로 감싸고, 모바일 장치에서 고정 너비 아이콘 옆에 텍스트가 깔끔하게 줄바꿈되도록 `<span style="overflow-wrap: anywhere;">`(또는 컨테이너의 전역 `overflow-wrap`)로 감싼 텍스트 콘텐츠 옆에 `flex-shrink: 0` 속성을 사용하여 아이콘을 격리하십시오.
...
**학습:** 화면 판독기를 사용하지 않는 비장애인 사용자에게 네이티브 툴팁 없이 아이콘이나 기술적 기호(`..` 등)에만 의존하는 것은 혼란을 줄 수 있습니다. `aria-label`과 일치하는 `title` 속성이 동등한 정보를 제공합니다.
...
## 2024-07-26 - 빈 디렉토리 상태의 시각적 일관성 개선
**학습:** 빈 디렉토리 상태에서 폴더가 열려 있는 아이콘(`&#128194;`)을 추가하면 사용자가 상태를 직관적으로 이해하기 좋습니다. 이때 아이콘은 장식용이므로 스크린 리더가 불필요하게 읽지 않도록 `aria-hidden="true"`를 적용해야 합니다.
**조치:** 향후 장식용 이모지나 아이콘을 추가할 때는 반드시 `aria-hidden="true"`를 적용하고, 시각적 계층이 무너지지 않도록 Flexbox를 사용하여 텍스트와 정렬을 맞춥니다.
