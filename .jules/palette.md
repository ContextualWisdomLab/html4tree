...
## 2026-07-25 - 빈 상태 메시지에 대한 스크린 리더 인지도 향상
**Learning:** 스크린 리더는 빈 디렉토리와 같은 정적 상태 메시지가 단순 텍스트로 렌더링될 때 그 중요성이나 상태를 명확히 인지하지 못할 수 있습니다.
**Action:** 시각적 피드백 외에도 스크린 리더가 상태 메시지임을 정확히 식별할 수 있도록 상태 메시지를 포함하는 요소에 항상 `role="status"`를 명시적으로 추가하십시오.
