...
## 2024-09-20 - 내비게이션 요소의 접근성 및 번역 호환성 향상
**학습:** `nav` 요소에서 `aria-label` 속성만 사용하면 화면 판독기가 요소를 인식할 수 있지만, 브라우저 내장 번역 도구는 해당 텍스트를 번역하지 않아 다국어 접근성(i18n) 문제를 일으킵니다. 또한 헤딩 단위 탐색을 이용하는 사용자에게 누락됩니다.
**조치:** 내비게이션(`nav`) 컨테이너를 구성할 때 `aria-label` 대신 내부에 `.visually-hidden` 유틸리티 클래스가 적용된 숨겨진 `<h2>` 요소를 추가하고 `aria-labelledby` 속성을 사용하십시오. 이를 통해 시각적으로는 보이지 않지만, 화면 판독기의 헤딩 내비게이션을 지원하고 브라우저가 일반 텍스트 노드로 인식하여 올바르게 번역할 수 있도록 보장하십시오.
