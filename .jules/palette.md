...
## 2026-08-17 - 빈 디렉토리 상태의 접근성 개선
**Learning:** 초기 페이지 로드 시 렌더링되는 정적 요소(예: 빈 디렉토리 메시지)에 `role="status"`와 같은 ARIA 라이브 리전을 적용하는 것은 스크린 리더가 내용을 중복해서 읽거나 페이지 읽기 흐름을 방해할 수 있는 안티패턴입니다.
**Action:** 정적 텍스트나 초기 로드 시 표시되는 메시지에는 불필요한 라이브 리전 역할을 추가하지 않고 의미론적인 HTML 구조만 유지합니다.
