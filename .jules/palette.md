...
## 2024-08-18 - 브라우저 자동 번역을 위한 aria-label 대체 패턴
**Learning:** `aria-label` 속성은 브라우저 내장 번역기(예: Chrome Translate)가 번역하지 못하는 경우가 많아, 번역 기능을 사용하는 다국어 환경의 스크린 리더 사용자에게 현지화되지 않은 접근성 텍스트를 제공하는 문제가 있습니다.
**Action:** 접근성을 위한 숨김 텍스트가 번역기의 대상이 될 수 있도록 `aria-label` 대신 `.visually-hidden` CSS 유틸리티를 적용한 `<span>` 요소를 HTML 본문 내에 배치하여 사용하십시오.
