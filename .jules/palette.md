...
## 2024-09-02 - 링크에 시각적/촉각적 피드백 제공 (a:active)
**Learning:** 상호작용 가능한 요소(링크, 버튼 등)에 대해 `:hover` 및 `:focus-visible` 상태만 제공하고 `:active` 상태가 누락되는 경우가 많습니다. 클릭/터치할 때 즉각적인 시각적 및 촉각적 피드백이 없으면 사용자는 입력이 등록되었는지 불확실성을 느낄 수 있습니다.
**Action:** 항상 상호작용 요소에 대해 `:active` 상태(보통 `:hover`나 `:focus` 상태와 동일하거나 더 눈에 띄는 스타일)를 추가하여 즉각적인 피드백을 제공하십시오.
