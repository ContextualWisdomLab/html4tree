...
## 2024-08-26 - 동적 텍스트를 위한 양방향 텍스트(BiDi) 지원
**학습:** 디렉토리 및 파일 이름은 사용자가 생성하는 콘텐츠이므로, 아랍어나 히브리어와 같은 우측에서 좌측으로(Right-to-Left, RTL) 읽는 언어가 포함될 수 있습니다. 기존의 레이아웃(좌측에서 우측으로 읽는 LTR)에 이러한 텍스트가 렌더링될 때 텍스트가 깨지거나 정렬이 어긋나는 문제가 발생할 수 있습니다.
**조치:** 동적으로 생성되는 텍스트를 포함하는 요소(예: `<h1>` 및 파일 이름을 표시하는 `<span>`)에 `dir="auto"` 속성을 추가하여, 브라우저가 해당 콘텐츠의 언어 방향을 자동으로 감지하고 혼합된 방향의 텍스트(BiDi)를 올바르게 렌더링하도록 보장하십시오.
