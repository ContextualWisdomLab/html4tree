...
## 2024-09-23 - 랜드마크 접근성: 번역기 호환성을 위한 aria-labelledby 사용
**학습:** `<nav>`와 같은 랜드마크 요소에 `aria-label`을 사용하여 접근성 이름을 제공하면, 브라우저 번역 도구가 해당 속성을 번역하지 못해 언어가 변경되어도 화면 판독기가 원본 언어로 읽어 다국어 환경에서 혼란을 야기할 수 있습니다.
**조치:** 랜드마크에 이름을 제공할 때, `aria-label` 대신 시각적으로 숨겨진 제목 요소(예: `<h2 id="nav-heading" class="visually-hidden">`)를 사용하고 랜드마크에는 `aria-labelledby="nav-heading"`을 적용하여 화면 판독기와 번역 도구 모두에서 올바르게 처리될 수 있도록 하십시오.
