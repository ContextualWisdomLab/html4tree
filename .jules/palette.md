...
## 2024-09-26 - 엔트리 시각적 밑줄 문제 수정
**Learning:** `a:hover span:last-child`와 같은 구조적 선택자를 사용할 때 마크업 구조가 변경되어 마지막 요소가 `span`이 아닌 경우 (예: 스크린 리더용 `.visually-hidden` 요소가 추가되었을 때) 원하는 텍스트 요소(엔트리 이름)에 밑줄 등의 시각적 스타일링이 올바르게 적용되지 않습니다. 이는 사용자의 시각적 호버 피드백을 저하시킵니다.
**Action:** 구조적 가상 클래스(pseudo-class) 대신 명시적인 시맨틱 클래스(`.entry-label`)를 추가하여, 스타일링이 DOM 구조의 변화에 영향을 받지 않고 안정적으로 유지되도록 해야 합니다. 상위 디렉토리 링크(`..`) 및 하위 파일/디렉토리 링크의 텍스트에 `.entry-label` 클래스를 적용했습니다.
