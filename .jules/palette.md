...
## 2024-05-19 - Use Semantic Classes over Structural Pseudo-classes for Interactions
**Learning:** Structural pseudo-classes like `:last-child` are brittle. When visually hidden elements (like screen reader text) are added, `:last-child` matches the hidden element instead of the intended visible text, breaking hover effects (like underline).
**Action:** Use semantic classes (e.g., `.entry-label`) on the elements that actually need the visual styling, rather than relying on their position in the DOM structure.
