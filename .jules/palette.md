...
## 2026-07-19 - 빈 디렉토리 이름에 대한 폴백 제공 (접근성 향상)
**학습:** 파일 시스템 루트와 같이 `File.getName()`이 빈 문자열을 반환하는 디렉토리의 경우, HTML 생성 시 `<title>` 및 `<h1>` 태그가 비어 있게 되어 화면 판독기 사용자에게 페이지의 목적을 전달하지 못하고 접근성을 심각하게 해칩니다.
**조치:** 디렉토리 이름이 빈 문자열인 경우 `<title>`과 `<h1>` 등 UI 요소에 절대 경로(`absolutePath`)를 제공하여(폴백) 접근성과 유용성을 유지하십시오.
