...
## 2026-09-16 - 시각적 숨김 요소에 대한 의사 클래스 스타일링
**Learning:** `.visually-hidden` 클래스를 사용하여 텍스트(예: "파일", "디렉토리")를 구조적으로 숨겼을 때, CSS에서 `:last-child` 같은 구조적 의사 클래스(pseudo-class)를 사용하면 숨겨진 요소 때문에 의도하지 않은 시각적 스타일링(예: `:hover` 시 아이콘과 파일명이 아닌 숨겨진 텍스트 영역에 언더라인 적용)이 깨질 수 있습니다.
**Action:** DOM에 숨겨진 요소를 추가할 때는 구조적 의사 클래스(`:last-child`, `:nth-last-child()` 등)에 의존하여 스타일을 적용하는 대신, 시각적으로 노출된 특정 요소에 의미 있는 클래스(예: `.entry-label`)를 부여하고 해당 클래스에 스타일을 적용하여 시각적 디자인이 깨지지 않도록 하십시오.
