...
## 2026-07-26 - 빈 디렉토리 메시지 접근성 개선
**Learning:** 정적인 HTML 생성 결과물에서도 스크린 리더 등 보조 기기가 상태 변화나 빈 상태를 올바르게 인지할 수 있도록 적절한 ARIA role(예: `role="status"`)을 제공하는 것이 중요함.
**Action:** 빈 디렉토리 등 상태를 나타내는 메시지 요소에 `role="status"`를 추가하여 접근성을 높일 것.
