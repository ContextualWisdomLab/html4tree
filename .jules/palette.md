...
## 2024-08-05 - 빈 상태 아이콘 및 접근성 개선
**학습:** 디렉토리가 비어 있을 때 단순히 텍스트만 표시하는 것보다 컨텍스트를 제공하는 아이콘(예: 열린 폴더 아이콘 `&#128194;`)을 함께 표시하면 빈 상태임을 훨씬 직관적으로 인지할 수 있습니다. 이때 아이콘은 장식용이므로 화면 판독기가 읽지 않도록 처리해야 합니다.
**조치:** 빈 상태를 나타내는 요소에 시각적 단서를 추가할 때 명확한 아이콘(예: `&#128194;`)을 사용하고, 반드시 `<span aria-hidden="true">`로 감싸 화면 판독기 사용자에게 불필요한 소음이 발생하지 않도록 해야 합니다. 또한, 아이콘과 텍스트를 나란히 정렬하기 위해 flexbox(`display: flex; align-items: flex-start; gap: 0.5rem;`)를 사용하십시오.
