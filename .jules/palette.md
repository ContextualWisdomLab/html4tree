...
## 2026-08-22 - 정적 요소에 대한 ARIA 라이브 리전 안티패턴
**Learning:** 초기 페이지 로드 시 렌더링되는 정적 요소(예: 빈 디렉토리 메시지)에 role="status"와 같은 ARIA 라이브 리전 역할을 적용하면, 스크린 리더가 콘텐츠를 중복해서 읽거나 페이지 읽기 흐름을 방해하는 접근성 안티패턴이 발생할 수 있음을 배웠습니다.
**Action:** 라이브 리전(role="status", aria-live 등)은 동적으로 업데이트되는 콘텐츠에만 사용하고, 초기 로드되는 정적 요소에는 시맨틱 HTML을 사용하는 것으로 충분하므로 불필요한 ARIA 속성을 피해야 합니다.
