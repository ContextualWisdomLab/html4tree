...
## 2026-08-16 - 양방향 파일명을 격리하고 번역 가능한 유형 라벨과 함께 표시
**Learning:** `dir="auto"`만 넣으면 파일명 방향은 맞지만, 같은 링크 안의 한국어 유형 라벨과 `title` 속성은 유니코드 양방향 알고리즘에 의해 재배치될 수 있다. #459 이후 `span:last-child` 밑줄은 숨겨진 유형 라벨을 가리키므로 호버 피드백도 사라진다.
**Action:** 보이는 파일명에 `class="entry-name" dir="auto"`와 `unicode-bidi: isolate`를 적용하고, 유형 설명은 형제 `.visually-hidden`으로 두며, `title`에는 U+2068/U+2069 격리를 넣는다. 호버 밑줄은 `.entry-name`을 대상으로 한다.
