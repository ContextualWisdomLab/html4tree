...
## 2024-08-20 - 내비게이션 요소의 aria-label을 aria-labelledby와 숨겨진 heading으로 대체
**Learning:** `<nav>` 요소에 `aria-label`을 사용하는 경우 Chrome 번역과 같은 브라우저 번역 도구가 이를 번역하지 못하는 문제가 발생할 수 있습니다. 또한 스크린 리더 사용자가 '헤딩으로 탐색(navigate by heading)' 단축키를 사용하여 내비게이션을 찾지 못할 수 있습니다.
**Action:** `<nav>` 요소 내부를 설명할 때, `aria-label` 대신 내부에 시각적으로 숨겨진(`.visually-hidden`) 헤딩 요소(예: `<h2>`)를 배치하고 `<nav>`에 `aria-labelledby`를 사용하여 이 헤딩 요소를 참조하도록 하십시오. 이를 통해 번역 도구 호환성과 스크린 리더의 헤딩 탐색 접근성을 모두 향상시킬 수 있습니다.
