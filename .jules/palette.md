...
## 2026-08-25 - 정적 빈 디렉토리 메시지의 ARIA live region 속성 제거
**Learning:** 빈 디렉토리 메시지와 같이 페이지가 로드될 때 처음부터 렌더링되는 정적 요소에 `role="status"`와 같은 ARIA live region 속성을 적용하는 것은 접근성 안티패턴입니다. 이는 스크린 리더가 컨텐츠를 불필요하게 중복해서 읽거나 페이지 읽기 흐름을 방해하게 만듭니다.
**Action:** 비동기적으로 업데이트되는 동적 상태 변경이 아닌, 초기 로드 시점부터 존재하는 정적 상태 메시지에는 `role="status"` 또는 `aria-live` 속성을 사용하지 마십시오.
