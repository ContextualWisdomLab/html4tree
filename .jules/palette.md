...
## 2024-08-20 - 숨김 요소(visually-hidden)와 CSS 구조 선택자 충돌
**Learning:** 접근성을 위해 스크린 리더 전용 요소(`.visually-hidden`)를 DOM 컨테이너의 마지막에 추가하면, 기존의 `:last-child`와 같은 CSS 구조 선택자가 더 이상 시각적 텍스트를 타겟팅하지 못하게 되어 링크의 hover 또는 focus 시 나타나는 밑줄 등의 시각적 피드백이 깨질 수 있습니다.
**Action:** 보이지 않는 접근성 요소를 추가할 때는 부모 컨테이너 내의 구조적 가상 클래스(예: `:last-child`)가 시각적 스타일링에 사용되고 있는지 항상 확인하고, 필요한 경우 `:nth-child` 등을 사용하여 대상이 정확히 유지되도록 스타일을 업데이트하십시오.
