...
## 2024-05-16 - 링크의 활성(Active) 상태 시각적 피드백
**Learning:** 사용자가 링크를 클릭하거나 터치할 때 즉각적인 시각적 피드백(Active 상태)이 없으면, 시스템이 응답하고 있는지 확신하기 어려워 여러 번 클릭하는 등의 불편함을 겪을 수 있습니다. 특히 모바일 환경에서 터치 피드백은 중요한 UX 요소입니다.
**Action:** 상호작용이 가능한 요소(`a` 태그 등)에 `:hover` 및 `:focus-visible`뿐만 아니라 `:active` 상태에 대한 스타일(예: 배경색을 약간 더 어둡게 변경)을 추가하여, 사용자가 클릭/터치하는 순간 즉각적이고 물리적인 피드백을 느낄 수 있도록 하십시오.
