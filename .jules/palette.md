...
## 2024-09-21 - 내비게이션 요소의 화면 판독기 헤딩 탐색 지원
**학습:** `nav` 요소에 `aria-label` 속성만 사용하면 화면 판독기가 탐색 랜드마크로 인식할 수 있지만, 헤딩(Heading) 단위로 페이지 구조를 탐색하는 사용자에게는 구조적 컨텍스트가 누락되어 빠른 내비게이션에 제약이 생깁니다.
**조치:** 내비게이션(`nav`) 컨테이너를 구성할 때 `aria-label` 대신 내부에 `.visually-hidden` 유틸리티 클래스가 적용된 숨겨진 `<h2>` 요소를 추가하고 `aria-labelledby` 속성으로 연결하십시오. 이를 통해 시각적 디자인을 유지하면서도 화면 판독기 사용자가 헤딩 탐색(예: 'H' 키)을 통해 내비게이션 영역을 쉽게 찾고 이동할 수 있도록 지원하십시오.
