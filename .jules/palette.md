...
## 2024-11-20 - 빈 디렉토리 상태의 시각적 메타포 일관성 개선
**학습:** 디렉토리가 비어 있을 때 텍스트만 표시하면 다른 일반 파일이나 디렉토리 항목들과 시각적 정렬 및 메타포가 맞지 않아 사용자가 즉각적으로 상태를 인지하기 어려울 수 있습니다.
**조치:** 비어 있는 디렉토리 메시지에도 다른 항목들과 동일한 구조(`display: flex`, 아이콘 공간 확보)를 적용하고, 일관된 시각적 메타포로 열린 폴더 아이콘(`&#128194;`)을 추가하여 사용자가 빈 디렉토리임을 빠르게 인지할 수 있도록 하십시오. 화면 판독기를 위해서는 반드시 `aria-hidden="true"`를 적용해야 합니다.
