...
## 2024-05-30 - Fix broken hover styles due to visually-hidden elements
**Learning:** When adding `.visually-hidden` text at the end of interactive elements (like `<a>` or `<button>`) for accessibility, CSS pseudo-classes like `:last-child` may break because the target element is no longer the last child in the DOM.
**Action:** Update fragile structural selectors (e.g., changing `:last-child` to `:nth-child(2)`) to accurately target the intended visual element when visually hidden text alters the DOM order.
