...
## 2024-05-18 - [Fix visually-hidden elements being styled on hover]
**Learning:** 숨겨진 스크린 리더 텍스트(`.visually-hidden`)가 부모 `<a>` 태그 내의 마지막 자식 요소일 때, 일반적인 CSS 선택자(예: `a:hover span:last-child`)를 사용하면 화면에 숨겨진 텍스트에도 포커스 스타일(예: 밑줄)이 잘못 적용되는 문제가 발생할 수 있습니다.
**Action:** 인터랙티브 요소 내에서 특정 콘텐츠를 시각적으로 꾸밀 때는 구조적 가상 클래스(pseudo-class)보다는 고유한 클래스(예: `.entry-name`)를 명시적으로 부여하여 스타일링하는 방식을 도입해야 합니다.
