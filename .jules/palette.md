...
## 2024-09-01 - 내비게이션 ARIA 라벨 다국어 번역 및 제목 탐색 호환성 개선
**Learning:** `<nav aria-label="...">` 속성은 Chrome Translate와 같은 브라우저 번역 도구가 텍스트를 번역하지 못하는 문제가 있으며, 화면 판독기 사용자가 '제목(heading) 단위 탐색' 단축키를 이용해 내비게이션 영역을 빠르게 찾는 데 도움을 주지 못합니다.
**Action:** `<nav>` 블록을 생성할 때 `aria-label` 대신 내부에 시각적으로 숨겨진 제목 요소(`<h2 id="nav-heading" class="visually-hidden">`)를 추가하고 `aria-labelledby="nav-heading"`을 사용하여 번역 호환성 및 화면 판독기 접근성을 모두 개선하십시오.
