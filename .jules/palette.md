...
## 2024-07-18 - 빈 디렉토리 상태 메시지 접근성 개선
**Learning:** 비어 있는 디렉토리를 안내하는 텍스트는 일반 텍스트로 존재할 경우, 스크린 리더 사용자가 해당 요소에 직접 포커스를 맞추기 전까지 상태를 인지하기 어렵습니다.
**Action:** 상태를 나타내는 메시지 요소에 `role="status"`를 추가하여, 스크린 리더가 상태 변화(비어 있음)를 동적으로 안내하도록 접근성을 개선했습니다.
