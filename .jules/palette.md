...
## 2024-08-18 - 다국어 호환 내비게이션 랜드마크 텍스트 및 모바일 탭 피드백
**학습:** `aria-label`은 브라우저 번역기에 의해 번역되지 않아 외국어 환경에서 원본 언어로 읽히는 문제가 있으며, 터치 디바이스에서는 링크를 탭할 때 `:hover`나 `:focus`가 즉시 반응하지 않아 피드백이 부족합니다.
**조치:** 내비게이션 등 중요한 랜드마크에 `aria-label` 대신 시각적으로 숨겨진(`.visually-hidden`) 일반 텍스트 요소를 `aria-labelledby`로 참조하여 번역기가 텍스트 변환을 할 수 있도록 하고, 모든 상호작용 요소(`a`)에 `:active` 스타일을 추가하여 터치 시 즉각적인 시각적 피드백을 제공하십시오.
