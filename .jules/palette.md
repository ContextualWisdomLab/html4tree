...
## 2024-05-16 - 브라우저 번역 호환성을 위한 랜드마크 텍스트 처리
**학습:** `<nav aria-label="디렉토리 목록">`과 같이 랜드마크 역할 요소에 `aria-label` 속성으로 부여된 숨겨진 이름은 브라우저의 기본 번역 기능(예: 크롬 번역)에 의해 번역되지 않고 원본 텍스트 그대로 유지됩니다. 이로 인해 다국어 사용자 및 화면 판독기 사용자가 랜드마크를 탐색할 때 잘못된 언어로 들리게 되는 호환성 문제가 발생합니다.
**조치:** 랜드마크 영역 등에 접근성 이름을 제공할 때는 번역기가 처리할 수 있는 실제 텍스트 노드(`<h2 id="nav-heading" class="visually-hidden">디렉토리 목록</h2>`)를 생성하고, 이를 `aria-labelledby` 속성과 연결(`<nav aria-labelledby="nav-heading">`)하여 다국어 지원과 화면 판독기 접근성을 모두 만족시키십시오.
