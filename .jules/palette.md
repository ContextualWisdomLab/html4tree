...
## 2026-08-18 - 파일 및 디렉토리명 양방향 텍스트(BiDi) 지원
**Learning:** LTR(Left-To-Right) 레이아웃 내에서 RTL(Right-To-Left) 문자가 포함된 파일이나 디렉토리명을 올바르게 렌더링하려면 브라우저가 해당 텍스트의 방향성을 자동으로 유추할 수 있어야 합니다. 그렇지 않으면 텍스트 표시가 깨질 수 있습니다.
**Action:** 텍스트를 포함하는 요소(예: `<h1>` 및 파일명을 감싸는 `<span>`)에 `dir="auto"` 속성을 추가하여 양방향 텍스트가 올바르게 렌더링되도록 하십시오.
