...
## 2026-08-18 - 정적 요소의 불필요한 ARIA 라이브 리전 제거
**Learning:** 초기 페이지 로드 시 렌더링되는 정적 요소(예: 빈 디렉토리 메시지)에 `role="status"`와 같은 ARIA 라이브 리전 역할을 적용하면, 스크린 리더가 내용을 중복해서 읽거나 페이지 읽기 흐름을 방해하는 접근성 안티패턴이 됩니다.
**Action:** 시각적 요소가 동적으로 업데이트되거나 사용자 상호 작용 결과로 나타나는 것이 아니라면 초기 로드 시 생성되는 정적 텍스트 요소에는 ARIA 라이브 리전(`role="status"`, `role="alert"` 등)을 적용하지 마십시오.
