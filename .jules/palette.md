...
**Learning:** 파일 시스템 루트 디렉토리(예: Linux/Mac의 `/` 또는 Windows의 `C:\`)를 처리할 때, Java의 `File.getName()`은 빈 문자열을 반환합니다. 이로 인해 생성된 HTML의 `<title>`과 `<h1>` 요소가 비어 있게 되어, 스크린 리더 사용자가 페이지의 맥락을 이해하기 어려워집니다.
