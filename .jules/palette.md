...
## 2024-08-20 - 랜드마크 라벨의 번역 호환성 개선
**학습:** 브라우저 번역 도구는 랜드마크(예: `<nav>`)에 사용된 `aria-label` 속성을 번역하지 않는 경우가 많아, 번역 후에도 스크린 리더가 원본 언어로 랜드마크를 읽게 되어 다국어 접근성이 떨어집니다.
**조치:** 랜드마크에 접근성 이름을 제공할 때 `aria-label` 대신 시각적으로 숨겨진 제목 요소(예: `<h2 class="visually-hidden">`)와 `aria-labelledby`를 짝지어 사용하여 번역 도구가 내용을 번역할 수 있도록 하십시오.
