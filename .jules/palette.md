...
## 2024-08-23 - BiDi(Bidirectional text) 텍스트 지원을 위한 dir="auto" 속성 추가
**Learning:** 파일 및 디렉토리 이름에 우에서 좌로 읽는(Right-To-Left) 문자가 포함되어 있을 경우, 좌에서 우로 읽는(Left-To-Right) 기본 레이아웃에서 텍스트가 깨지거나 잘못 렌더링될 수 있습니다.
**Action:** BiDi 텍스트가 올바르게 렌더링되도록, 텍스트가 포함된 특정 요소(예: `<h1>`, `<span>`)에 `dir="auto"` 속성을 추가하여 브라우저가 콘텐츠의 텍스트 방향을 자동으로 감지하고 렌더링하도록 합니다.
