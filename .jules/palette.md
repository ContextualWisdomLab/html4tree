...
## 2026-08-18 - 랜드마크의 다국어 번역 지원을 위한 aria-labelledby 사용
**Learning:** `<nav>` 랜드마크와 같은 요소에서 `aria-label` 속성을 사용하면 브라우저 번역 도구(예: Chrome Translate)가 이를 번역하지 못하는 경우가 발생합니다. 이로 인해 번역된 페이지를 사용하는 스크린 리더 사용자에게 원본 언어가 읽히게 되어 접근성에 악영향을 미칩니다.
**Action:** 접근성 높은 이름을 제공할 때, `aria-label` 속성 대신 `aria-labelledby`를 시각적으로 숨겨진(`.visually-hidden`) 헤딩(예: `<h2>`)과 연결하여 사용하십시오. 이렇게 하면 시각적으로는 숨겨져 있으면서도 브라우저가 일반 텍스트 노드로 인식하여 올바르게 번역할 수 있습니다.
