...
## 2024-05-18 - 인터랙티브 요소의 :active 상태 추가
**Learning:** 생성되는 정적 HTML에서 링크(디렉토리/파일) 클릭 시 시각적/촉각적 피드백(:active 상태)이 없으면 사용자가 클릭이 정상적으로 이루어졌는지 즉각적으로 인지하기 어려움.
**Action:** 접근성과 UX 향상을 위해 대화형 HTML 요소(링크 등) 생성 시 항상 `:active` 의사 클래스 스타일을 추가하여 클릭 이벤트 중 피드백을 제공하도록 함.
