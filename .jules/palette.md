...
## 2026-08-01 - 빈 디렉토리 상태의 접근성 개선
**Learning:** 정적인 HTML 생성 환경에서 화면 판독기가 빈 상태를 인식하게 하려면 role="status"를 부여해야 하며, 장식용 아이콘에는 반드시 aria-hidden="true"를 추가해야 혼란을 방지할 수 있습니다.
**Action:** 앞으로 상태 메시지를 표시하는 UI 요소를 추가할 때는 시각적 피드백과 함께 스크린 리더 호환성을 고려한 ARIA 역할을 기본적으로 적용해야 합니다.
