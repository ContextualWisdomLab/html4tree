...
## 2026-08-11 - 모바일 브라우저 테마 색상 동기화
**Learning:** `<meta name="color-scheme" content="light dark">` 만으로는 모바일 브라우저의 탭 바나 상태 표시줄 같은 네이티브 UI 요소의 색상까지 일치시킬 수 없어 테마 간 전환 시 시각적 단절감을 유발합니다.
**Action:** 모바일 브라우저 UI와의 매끄러운 통합을 위해 `<meta name="theme-color">` 태그를 `media="(prefers-color-scheme: ...)"` 속성과 함께 추가하여 다크/라이트 테마 색상(예: 배경색)과 동기화하십시오.
