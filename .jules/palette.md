...
## 2026-09-01 - 정적 요소에 대한 ARIA 실시간 영역(role="status") 오남용 제거
**Learning:** 빈 디렉토리 메시지와 같이 초기 페이지 로드 시 렌더링되는 정적 요소에 `role="status"`(ARIA live region)를 적용하면 스크린 리더가 페이지를 처음 읽을 때 콘텐츠를 불필요하게 중복해서 읽거나 읽기 흐름을 방해하는 접근성 안티패턴이 발생합니다.
**Action:** 동적으로 업데이트되는 콘텐츠(예: 비동기 작업 결과 알림)가 아닌 이상, 초기 렌더링되는 정적 메시지에는 `role="status"`와 같은 실시간 알림 속성을 추가하지 마십시오.
