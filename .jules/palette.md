...
## 2024-07-29 - 빈 디렉토리 상태의 스크린 리더 접근성 개선
**Learning:** 스크린 리더 사용자는 정적으로 생성된 빈 디렉토리 페이지에 접근했을 때, 시각적으로 표시되는 "이 디렉토리는 비어 있습니다." 메시지를 인지하지 못할 수 있습니다. 상태 변화나 현재 상태를 나타내는 UI 요소에는 적절한 ARIA 역할이 필요합니다.
**Action:** 생성되는 정적 HTML의 상태 메시지(예: 빈 상태)에 `role="status"`를 추가하여 스크린 리더가 해당 내용을 자동으로 읽어주도록 개선합니다.
