...
## 2026-07-27 - 파일시스템 루트 디렉토리의 제목(Title) 및 헤딩(Heading) 접근성 개선
**학습:** 루트 디렉토리(예: `/` 또는 `C:\`)의 경우 `File.getName()`이 빈 문자열을 반환합니다. 이로 인해 생성된 HTML에서 `<title>` 및 `<h1>` 태그가 비어 있게 되어 화면 판독기 사용자에게 혼란을 주고 접근성을 해치게 됩니다.
**조치:** 항상 `ifEmpty { absolutePath }`와 같은 폴백(fallback) 메커니즘을 제공하여, 빈 이름 대신 전체 경로라도 렌더링되도록 보장함으로써 문서에 유효한 제목과 시각적/구조적 헤딩이 존재하게 하십시오.
