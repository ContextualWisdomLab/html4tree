...
## 2026-07-10 - 브라우저 번역기 호환성을 위한 랜드마크 네이밍
**Learning:** 브라우저 번역 도구는 종종 `aria-label` 속성의 값을 번역하지 못하여, 다국어 환경에서 화면 판독기가 접근성 랜드마크를 원본 언어로 읽게 만드는 문제가 발생합니다.
**Action:** 접근성 랜드마크(예: `<nav>`)에 이름을 제공할 때, `aria-label` 대신 `aria-labelledby`를 시각적으로 숨겨진 요소(예: `<h2 id="nav-heading" class="visually-hidden">`)와 함께 사용하여 브라우저 번역기와의 호환성을 확보하십시오.
