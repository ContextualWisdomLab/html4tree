...
## 2024-11-20 - 시각적 Hover 피드백 복원
**Learning:** 컨테이너의 마지막 자식으로 `<span class="visually-hidden">`과 같은 시각적으로 숨겨진 요소를 추가하면, 호버 텍스트 밑줄과 같이 `:last-child`를 사용하는 CSS 선택자가 깨져서 접근성이 향상되더라도 시각적 상호작용 피드백이 실패하게 됩니다.
**Action:** 위치 기반 선택자(예: `:last-child`, `:nth-last-child()`)는 마크업 구조가 변경될 때 쉽게 깨질 수 있으므로, 명시적인 시맨틱 클래스(예: `.entry-label`)를 부여하여 타겟팅함으로써 견고한(robust) UI 스타일을 유지하십시오.
