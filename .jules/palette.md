...
## 2024-09-09 - Bidirectional (BiDi) 텍스트 지원 추가
**Learning:** 파일 서버 탐색 시, 디렉토리나 파일 이름에 양방향 텍스트(예: 아랍어/히브리어 + 라틴어 혼합)가 포함될 경우, 방향성이 브라우저에 의해 올바르게 처리되지 않아 텍스트 정렬 및 가독성에 심각한 문제를 야기할 수 있습니다.
**Action:** 항상 사용자 제어 텍스트가 포함된 HTML 요소(예: 파일명 `<span>`, 디렉토리명 `<h1>`)에 `dir="auto"`를 적용하고, `<title>` 태그나 `title="..."` 속성처럼 `dir="auto"`를 적용하기 어려운 곳에는 First Strong Isolate (FSI, `&#x2068;`)와 Pop Directional Isolate (PDI, `&#x2069;`)를 추가하여 BiDi 텍스트가 주변 고정 UI 요소의 방향성을 망가뜨리지 않도록 보호하십시오.
