...
## 2024-09-18 - 스크린 리더용 숨김 텍스트에 시각적 스타일 누수 방지
**Learning:** `span:last-child`와 같은 구조적 선택자를 사용할 경우 나중에 접근성을 위해 추가된 `.visually-hidden` 스크린 리더 텍스트에 예기치 않게 호버 스타일(예: 밑줄)이 적용되는 문제가 발생할 수 있음을 확인했습니다.
**Action:** 호버 상태 등 시각적 스타일이 적용되는 대상에는 구조적 선택자 대신 `.entry-name`과 같은 명시적인 콘텐츠 클래스를 지정하여 스크린 리더용 숨김 텍스트에 스타일이 누수되지 않도록 수정했습니다.
