...
## 2024-09-24 - 내비게이션 랜드마크의 번역 호환성
**Learning:** `nav` 등의 랜드마크 요소에 `aria-label`을 사용하여 접근성 이름을 제공하면 크롬 번역 등의 브라우저 내장 번역기가 이를 번역하지 못해 접근성이 저하되는 경우가 있습니다.
**Action:** 브라우저 번역 도구와의 호환성을 높이기 위해 `aria-label` 대신 `aria-labelledby`를 시각적으로 숨겨진(`.visually-hidden`) 헤더(예: `<h2>`)와 함께 사용하여 접근성 이름을 제공하십시오.
