...
## 2024-09-12 - 시각적으로 숨겨진 요소와 구조적 가상 클래스의 충돌
**Learning:** 접근성을 위해 화면 판독기용 텍스트(`visually-hidden` 클래스가 적용된 요소 등)를 DOM 컨테이너의 마지막에 추가할 경우, 기존의 `:last-child`와 같은 구조적 가상 클래스가 숨겨진 요소를 타겟팅하게 되어 의도했던 시각적 스타일(예: hover 시 밑줄)이 깨지는 문제가 발생할 수 있습니다.
**Action:** `visually-hidden`과 같은 시각적으로 숨겨진 요소를 DOM 구조에 추가할 때는, 주변의 CSS 구조적 가상 클래스(예: `:last-child`)가 의도하지 않은 숨겨진 요소를 타겟팅하여 기존 시각적 디자인을 손상시키지 않는지 확인하고 필요한 경우 구체적인 클래스나 `nth-child` 등으로 선택자를 업데이트하십시오.
