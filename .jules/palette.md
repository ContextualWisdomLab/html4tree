...
## 2026-11-20 - BiDi 텍스트 호환성을 위한 방향성(dir) 속성 추가
**Learning:** 파일 서버 탐색 시 디렉토리 또는 파일 이름이 양방향 텍스트(BiDi, 예: 아랍어와 영어가 혼합된 텍스트)를 포함할 수 있습니다. `dir` 속성을 명시하지 않으면 혼합된 언어가 렌더링될 때 문장 부호와 방향이 잘못 배치되어 접근성과 사용성이 심각하게 저하됩니다.
**Action:** 항상 사용자가 제어하는 텍스트(예: 디렉토리 또는 파일 이름)가 표시되는 요소(`<h1>` 및 `<span>`)에 `dir="auto"`를 추가하여, 브라우저가 해당 콘텐츠에 맞는 올바른 텍스트 방향을 렌더링하도록 보장하십시오.
