...
## 2024-07-20 - 빈 디렉토리 상태 메시지의 스크린 리더 접근성 개선
**Learning:** 정적 HTML 생성 시, 상태를 나타내는 메시지(예: 빈 디렉토리 안내 메시지)에 `role="status"`를 추가하면 스크린 리더가 이 상태 메시지를 적절하게 인식할 수 있어 접근성을 높일 수 있습니다.
**Action:** 사용자 인터페이스의 상태(비어있음, 진행중, 에러 등)를 나타내는 텍스트 요소에는 ARIA 역할인 `role="status"` 또는 그와 유사한 알림 역할을 항상 추가합니다.
