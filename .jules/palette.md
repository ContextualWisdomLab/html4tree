...
## 2024-09-16 - 브라우저 번역 도구와의 호환성을 위한 네비게이션 랜드마크 라벨링 개선
**학습:** `<nav>`와 같은 랜드마크 요소에 `aria-label`을 사용하여 접근성 이름을 제공하면, 브라우저의 기본 번역 도구가 해당 텍스트를 인식하지 못하여 언어가 변환되어도 스크린 리더에서 원본 언어로 읽히는 문제가 발생할 수 있습니다.
**조치:** 번역 도구와의 다국어 호환성을 보장하기 위해 `aria-label` 대신 `<h2 class="visually-hidden">`과 같이 시각적으로 숨겨진 실제 텍스트 요소를 사용하고, `aria-labelledby`를 통해 랜드마크를 연결하십시오.
