...
## 2024-08-11 - 루트 디렉토리의 빈 제목/헤딩 접근성 문제 해결
**Learning:** 파일 시스템의 루트 디렉토리(예: `/` 또는 드라이브 루트)는 `File.getName()` 호출 시 빈 문자열을 반환합니다. 이를 생성된 HTML의 `<title>`이나 `<h1>`에 그대로 사용하면 빈 요소가 렌더링되어 화면 판독기 사용자에게 페이지의 컨텍스트를 제공하지 못하고 접근성을 크게 해칩니다.
**Action:** 디렉토리 이름이 비어 있는 경우 서버의 절대 경로를 노출하여 정보 노출 취약점을 발생시키는 대신, `/`나 `Root`와 같은 일반적인 폴백 텍스트를 UI 요소에 제공하여 접근성과 보안을 동시에 확보하십시오.
