...
## 2024-09-12 - 구조적으로 숨겨진 요소와 CSS 선택자
**Learning:** 시각적으로 숨겨진 요소(`<span class="visually-hidden">`)를 DOM의 마지막에 추가하면, `:last-child`와 같은 CSS 구조적 가상 클래스가 예상치 못한 요소를 타겟팅하게 되어 기존의 시각적 스타일(예: 호버 시 밑줄 효과)이 깨질 수 있습니다.
**Action:** 시각적으로 숨겨진 접근성 요소를 추가할 때는, 주변의 CSS 구조적 가상 클래스(예: `:last-child`를 `:nth-child(2)`로 변경)를 검토하고 업데이트하여 시각적 스타일이 깨지지 않도록 보장하십시오.
