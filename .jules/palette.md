...
## 2024-08-20 - 혼합 문자열(BiDi) 디렉토리 및 파일명 렌더링 오류 방지
**학습:** 디렉토리 또는 파일명에 오른쪽에서 왼쪽으로 쓰는(RTL) 문자(예: 아랍어, 히브리어)가 포함될 경우, 주변의 왼쪽에서 오른쪽으로 쓰는(LTR) 텍스트 컨텍스트로 인해 파일 확장자나 기호의 위치가 잘못 렌더링되어 혼란을 야기할 수 있습니다.
**조치:** 렌더링 방향 독립성을 보장하기 위해 사용자 생성 텍스트가 표시되는 컨테이너(예: `<h1>`, `<span>`)에 항상 `dir="auto"`를 추가하여, 브라우저가 해당 콘텐츠의 언어에 맞게 양방향 텍스트(BiDi)를 올바르게 격리 및 표시하도록 하십시오.
