...
## 2024-08-18 - 시각적 숨김 요소(Visually Hidden) 추가로 인한 CSS 선택자 충돌 해결
**Learning:** 접근성 개선을 위해 링크 내부에 화면 판독기용 숨김 텍스트(`<span class="visually-hidden">`)를 마지막 자식 요소로 추가했을 때, 기존의 CSS `:last-child` 선택자에 의존하던 호버(Hover) 밑줄 효과가 깨지는 시각적 회귀(Visual Regression)가 발생했습니다. 숨겨진 요소에 시각적 효과가 적용되어 정작 화면에 보이는 텍스트에는 피드백이 제공되지 않았습니다.
**Action:** 텍스트 스타일링이나 호버 효과를 위해 HTML 구조에 의존적인 `:last-child`와 같은 가상 클래스(Pseudo-class) 선택자 사용을 피하고, 시각적 효과가 필요한 요소에 명시적인 클래스(예: `.name`)를 부여하여 DOM 구조 변경에 강건한(Robust) CSS를 작성하십시오.
