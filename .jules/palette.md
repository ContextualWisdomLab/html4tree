...
## 2024-09-01 - 내비게이션 랜드마크의 다국어 지원 및 스크린 리더 헤딩 탐색 개선
**Learning:** <nav> 태그에 직접 aria-label을 사용하면 브라우저 번역 도구(Chrome Translate 등)가 이를 텍스트 노드로 인식하지 못해 번역되지 않는 문제가 있으며, 스크린 리더의 헤딩 탐색 단축키(H)에 잡히지 않습니다.
**Action:** <nav aria-label="..."> 대신 <nav aria-labelledby="nav-heading">를 사용하고 내부에 시각적으로 숨겨진 제목 태그(<h2 id="nav-heading" class="visually-hidden">...</h2>)를 추가하여 번역 호환성과 헤딩 탐색 접근성을 모두 확보하십시오.
