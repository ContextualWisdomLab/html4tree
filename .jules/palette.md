...
## 2026-08-21 - Fix fragile CSS selector causing a11y issues
**Learning:** Using structural pseudo-classes like `:last-child` for hover states is fragile and breaks easily when new elements (like `.visually-hidden` spans for screen readers) are added at the end of the DOM structure.
**Action:** Use specific structural targeting like `:nth-child(2)` or dedicated classes for hover effect targets to ensure visual styles remain intact even when accessibility enhancements alter the DOM tree.
