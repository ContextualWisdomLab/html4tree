...
## 2024-09-23 - 숨겨진 텍스트로 인한 CSS 스타일링 누수 방지
**Learning:** 상호작용 요소(예: 링크) 안에서 화면 판독기를 위한 `.visually-hidden` 텍스트를 사용할 때, `span:last-child`와 같은 구조적 선택자를 사용하면 시각적으로 보이지 않는 요소가 마지막 자식으로 인식되어 의도한 요소(텍스트)에 호버 스타일(예: 밑줄)이 적용되지 않거나 숨겨진 요소로 스타일이 누수되는 문제가 발생합니다.
**Action:** 숨겨진 텍스트가 포함된 요소의 상태(hover, focus, active 등)를 스타일링할 때는 구조적 선택자 대신 텍스트 콘텐츠 자체에 명시적인 클래스(예: `.entry-name`)를 부여하고 해당 클래스를 타겟팅하여 스타일 충돌과 누수를 방지하십시오.
