...
## 2026-08-28 - 정적 요소의 ARIA 라이브 리전(role="status") 제거
**Learning:** 초기 페이지 로드 시 렌더링되는 정적 요소(예: 빈 디렉토리 메시지)에 `role="status"`와 같은 ARIA 라이브 리전 역할을 적용하는 것은 접근성 안티패턴입니다. 이로 인해 스크린 리더가 콘텐츠를 중복해서 읽거나 페이지 읽기 흐름을 방해할 수 있습니다.
**Action:** 비동기적으로 추가되거나 상태가 변경되는 요소에만 `role="status"`와 같은 라이브 리전을 사용하고, 처음부터 렌더링되어 있는 정적 요소에는 이러한 역할을 추가하지 마십시오.
