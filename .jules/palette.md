...
## 2024-05-13 - 인터랙티브 요소의 :active 상태 피드백
**Learning:** 인터랙티브 요소(링크, 버튼 등)에 `:hover` 및 `:focus-visible`과 함께 `:active` 상태를 추가하면, 클릭이나 탭 이벤트 중에 사용자에게 즉각적인 시각적, 촉각적 피드백을 제공하여 인지된 반응성과 접근성을 크게 향상시킬 수 있음을 확인했습니다.
**Action:** CSS에서 인터랙티브 요소에 `:active` 가상 클래스(pseudo-class) 스타일(예: 배경색 변경, 크기 축소 애니메이션)을 적용합니다.
