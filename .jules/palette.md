...
## 2024-07-13 - 빈 디렉토리 상태의 접근성(Accessibility) 개선 (Superseded)
**Learning:** 정적 파일 서버의 빈 디렉토리 상태는 스크린 리더 사용자에게 컨텐츠 누락으로 오해받을 수 있으며, 시각적으로도 일반 리스트 아이템과 정렬이 맞지 않는 문제가 있었습니다.
**Action:** (이전 지침 - 취소됨) 빈 상태를 나타내는 요소에 `role="status"`를 추가하여 스크린 리더가 명확하게 인지할 수 있도록 하고, 아이콘과 flex 레이아웃을 통해 다른 리스트 아이템과 일관된 시각적 흐름을 제공하도록 합니다. (주의: 이는 동적으로 갱신되는 empty-state에만 적용되며, 최초 렌더링되는 정적 empty-state에는 적용하지 않습니다. 2026-08-12 지침 참조)
...
## 2026-08-12 - 빈 디렉토리 상태의 접근성 개선
**Learning:** 초기 페이지 로드 시 처음부터 렌더링되는 정적 상태 메시지(예: 빈 디렉토리 안내)에 `role="status"`와 같은 라이브 리전 속성을 부여하면, 스크린 리더가 이를 동적 업데이트로 오인하여 초기 로드 시 불필요하고 중복된 알림을 발생시킬 수 있습니다.
**Action:** 사용자 상호작용 없이 처음부터 렌더링되는 정적인 빈 상태 요소에는 `role="status"`를 사용하지 않고 문서의 기본 흐름에 따라 자연스럽게 읽히도록 구현해야 합니다.
