...
## 2024-08-20 - 숨겨진 텍스트 스타일링 충돌 방지
**Learning:** 상호작용 요소(예: 파일 링크)에 화면 판독기를 위한 `.visually-hidden` 텍스트를 포함하고 스타일 지정 시 범용 구조 선택자(예: `span:last-child`)를 사용하면 시각적 스타일(예: 밑줄)이 시각적으로 숨겨진 텍스트 영역 밖으로 벗어나 화면을 어지럽힐 수 있습니다.
**Action:** 숨겨진 텍스트(`span.visually-hidden`)를 포함하는 요소를 스타일링할 때는 범용 구조 선택자를 사용하지 말고, 항상 표시될 내용 자체에 구체적인 클래스(예: `.entry-name`)를 지정하고 이를 통해 hover, focus, active 상태 스타일을 제어하여 시각적 결함을 방지하십시오.
