...
## 2024-08-17 - 네비게이션 랜드마크의 접근성 이름 제공 방식 개선
**학습:** `aria-label`은 브라우저 번역 도구(예: Chrome 번역)에서 무시되는 경우가 많아, 외국어 사용자가 자동 번역을 사용할 때 스크린 리더에서 네비게이션의 이름이 원본 언어(예: 한국어 문서가 영어로 번역될 때 "디렉토리 목록"이 그대로 한국어로 남음)로 남게 되어 다국어 접근성을 심각하게 해칩니다.
**조치:** `<nav>`와 같은 랜드마크 요소에 접근성 이름을 제공할 때, `aria-label` 대신에 `visually-hidden` 클래스가 적용된 숨겨진 제목(예: `<h2 class="visually-hidden" id="nav-title">...</h2>`)을 생성하고, `<nav aria-labelledby="nav-title">`을 연결하여 브라우저가 일반 텍스트로 인식하고 번역할 수 있도록 하여 번역 호환성을 극대화하십시오.
