...
## 2026-08-26 - 빈 디렉토리 상태의 ARIA 속성 오용 수정
**Learning:** 초기 페이지 로드 시 렌더링되는 정적 요소(예: 빈 디렉토리 메시지)에 `role="status"`와 같은 ARIA 라이브 영역 역할을 부여하면 스크린 리더가 콘텐츠를 중복해서 읽거나 페이지 읽기 흐름을 방해하는 접근성 안티패턴이 발생합니다.
**Action:** 정적인 안내 메시지에는 `role="status"`를 사용하지 않고 시맨틱 HTML과 적절한 텍스트로만 구조화하여 스크린 리더 사용자의 혼란을 방지하십시오.
