...
## 2026-07-18 - 루트 디렉토리 빈 이름 시맨틱 개선
**학습:** 루트 디렉토리와 같이 File.getName()이 빈 문자열("")을 반환하는 디렉토리를 처리할 때, `<title>` 및 `<h1>` 요소가 빈 텍스트로 생성되어 스크린 리더 사용자와 시각적 사용자 모두에게 혼란을 주고 웹 접근성(a11y)을 저해할 수 있습니다.
**조치:** 디렉토리 목록의 HTML을 동적으로 생성할 때 항상 디렉토리 이름이 비어있는지 확인하고, 비어있는 경우 빈 태그 렌더링을 방지하기 위해 절대 경로나 적절한 폴백(예: `absolutePath`)을 제공하여 명확한 컨텍스트와 접근성 시맨틱을 유지하십시오.
