...
## 2026-08-29 - 정적 빈 상태 요소의 접근성 안티 패턴 제거
**Learning:** 페이지 로드 시점부터 정적으로 렌더링되는 요소(예: 비어있는 디렉토리 안내 문구)에 `role="status"`와 같은 ARIA 라이브 영역(live region) 역할을 부여하는 것은 접근성 안티 패턴입니다. 스크린 리더가 해당 내용을 중복해서 읽거나 페이지를 읽는 흐름을 방해할 수 있습니다.
**Action:** 동적으로 추가되거나 변경되는 상태 메시지가 아닌 한, 정적 콘텐츠에는 ARIA 라이브 영역 역할 적용을 피하고 기본 시맨틱 구조를 활용하십시오.
