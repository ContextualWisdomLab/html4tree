...
## 2026-08-27 - 정적 초기 로딩 요소의 ARIA 라이브 리전 오용 방지
**Learning:** 초기 페이지 로드 시 렌더링되는 정적 요소(예: 빈 디렉토리 메시지)에 `role="status"`와 같은 ARIA 라이브 리전 역할을 적용하는 것은 접근성 안티패턴입니다. 스크린 리더가 콘텐츠를 중복해서 읽거나 페이지 읽기 흐름을 방해할 수 있습니다.
**Action:** 동적으로 추가되거나 업데이트되는 상태 메시지에만 라이브 리전(예: `role="status"`)을 사용하고, 초기 페이지 로드 시 정적으로 렌더링되는 빈 상태 메시지에서는 이를 제거하여 스크린 리더의 자연스러운 읽기 흐름을 방해하지 않도록 합니다.
