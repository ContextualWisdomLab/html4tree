...
## 2026-07-27 - 빈 디렉토리 상태 메시지에 대한 접근성 향상
**Learning:** 빈 디렉토리와 같은 상태 메시지는 스크린 리더가 인식할 수 있도록 `role="status"`를 부여해야 하며, 장식용 아이콘은 `aria-hidden="true"`로 숨겨야 합니다.
**Action:** 정적 HTML 생성 시, 시각적 일관성을 위한 장식 요소와 상태 정보 요소의 ARIA 속성을 적절히 분리하여 적용합니다.
