...
**Learning:** When using visually hidden spans (e.g. for accessibility like screen readers) combined with flexbox and pseudo structural classes (like `:last-child`), it can break visual hover styles (like underlines) because the last element is the hidden one, rather than the text we want to hover.
**Action:** Instead of targeting the text using `:last-child`, we need to use `:nth-child(2)` or find another more reliable selector to prevent applying the visual effects on visually hidden elements.
