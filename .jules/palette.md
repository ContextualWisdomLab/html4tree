...
## 2024-08-22 - 양방향 텍스트(BiDi) 호환성 개선
**Learning:** 디렉토리 및 파일 이름은 전 세계 여러 언어로 작성될 수 있으며, 아랍어나 히브리어와 같이 우에서 좌로 읽는(RTL) 텍스트가 좌에서 우로 읽는(LTR) 기본 레이아웃 내에 혼재될 경우, 파일명이 깨지거나 텍스트 렌더링이 엉키는 현상이 발생합니다.
**Action:** 생성되는 정적 HTML의 텍스트가 포함된 요소(예: `<h1>`, 텍스트를 감싸는 `<span>`)에 항상 `dir="auto"` 속성을 추가하여, 브라우저가 해당 콘텐츠에 맞는 텍스트 방향을 동적으로 결정하고 BiDi 텍스트가 올바르게 렌더링되도록 보장하십시오.
