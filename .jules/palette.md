...
## 2024-09-11 - Hover state targets specific content class
**Learning:** Targeting `span:last-child` for hover states applies visual styling (like underlines) to hidden screen-reader text, creating confusing or misplaced visual cues.
**Action:** Always target specific content classes (e.g., `.entry-name`) for interactive states (hover, focus, active) instead of relying on generic structural selectors to ensure styles only apply to visible elements.
