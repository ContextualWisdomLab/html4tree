...
## 2026-07-28 - 빈 디렉토리 상태의 화면 판독기 접근성 개선
**Learning:** 정적 HTML에서 상태를 나타내는 텍스트(예: "이 디렉토리는 비어 있습니다.")는 화면 판독기 사용자가 일반적인 목록 항목과 상태 메시지를 명확하게 구분하기 어려울 수 있습니다.
**Action:** 상태 메시지 요소에 `role="status"`를 추가하여 화면 판독기가 이를 단순 텍스트가 아닌 중요한 상태 정보로 인식하도록 보조 기술 호환성을 향상시키십시오.
