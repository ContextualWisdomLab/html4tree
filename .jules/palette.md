...
## 2024-05-24 - [BiDi(양방향 텍스트) 렌더링 접근성 개선]
**Learning:** 파일 및 디렉토리 이름에 RTL(Right-To-Left) 언어(예: 아랍어, 히브리어)가 포함될 경우, LTR(Left-To-Right) 레이아웃에서 텍스트가 깨지거나 잘못된 순서로 렌더링될 수 있습니다. 순수 HTML 생성 시 특정 텍스트를 포함하는 요소에 `dir="auto"` 속성을 부여하면 브라우저가 콘텐츠의 언어 방향을 스스로 판단하여 정확히 렌더링합니다.
**Action:** 사용자 생성 콘텐츠나 가변 텍스트(예: 파일명, 디렉토리명)를 렌더링하는 컨테이너 요소(`<h1>`, `<span>` 등)에는 항상 `dir="auto"`를 적용하여 국제화(i18n) 및 접근성을 확보합니다.
