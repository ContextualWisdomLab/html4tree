...
## 2024-09-24 - hover 시 레이블 텍스트에만 밑줄 적용
**Learning:**
아이콘을 포함한 파일/디렉토리 링크에 hover나 focus-visible 이벤트를 발생시킬 때, a 태그 내의 span 요소들이 여러 개일 수 있습니다.
이 경우 단순하게 `span:last-child` 같은 구조 선택자를 사용하면 스크린 리더용 숨김 텍스트(.visually-hidden)에 효과가 적용되는 문제가 발생할 수 있습니다.
**Action:**
동적이고 명시적인 의도를 표현하기 위해 특정 요소 (예: `.entry-label`)에만 스타일을 적용하도록 구조를 개선합니다. 앞으로는 `.entry-label` 처럼 의미를 가진 클래스를 추가하여 명확한 타겟을 가지도록 설계해야 합니다.
