...
## 2024-09-07 - 숨김 텍스트와 상호작용 요소의 포커스 스타일 일치
**학습:** `a` 요소의 hover나 focus-visible 상태에서 단순히 `span:last-child`에 스타일을 적용하면 구조 변경이나 시각적으로 숨겨진(`visually-hidden`) 텍스트가 추가되었을 때 숨김 텍스트에만 밑줄이 그어져 실제 보여지는 텍스트는 시각적 피드백을 받지 못하는 문제가 발생합니다.
**조치:** 항상 실제 보여지는 텍스트를 담고 있는 명시적인 클래스(예: `.entry-name`)를 지정하고 해당 클래스에 스타일을 적용하여 시각적으로 정확한 피드백을 유지하도록 하십시오.
