...
## 2024-05-24 - [BiDi 텍스트 지원]
**Learning:** 자동 생성된 HTML 인덱스에서 왼쪽에서 오른쪽(LTR) 레이아웃 내에 오른쪽에서 왼쪽(RTL) 문자(예: 아랍어, 히브리어)가 포함된 파일이나 디렉토리 이름이 깨지지 않도록 하려면, 해당 텍스트를 포함하는 `<h1>` 및 `<span>` 엘리먼트에 `dir="auto"` 속성을 추가하여 브라우저가 방향을 올바르게 감지하도록 해야 한다.
**Action:** 다국어 지원 가능성이 있는 텍스트를 렌더링하는 컨테이너 요소에 `dir="auto"`를 적용하여 접근성 및 UI 깨짐 문제를 방지한다.
