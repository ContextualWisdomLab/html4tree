...
## 2026-07-24 - 정적 HTML 빈 상태 접근성 향상
**Learning:** 빈 디렉토리 메시지와 같은 정적 상태 메시지에 `role="status"`를 추가하면 상태를 명확히 이해하는 데 도움이 되며, 장식용 아이콘은 `aria-hidden="true"`를 사용하여 스크린 리더 접근성을 높여야 한다.
**Action:** 화면에 표시되는 빈 상태(Empty State)에는 항상 적절한 ARIA 역할(`role="status"`)과 장식용 아이콘 처리(`aria-hidden`)를 기본적으로 적용할 것.
