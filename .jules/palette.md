...
## 2024-08-08 - 빈 상태 아이콘 메타포
**학습:** 빈 상태(Empty state)를 나타낼 때 정보 아이콘(`&#8505;`)과 같은 일반적인 기호를 사용하면, 기존의 파일(페이지 이모지) 및 디렉토리(폴더 이모지) 아이콘과 시각적 메타포가 일치하지 않아 일관성이 떨어집니다.
**조치:** 빈 디렉토리 상태의 아이콘을 열린 폴더 이모지(`&#128194;`)로 변경하여 사용자가 "비어 있는 디렉토리"임을 직관적으로 이해할 수 있도록 하고, 전체 디렉토리 트리의 시각적 일관성을 유지하십시오.
