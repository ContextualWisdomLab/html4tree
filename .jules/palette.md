...
## 2024-09-26 - aria-label 대신 aria-labelledby와 visually-hidden 요소 사용
**학습:** `aria-label` 속성은 종종 브라우저 번역 도구(예: 크롬 번역)에 의해 번역되지 않아, 문서 언어가 변경되더라도 화면 판독기에서는 원본 언어로 읽히는 문제가 발생하여 다국어 접근성이 저하됩니다.
**조치:** 화면 판독기용 접근성 텍스트를 제공할 때 `aria-label`을 사용하는 대신, 시각적으로 숨겨진(`.visually-hidden`) 요소(예: `<h2>`)를 생성하고 이를 `aria-labelledby`와 연결하십시오. 이렇게 하면 브라우저가 일반 텍스트로 인식하여 정상적으로 번역할 수 있습니다.
