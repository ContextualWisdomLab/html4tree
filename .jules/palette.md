...
## 2024-08-20 - Bidirectional(BiDi) 텍스트 지원 개선
**학습:** 아랍어나 히브리어와 같은 Right-To-Left(RTL) 텍스트가 파일/디렉토리 이름에 포함될 경우, 명시적인 방향성(Directionality) 지정이 없으면 Left-To-Right(LTR) 레이아웃 내에서 텍스트가 깨지거나 잘못 렌더링되어 접근성 및 가독성을 크게 저해합니다.
**조치:** 텍스트를 포함하는 요소(예: `<h1>`, 파일 이름을 감싸는 `<span>`)에 항상 `dir="auto"` 속성을 추가하여, 브라우저가 콘텐츠 자체의 텍스트 방향을 올바르게 감지하고 렌더링할 수 있도록 보장하십시오.
