...
**학습:** 명시적인 플렉스박스 줄바꿈이 없는 유동적 레이아웃에서 매우 긴 파일 이름은 앞의 인라인 아이콘 아래로 텍스트를 줄바꿈하여 시각적 계층과 정렬을 깨뜨릴 수 있습니다.
**조치:** 파일/디렉토리 목록을 `align-items: flex-start`가 있는 플렉스 컨테이너로 항상 감싸고, `<span style="overflow-wrap: anywhere;">`로 감싼 텍스트 콘텐츠(또는 컨테이너의 전역 `overflow-wrap`) 옆에 `flex-shrink: 0`을 사용하여 모바일 기기에서 고정 너비 아이콘 옆에 텍스트가 깔끔하게 줄바꿈되도록 하십시오.
...
## 2026-07-13 - aria-label과 동일성을 위한 title 속성 추가
**학습:** 네이티브 툴팁 없이 아이콘이나 기술적 기호(예: `..`)에만 의존하면 화면 판독기를 사용하지 않는 시각 장애가 없는 사용자에게 혼란을 줄 수 있습니다. `aria-label`과 일치하는 `title` 속성은 동일성을 제공합니다.
...
## 2024-08-05 - 빈 디렉토리 아이콘 메타포
**학습:** 빈 디렉토리를 나타낼 때 정보 아이콘(`&#8505;`)을 사용하면 일반적인 파일 디렉토리 아이콘 메타포와 시각적인 불일치를 발생시킬 수 있습니다. 열린 폴더 아이콘(`&#128194;`)을 사용하면 사용자가 해당 상태가 디렉토리와 관련됨을 더 직관적으로 인식할 수 있습니다.
**조치:** 빈 디렉토리 상태를 표시할 때 기존 정보 아이콘(`&#8505;`) 대신 열린 폴더를 의미하는 아이콘(`&#128194;`)을 사용하여 일반 디렉토리(`&#128193;`)와 시각적 일관성을 유지하고 의미를 명확히 하십시오.
