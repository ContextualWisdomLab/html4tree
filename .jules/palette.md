...
## 2026-08-23 - 빈 디렉토리 정적 렌더링 시 접근성 안티패턴 개선
**Learning:** 초기 페이지 로드 시 정적으로 렌더링되는 요소(예: 빈 디렉토리 안내 메시지)에 `role="status"`를 적용하면, 스크린 리더가 불필요하게 콘텐츠를 중복해서 읽거나 페이지 읽기 흐름을 방해할 수 있습니다.
**Action:** 스크린 리더의 접근성을 개선하기 위해, 페이지 로드 시 정적으로 표시되는 안내 메시지에서 `role="status"`와 같은 ARIA live region 속성을 제거하고 기본적인 시맨틱 구조에 의존하십시오.
