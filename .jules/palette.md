...
## 2024-09-14 - Improve screen reader hover and focus styling semantics
**Learning:** Adding `.visually-hidden` or ARIA elements inside interactive elements (like `<a>`) can cause structural CSS selectors (like `span:last-child`) to target the visually hidden text instead of the visible entry name. This causes visual styling (like underlines on hover/focus) to break.
**Action:** When adding `.visually-hidden` or ARIA elements inside interactive elements (like `<a>`), ensure hover, focus, and active CSS states target specific content classes (e.g., `.entry-name`) rather than generic structural selectors like `span:last-child`. This prevents visual styling (like underlines) from incorrectly applying to hidden screen-reader text.
