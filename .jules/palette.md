...
## 2026-07-28 - 루트 디렉토리 접근성 보장
**Learning:** 파일 시스템 루트(예: `/` 또는 `C:\`)의 경우 `File.getName()`이 빈 문자열을 반환하여 `<title>` 및 `<h1>` 태그가 비어 있게 됩니다. 이는 스크린 리더 사용자에게 혼란을 주고 접근성을 심각하게 훼손합니다.
**Action:** `getName()`이 빈 문자열을 반환할 때 대체값(fallback)으로 `absolutePath`를 사용하도록 수정하여, 어떤 디렉토리에서든 항상 유효한 식별자가 UI에 표시되도록 보장하십시오.
