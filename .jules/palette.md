...
## 2024-08-01 - 빈 디렉토리 상태 메시지 접근성 향상
**학습:** 디렉토리가 비어 있을 때 표시되는 "이 디렉토리는 비어 있습니다."와 같은 상태 메시지는 시각적으로는 구별되지만, 스크린 리더 사용자는 이를 상태 변화로 인지하지 못하고 일반 텍스트로 읽고 지나칠 수 있습니다.
**조치:** 빈 상태를 나타내는 요소(예: `<div class="empty-dir">`)에 `role="status"` 속성을 추가하여, 스크린 리더가 이를 단순한 텍스트가 아닌 상태 메시지로 인식하도록 개선하십시오.
