...
## 2024-08-18 - 링크 Hover 밑줄 피드백 시각적 오류 수정
**학습:** 기존 CSS는 `a:hover span:last-child`를 사용하여 파일 텍스트에 밑줄을 표시했습니다. 하지만 접근성을 위해 `span.visually-hidden` 요소가 링크의 마지막 자식으로 추가된 후, 일반 텍스트 대신 숨겨진 요소에 밑줄 스타일이 적용되어 사용자가 시각적인 hover 피드백을 받을 수 없게 되었습니다.
**조치:** 여러 개의 `span` 자식 요소가 있는 복합 컴포넌트에서 시각적 텍스트에 hover 스타일을 적용할 때는, 숨겨진 접근성 레이블을 고려하여 `span:nth-last-child(2)` 등을 사용해 실제로 보이는 텍스트 요소를 명확히 타겟팅해야 합니다.
