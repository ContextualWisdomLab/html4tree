...
## 2024-09-20 - 접근성 향상을 위한 aria-label 대신 aria-labelledby 사용
**학습:** 자동 번역 도구(예: Chrome 번역)는 종종 `aria-label` 속성의 값을 번역하지 못하여, 사용자가 문서의 언어를 번역해도 스크린 리더는 여전히 원래 언어로 읽게 되는 문제가 발생합니다.
**조치:** 시각적으로는 숨겨져 있지만 번역 가능한 텍스트 요소(예: `<h2 class="visually-hidden">`)를 사용하고, 해당 요소를 참조하는 `aria-labelledby`를 랜드마크(예: `<nav>`)에 적용하여 다국어 지원 및 번역 접근성을 향상시키십시오.
