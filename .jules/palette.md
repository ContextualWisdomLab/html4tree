...
## 2024-08-20 - 시각적으로 숨겨진 요소 적용 시 구조적 CSS 선택자 문제
**Learning:** `<span class="visually-hidden">` 요소 등을 도입할 때 기존에 사용 중인 `:last-child` 등의 구조적 가상 클래스(structural pseudo-class)가 의도치 않은 요소에 적용되어, 호버(hover) 시 텍스트 밑줄과 같은 시각적 스타일이 깨질 수 있습니다.
**Action:** 접근성을 위해 구조적으로 숨겨진 요소를 DOM에 추가할 때에는 CSS 스타일링 시 깨지기 쉬운 `:last-child`나 `:nth-last-child()` 등 구조 기반 선택자 대신, 타겟팅하려는 텍스트 등에 명시적인 시맨틱 클래스(예: `.entry-label`)를 부여하여 스타일 안정성을 보장하십시오.
