...
## 2024-07-12 - 긴 파일 이름에서 아이콘 정렬 어긋남 방지
**학습:** 명시적인 flexbox 줄바꿈이 없는 유동적인 레이아웃에서 파일 이름이 매우 길면 텍스트가 선행하는 인라인 아이콘 아래로 줄바꿈되어 시각적 계층 구조와 정렬이 깨질 수 있습니다.
**조치:** 모바일 장치에서 고정 너비 아이콘 옆에 텍스트가 깔끔하게 줄바꿈되도록 하려면 항상 파일/디렉토리 목록을 `align-items: flex-start`가 있는 flex 컨테이너로 감싸고 `<span style="overflow-wrap: anywhere;">` (또는 컨테이너의 전역 `overflow-wrap`)로 감싼 텍스트 콘텐츠와 함께 `flex-shrink: 0`을 사용하여 아이콘을 분리하십시오.
...
## 2026-07-13 - aria-labels와 동일한 title 속성 추가
**학습:** 네이티브 툴팁 없이 아이콘이나 기술적인 기호(예: `..`)에만 의존하면 화면 판독기를 사용하지 않는 시각 장애가 없는 사용자에게 혼란을 줄 수 있습니다. `aria-label`과 일치하는 `title` 속성은 동일성을 제공합니다.
...
## 2024-08-09 - 모바일 환경을 위한 터치 타겟 크기 최적화
**학습:** 모바일 기기 등 터치 인터페이스에서 링크나 버튼의 상하 패딩이 충분하지 않으면(예: `0.5rem`) 터치 타겟 크기가 작아져 사용자가 실수로 다른 항목을 탭하기 쉽고 조작성이 떨어집니다. WCAG는 최소 44x44 CSS 픽셀의 터치 타겟을 권장합니다.
**조치:** 클릭 가능한 요소(예: `a` 태그 및 `.empty-dir` 등)의 상하 패딩을 늘려(예: `padding: 0.75rem 0.5rem;`) 터치 타겟 크기를 확보하고 모바일 사용성을 개선하십시오.
