...
## 2024-08-20 - 시각적으로 숨겨진 요소 추가 시 CSS 선택자 문제
**Learning:** 접근성을 위해 화면 판독기용 텍스트를 담은 `<span class="visually-hidden">` 요소를 목록 항목 끝에 추가하면, 기존의 `:last-child` CSS 선택자가 의도치 않게 이 숨겨진 요소에 적용되어 hover 시 시각적인 텍스트에 밑줄이 생기지 않는 문제가 발생합니다.
**Action:** 시각적 요소(예: 파일/디렉토리 이름)를 대상으로 할 때는 `:last-child` 대신 `:nth-child(2)`와 같이 구체적인 요소 순서를 지정하는 선택자를 사용하여 보이지 않는 시맨틱 요소와의 충돌을 피하십시오.
