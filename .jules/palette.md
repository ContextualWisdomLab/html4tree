...
## 2024-09-20 - 시각적 Hover 피드백 복원
**Learning:** `span:last-child`와 같은 구조적 클래스(structural classes)를 스타일링에 사용하는 것은 취약할 수 있습니다. 특히 접근성을 위해 눈에 보이지 않는 요소(예: 스크린 리더용 숨김 텍스트)를 요소 끝에 추가하면, 구조적 클래스가 시각적인 부분을 제대로 타겟팅하지 못해 스타일이 깨지게 됩니다.
**Action:** 접근성 향상을 위해 숨겨진 텍스트를 추가할 때 시각적 스타일이 깨지지 않도록 구조적 가상 클래스(pseudo-classes) 대신 `.entry-label`과 같은 의미론적 클래스(semantic classes)를 사용하여 시각적 부분을 타겟팅하십시오.
