...
## 2026-08-18 - 스크린 리더 텍스트 번역 지원 향상
**Learning:** 브라우저 내장 번역기 및 확장 프로그램(예: Chrome 자동 번역)은 `aria-label` 속성의 텍스트를 번역하지 못하는 경우가 많습니다. 이로 인해 스크린 리더 사용자가 외국어 환경에서 번역되지 않은 접근성 텍스트를 그대로 듣게 되어 사용자 경험이 크게 저하됩니다.
**Action:** 접근성 및 다국어 지원(i18n) 개선을 위해 `aria-label` 대신 시각적으로 숨겨진 텍스트(예: `<span class="visually-hidden">`)를 사용하여 스크린 리더가 DOM 텍스트 노드를 읽도록 변경하십시오. 이렇게 하면 번역기가 DOM 텍스트를 감지하고 번역할 수 있습니다.
