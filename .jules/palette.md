...
## 2026-08-02 - 루트 디렉토리의 빈 이름(Title/Heading) 렌더링 수정
**Learning:** 파일 시스템 루트(예: `/`, `C:\`)의 경우 `File.getName()`이 빈 문자열을 반환합니다. 이를 UI 요소(`<title>`, `<h1>`)에 그대로 사용하면 스크린 리더 환경 등에서 해당 요소가 비어 있어 접근성(a11y)이 저하됩니다.
**Action:** 디렉토리 이름을 출력할 때 `curr_dir.name.ifEmpty { curr_dir.absolutePath }`와 같이 fallback(절대 경로)을 제공하여 빈 `<title>`과 `<h1>`이 생성되지 않도록 보장하십시오.
