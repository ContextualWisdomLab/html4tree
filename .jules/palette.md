...
## 2024-09-10 - 구조적으로 숨겨진 요소(visually-hidden) 추가 시 CSS 구조 선택자(span:last-child) 충돌 방지
**Learning:** 화면 판독기 사용자 접근성 향상을 위해 시각적으로는 숨겨진(visually-hidden) 텍스트를 마지막 자식 요소로 추가하면, `span:last-child`와 같은 기존 CSS 구조 선택자가 더 이상 의도한 시각적 텍스트 요소(예: 파일 이름)를 타겟팅하지 못해 Hover 시 밑줄 등의 시각적 피드백이 사라지는 부작용이 발생할 수 있습니다.
**Action:** DOM의 순서나 숨겨진 구조에 의존하는 구조 선택자(예: `:last-child`) 대신 명시적인 클래스 이름(예: `.name`)을 사용하여 시각적 스타일을 지정함으로써 접근성 향상 코드가 기존 디자인을 해치지 않도록 보장하십시오.
