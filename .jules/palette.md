...
## 2026-07-25 - 빈 상태(Empty State) 시각적 일관성 향상
**학습:** 디렉토리가 비어있을 때 표시되는 메시지가 일반 파일/폴더 목록과 시각적 구조(아이콘 유무, 들여쓰기 등)가 달라 일관성을 저해하고 사용자가 목록의 일부로 인지하기 어려울 수 있습니다.
**조치:** 빈 상태를 나타내는 요소에도 다른 목록 요소와 동일하게 Flex 레이아웃을 적용하고, 열린 폴더 아이콘(`&#128194;`)을 추가하여 시각적 일관성을 유지하고 빈 상태임을 명확히 시각화하십시오. 이모지는 화면 판독기가 읽지 않도록 `aria-hidden="true"`로 숨기십시오.
