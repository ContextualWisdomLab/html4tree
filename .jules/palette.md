...
## 2024-09-06 - [시각적 숨김 텍스트 접근성 개선]
**Learning:** 스크린 리더용 숨김 텍스트(.visually-hidden)가 `span:last-child` 등 범용 구조 선택자에 의해 호버/포커스 시 밑줄과 같은 시각적 스타일링의 영향을 받는 문제가 발생할 수 있습니다.
**Action:** 호버/포커스 스타일링 적용 시 특정 콘텐츠 클래스(예: `.entry-name`)를 대상으로 지정하여, 숨김 텍스트에 원치 않는 시각적 스타일이 적용되는 것을 방지합니다.
