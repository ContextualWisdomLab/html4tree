...
## 2024-09-17 - Avoid generic structure selectors for hover styles
**Learning:** Using generic structural selectors like `span:last-child` for interaction styles (e.g. `a:hover`) incorrectly applies visual styling to visually hidden screen-reader text inside interactive elements, breaking visual focus logic.
**Action:** Always target specific content classes (e.g., `.entry-name`) for hover, focus, and active CSS states instead of generic structure selectors to prevent styling visually hidden text within links.
