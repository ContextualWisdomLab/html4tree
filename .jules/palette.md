...
## 2024-08-25 - 양방향 텍스트(BiDi) 렌더링 지원
**학습:** 아랍어, 히브리어 등 RTL(우에서 좌로 읽는) 언어로 작성된 파일이나 디렉토리 이름이 LTR(좌에서 우로 읽는) 컨텍스트에서 렌더링될 때 레이아웃이 깨지거나 텍스트 순서가 엉뚱하게 표시되는 등 심각한 접근성 및 사용성 문제가 발생할 수 있습니다.
**조치:** 텍스트 콘텐츠를 동적으로 포함하는 요소(`<h1>` 및 파일/디렉토리 이름을 포함하는 `<span>` 등)에 `dir="auto"` 속성을 명시적으로 추가하여, 브라우저가 해당 텍스트의 실제 내용에 따라 양방향 텍스트(BiDi) 방향을 올바르게 감지하고 렌더링하도록 보장하십시오.
