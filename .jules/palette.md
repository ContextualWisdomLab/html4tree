...
## 2024-05-18 - 시각적 스타일링 요소 분리
**Learning:** 상호작용 요소(예: `<a>`) 안에 `.visually-hidden` 텍스트나 ARIA 요소가 있을 때, 포커스나 호버와 같은 CSS 상태 변화를 범용적인 선택자(예: `span:last-child`)로 타겟팅하면 숨겨진 스크린 리더 텍스트에도 언더라인과 같은 시각적 스타일링이 잘못 적용되어 레이아웃 문제를 유발할 수 있습니다.
**Action:** 호버, 포커스 및 활성 상태의 CSS 스타일링이 숨겨진 텍스트에 적용되는 것을 방지하려면, 시각적 스타일링을 타겟팅하기 위한 특정 클래스(예: `.entry-name`)를 지정하고 해당 클래스를 가진 요소에만 스타일을 적용하십시오.
