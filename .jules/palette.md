...
## 2024-09-19 - 시각적 Hover 피드백 복원
**Learning:** 구조적 CSS 선택자(`span:last-child`)를 사용하여 hover 스타일을 지정하면, 접근성을 위해 보이지 않는 텍스트(예: `visually-hidden`)가 추가되었을 때 스타일 대상이 변경되어 hover 효과가 깨질 수 있습니다.
**Action:** 구조적 선택자 대신 명시적인 의미론적 클래스(`.entry-label`)를 추가하여, DOM 구조가 변경되더라도 시각적 효과가 올바른 대상에 안정적으로 적용되도록 해야 합니다.
