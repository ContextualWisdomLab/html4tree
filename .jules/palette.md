...
## 2026-08-02 - 빈 디렉토리 상태의 시각적 및 접근성 개선
**Learning:** 정적 HTML로 생성된 빈 디렉토리 안내 문구에 `role="status"`를 추가하여 스크린 리더 사용자에게 상태 변화(비어있음)를 명확하게 알리고, 장식용 아이콘에는 `aria-hidden="true"`를 적용하여 접근성 노이즈를 줄일 수 있습니다.
**Action:** 앞으로 정적 페이지 내의 빈 상태나 결과 없음 상태를 표시할 때도 상태 역할을 명시(status role)하고 시각적 요소와 의미적 요소를 분리(aria-hidden)하여 일관된 접근성 경험을 제공해야 합니다.
