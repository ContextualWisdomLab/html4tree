...
## 2023-10-27 - [Fix Focus and Hover Style Application]
**Learning:** When adding `.visually-hidden` or ARIA elements inside interactive elements (like `<a>`), hover, focus, and active CSS states should target specific content classes (e.g., `.entry-name`) rather than generic structural selectors like `span:last-child`. This prevents visual styling from incorrectly applying to hidden screen-reader text.
**Action:** Always add and target specific classes for visual text content within links to avoid conflicts with appended screen reader structural span elements.
