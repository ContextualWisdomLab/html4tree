...
## 2024-08-16 - BiDi 텍스트 지원 (RTL 언어 접근성 개선)
**학습:** 아랍어, 히브리어 등 RTL(우에서 좌로 쓰는) 언어로 된 파일이나 디렉토리 이름이 LTR(좌에서 우로 쓰는) 기반의 레이아웃에 렌더링될 때, 명시적인 방향 설정이 없으면 텍스트가 깨지거나 잘못된 순서로 표시될 수 있습니다.
**조치:** LTR 레이아웃 내에서 동적인 텍스트(예: 파일명, 디렉토리명)를 표시하는 요소(예: `<h1>`, `<span>`)에 `dir="auto"` 속성을 추가하여, 브라우저가 해당 콘텐츠에 맞는 방향(RTL 또는 LTR)을 올바르게 감지하고 렌더링할 수 있도록 보장하십시오.
