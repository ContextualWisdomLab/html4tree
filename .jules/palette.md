...
**Learning:** 파일 서버는 다국어 파일 및 폴더명을 지원해야 하며, 오른쪽에서 왼쪽으로 쓰는(Right-To-Left) 문자가 포함된 이름의 경우 시맨틱상 기본 방향성을 콘텐츠에 맞게 설정하는 것이 바람직합니다.
**Action:** 폴더나 파일명 등 사용자 생성 텍스트가 표시되는 구체적인 텍스트 노드(`<h1>` 및 파일명을 감싸는 `<span>` 태그)에 `dir="auto"` 속성을 명시하여 텍스트 내용에 맞게 방향성이 처리되도록 마크업을 개선하십시오. (고정된 UI 텍스트 제외)
