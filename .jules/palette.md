...
## 2024-09-17 - 호버/포커스 시 시각적으로 숨겨진 요소의 스타일 분리
**Learning:** 요소 내부에 `.visually-hidden`과 같은 스크린 리더용 숨김 텍스트가 있을 때, `span:last-child`와 같은 범용적인 구조적 선택자를 사용하여 호버 및 포커스 스타일(예: 밑줄)을 적용하면 화면에서는 숨겨져 있어야 할 텍스트에 스타일이 강제로 적용되어 의도치 않은 시각적 결함(UI 버그)이 발생합니다.
**Action:** 인터랙티브 요소(예: `<a>`) 안에 스크린 리더를 위한 숨김 텍스트와 일반 텍스트가 함께 있는 경우, 호버나 포커스 CSS 상태는 범용적인 구조 선택자가 아닌 특정 콘텐츠 클래스(예: `.entry-name`)를 명시적으로 타겟팅하도록 작성하여 시각적 스타일이 숨겨진 텍스트에 오적용되는 것을 방지하십시오.
