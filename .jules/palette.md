...
## 2024-09-01 - 빈 디렉토리 메시지의 ARIA role 속성 제거
**Learning:** 초기 페이지 로드 시 렌더링되는 정적인 빈 상태 메시지에 `role="status"`(ARIA live region)를 부여하면, 스크린 리더가 중복해서 안내하거나 페이지 읽기 흐름을 방해하는 안티 패턴이 됩니다.
**Action:** 동적으로 추가되는 상태 메시지가 아닌 한, 정적 콘텐츠에는 `role="status"`와 같은 ARIA live region 역할을 지정하지 마십시오.
