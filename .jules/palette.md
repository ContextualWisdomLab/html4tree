...
## 2026-07-20 - 빈 디렉토리 메시지 상태 역할 추가
**Learning:** 스크린 리더 사용자가 빈 상태(empty state) 메시지를 명확히 인지할 수 있도록 정적 HTML 생성 시 `role="status"` 속성이 유용하게 적용될 수 있습니다.
**Action:** 앞으로 빈 목록이나 검색 결과 없음 등 상태 메시지를 제공하는 요소에는 ARIA 상태 역할을 고려하여 접근성을 높일 것입니다.
