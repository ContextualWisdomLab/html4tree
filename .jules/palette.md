...
## 2024-08-18 - 다국어 양방향(BiDi) 텍스트를 위한 방향 속성 추가
**학습:** 아랍어(Arabic)나 히브리어(Hebrew) 같은 Right-to-Left(RTL) 텍스트가 포함된 디렉토리나 파일 이름이 기본 Left-to-Right(LTR) 레이아웃에서 렌더링될 때, 텍스트 방향이 올바르게 표시되지 않아 가독성이 떨어지는 문제가 발생할 수 있습니다.
**조치:** 생성된 HTML의 특정 텍스트를 포함하는 요소(예: `<h1>` 및 파일 이름을 표시하는 `<span>`)에 `dir="auto"` 속성을 추가하여, 브라우저가 해당 요소의 내용을 분석하고 적절한 텍스트 방향(LTR 또는 RTL)을 동적으로 적용하도록 하십시오.
