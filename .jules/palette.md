...
## 2026-08-16 - 정적 콘텐츠에 대한 잘못된 ARIA 라이브 리전(role="status") 제거
**Learning:** 초기 페이지 로드 시 렌더링되는 정적 요소(예: 빈 디렉토리 메시지)에 `role="status"`와 같은 ARIA 라이브 리전 역할을 적용하면, 스크린 리더가 해당 콘텐츠를 중복해서 읽거나 페이지 읽기 흐름을 방해하는 접근성 안티 패턴이 발생합니다.
**Action:** 동적으로 업데이트되지 않고 정적으로 생성되는 HTML 메시지(빈 디렉토리 알림 등)에는 `role="status"`를 사용하지 않고 일반적인 시맨틱 마크업을 유지하여 스크린 리더 사용자의 읽기 흐름을 방해하지 않도록 합니다.
