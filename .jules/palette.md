...
## 2026-08-03 - 루트 디렉토리 및 빈 이름 폴더의 접근성 처리
**Learning:** 파일 시스템 루트(예: `/`) 또는 빈 이름 반환 파일 구조의 경우, HTML 생성 시 `<title>` 및 `<h1>` 태그가 완전히 비워지게 되어 스크린 리더 사용자가 해당 페이지의 목적이나 현재 위치를 파악할 수 없는 치명적인 접근성 결함이 발생합니다.
**Action:** `File.getName()`이 빈 문자열을 반환하는 엣지 케이스를 처리할 때, 접근성 텍스트 노드가 비어 있지 않도록 항상 폴백(예: `absolutePath`)을 제공하여 UI 요소(`<title>`, `<h1>`)를 올바르게 채우십시오.
