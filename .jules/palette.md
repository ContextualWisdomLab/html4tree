...
## 2026-08-01 - 루트 디렉토리의 빈 제목 처리
**Learning:** 파일 시스템 루트와 같이 `getName()`이 빈 문자열을 반환하는 디렉토리를 처리할 때, 접근성을 해치고 스크린 리더 환경에서 제목 요소(`<title>`, `<h1>`)가 누락되는 문제가 발생할 수 있습니다.
**Action:** 디렉토리 이름이 비어있는 경우, 시각적 빈 상태를 방지하고 접근성을 유지하기 위해 `absolutePath`와 같은 의미 있는 대체 경로를 사용하여 제목과 헤더에 표시되도록 설정해야 합니다.
