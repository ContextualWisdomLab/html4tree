...
## 2024-09-09 - 네비게이션 랜드마크의 다국어 번역 호환성 개선
**Learning:** `<nav>`와 같은 랜드마크 요소에 사용되는 `aria-label` 속성은 Chrome Translate와 같은 브라우저 내장 번역 도구에 의해 번역되지 않는 경우가 많아, 문서 언어가 번역되어도 스크린 리더에서는 원본 언어로 읽히는 다국어(i18n) 접근성 문제가 발생합니다.
**Action:** 내비게이션 영역의 레이블을 지정할 때 `aria-label` 대신 내부에 `visually-hidden` 클래스가 적용된 숨겨진 제목 요소(예: `<h2 id="nav-heading">`)를 배치하고 `aria-labelledby`로 연결하십시오. 이는 일반 텍스트 노드로 인식되어 정상적으로 번역되며, '제목으로 탐색'하는 스크린 리더 단축키를 통해서도 쉽게 발견할 수 있어 사용성과 호환성이 모두 향상됩니다.
