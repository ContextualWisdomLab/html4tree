...
## 2026-08-14 - 정적 요소에서 부적절한 ARIA 라이브 리전(role="status") 제거
**Learning:** 초기 페이지 로드 시 정적으로 렌더링되는 요소(예: 빈 디렉토리 메시지)에 `role="status"`와 같은 ARIA 라이브 리전 역할을 적용하는 것은 접근성 안티패턴입니다. 화면 판독기가 페이지 읽기 흐름을 방해하거나 콘텐츠를 중복으로 소리내어 읽을 수 있습니다.
**Action:** 동적으로 업데이트되지 않는 정적 안내 메시지에서는 `role="status"` 등의 라이브 리전 역할을 제거하여 자연스러운 페이지 읽기 흐름을 보장하십시오.
