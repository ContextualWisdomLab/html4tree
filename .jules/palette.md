...
## 2026-08-20 - 빈 디렉토리 상태의 접근성 개선
**Learning:** 초기 페이지 로딩 시 렌더링되는 정적 요소에 `role="status"`와 같은 ARIA live region 속성을 부여하면 스크린 리더가 불필요하게 상태를 반복해서 읽거나 전체 페이지의 흐름을 방해할 수 있습니다.
**Action:** 상태 변경이 아닌 초기 로드부터 고정된 정보인 경우 라이브 리전(live region)을 부여하지 않고 시맨틱한 HTML 마크업에만 집중해야 합니다.
