...
## 2024-08-20 - Add semantic `<time>` for generated timestamp
**Learning:** Screen readers and automated parsers benefit from explicit semantic meaning for dates and times, but dynamically generated directory indexes often lack this. Adding a visually hidden timestamp to the document gives users context about when the index was last updated, without cluttering the UI.
**Action:** Add a visually hidden `<time>` element near the top of the `<main>` area that uses the current timestamp in ISO-8601 format in its `datetime` attribute.
