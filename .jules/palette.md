...
## 2026-07-31 - 루트 디렉토리의 접근성 향상 (빈 제목 방지)
**학습:** 루트 디렉토리(`/` 또는 `C:\`)의 경우 `File.getName()`이 빈 문자열을 반환하여, 생성된 HTML에 `<title>` 및 `<h1>` 태그가 비어 있게 됩니다. 이는 화면 판독기 사용자에게 페이지 제목을 제공하지 않아 접근성에 심각한 문제를 일으킵니다.
**조치:** 디렉토리 이름이 비어 있는 경우 `absolutePath`를 폴백(fallback)으로 사용하여, `<title>` 및 `<h1>` 요소가 항상 유의미한 텍스트를 포함하도록 수정하십시오.
