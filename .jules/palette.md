...
## 2026-08-18 - 랜드마크의 브라우저 번역 호환성을 위한 aria-labelledby 사용
**Learning:** 브라우저 번역 도구(예: 크롬 번역)는 `<nav>`와 같은 랜드마크 요소의 `aria-label` 속성을 무시하는 경우가 많아, 문서 언어가 변경되어도 화면 판독기에서 번역되지 않은 상태로 읽히는 문제가 발생할 수 있습니다.
**Action:** `<nav>`와 같은 주요 랜드마크 영역에 접근성 이름을 제공할 때, `aria-label` 대신 `.visually-hidden` 클래스가 적용된 숨겨진 텍스트 요소(예: `<h2 id="nav-heading" class="visually-hidden">`)와 함께 `aria-labelledby`를 사용하여 화면 판독기와 번역 도구 모두와 완벽하게 호환되도록 하십시오.
