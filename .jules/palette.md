...
## 2026-08-04 - 루트 디렉토리의 빈 제목/헤더로 인한 접근성 및 사용성 문제
**학습:** 파일 시스템 루트와 같이 `File.getName()`이 빈 문자열을 반환하는 디렉토리에 대해 `<title>` 및 `<h1>` 태그가 비워지면, 스크린 리더 사용자가 현재 위치를 파악하기 어렵고 시각적으로도 혼란을 주어 접근성(Accessibility)과 사용성(UX)을 크게 저해합니다.
**조치:** 디렉토리 이름이 비어 있는 경우(루트 디렉토리 등), 해당 디렉토리의 절대 경로(`absolutePath`)를 대체 이름으로 사용하여 빈 제목/헤더가 생성되는 것을 방지함으로써 접근성 및 시각적 유용성을 향상시킵니다.
