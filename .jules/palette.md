...
## 2024-05-24 - HTML 시각적 구조 안정성 개선을 위한 hover 스타일 타겟팅 변경
**Learning:** `a:hover span:last-child`와 같은 구조적 선택자(`:last-child`)를 사용하면 접근성 강화를 위해 `.visually-hidden` 텍스트 요소가 추가될 때 구조가 깨져 hover 효과(밑줄 등)가 엉뚱한(숨겨진) 요소에 적용되는 문제가 발생합니다.
**Action:** 불안정한 구조적 가상 클래스(pseudo-class) 대신, 표시되는 파일/디렉토리 이름 `span` 요소에 `.entry-label` 같은 시맨틱 클래스를 직접 부여하고 해당 클래스에 hover 스타일을 지정하여 숨김 요소 추가 시에도 시각적 정렬이 깨지지 않도록 하십시오.
