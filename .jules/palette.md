...
## 2026-07-22 - 빈 상태 메시지 접근성 개선
**Learning:** 스크린 리더 사용자는 시각적으로 빈 디렉토리임을 나타내는 텍스트를 인지하기 어려울 수 있으며, 정적 HTML에서도 상태 메시지에 `role="status"`를 부여하여 접근성을 높이고 상태 정보를 명확하게 전달할 수 있음을 깨달았습니다.
**Action:** 앞으로 빈 목록이나 결과가 없음을 알리는 UI 요소에는 시각적 표시뿐만 아니라 스크린 리더가 적절히 인식할 수 있도록 `role="status"`와 같은 ARIA 역할을 기본적으로 추가하겠습니다.
