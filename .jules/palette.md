...
## 2026-08-30 - 빈 디렉토리 상태의 정적 로드 시 role="status" 제거
**Learning:** 초기 페이지 로드 시 렌더링되는 정적 요소에 `role="status"`와 같은 ARIA 라이브 영역 역할을 적용하는 것은 접근성 안티 패턴입니다. 화면 판독기가 내용을 중복해서 읽거나 페이지 읽기 흐름을 방해할 수 있습니다.
**Action:** 동적으로 업데이트되지 않는 정적 콘텐츠에는 라이브 영역 역할(status, alert 등)을 적용하지 마십시오.
