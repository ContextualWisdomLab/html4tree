...
## 2024-05-18 - 시각적으로 숨겨진 요소로 인해 발생하는 시각적 Hover 피드백 회귀 수정
**Learning:** 접근성 개선을 위해 컨테이너의 마지막에 `<span class="visually-hidden">`과 같은 요소를 추가할 때 `:last-child`와 같은 구조적 가상 클래스를 사용하는 것은 위험합니다. 잘못된 요소에 스타일이 적용되어 hover 시 밑줄과 같은 시각적 효과가 깨질 수 있습니다.
**Action:** 구조적 가상 클래스 대신 시맨틱 클래스(예: `.entry-label`)를 사용하여 렌더링되는 시각적 요소를 안정적으로 타겟팅하고 스타일을 적용해야 합니다.
