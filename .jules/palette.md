...
## 2026-07-30 - 빈 디렉토리 상태의 접근성 개선
**Learning:** 상태 메시지에 `role="status"`를 부여하여 스크린 리더 사용자가 상태를 즉시 인지하게 하고, 장식용 아이콘에 `aria-hidden="true"`를 적용하여 불필요한 정보 노출을 방지해야 함.
**Action:** 빈 상태(Empty State)를 구현할 때 시각적 피드백뿐만 아니라 보조 기기를 위한 의미론적 ARIA 속성을 항상 포함할 것.
