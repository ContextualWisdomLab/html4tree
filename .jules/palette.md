...
## 2026-08-15 - 정적 콘텐츠에 대한 ARIA 라이브 영역 오남용 방지
**Learning:** 초기 페이지 로드 시 렌더링되는 정적 요소(예: 빈 디렉토리 메시지)에 `role="status"`와 같은 ARIA 라이브 영역 역할을 적용하는 것은 접근성 안티패턴입니다. 이로 인해 스크린 리더가 콘텐츠를 중복해서 읽거나 페이지 읽기 흐름을 방해할 수 있습니다.
**Action:** 페이지의 동적인 변경이나 업데이트를 사용자에게 알려야 하는 경우에만 ARIA 라이브 영역(예: `role="status"`, `aria-live="polite"`)을 사용하고, 초기 로드 시 렌더링되는 정적 텍스트 메시지에는 사용하지 마십시오.
