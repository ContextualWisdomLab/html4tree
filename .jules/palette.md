...
## 2024-08-20 - 내비게이션 랜드마크의 번역 호환성 향상
**Learning:** `aria-label` 속성값은 종종 브라우저 번역기(예: 크롬 번역)에 의해 번역되지 않아서, 사용자가 다른 언어로 페이지를 번역할 때 스크린 리더에서 올바르지 않은 접근성 문제가 발생할 수 있습니다.
**Action:** `<nav>` 요소에 `aria-label` 대신 내부에 시각적으로 숨겨진 제목(`<h2>`)을 두고 `aria-labelledby`를 사용하여, 번역기가 텍스트를 정상적으로 번역할 수 있도록 하고 "제목으로 탐색" 단축키로도 내비게이션을 찾을 수 있게 하십시오.
