...
## 2024-09-12 - 내비게이션 랜드마크의 다국어 지원 및 헤딩 탐색 접근성 향상
**학습:** `aria-label` 속성으로 지정된 내비게이션 랜드마크는 브라우저 번역기(예: Chrome Translate)에 의해 텍스트 노드처럼 번역되지 않는 경우가 많으며, 시각장애인 사용자의 '헤딩 단위 이동' 단축키로 검색되지 않습니다.
**조치:** `<nav>` 요소에 직접 `aria-label`을 사용하는 대신, 내부의 시각적으로 숨겨진 헤딩(예: `<h2 id="nav-heading" class="visually-hidden">`)을 가리키는 `aria-labelledby`를 사용하십시오. 이를 통해 화면 판독기의 헤딩 탐색을 지원하고 번역기 호환성 문제를 해결할 수 있습니다.
