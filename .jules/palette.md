...
## 2024-05-16 - 화면 판독기 텍스트의 시각적 스타일 방지
**Learning:** `.visually-hidden` 클래스가 적용된 숨겨진 스크린 리더용 텍스트 요소가 컨테이너 내에서 마지막 자식 요소로 위치할 경우, CSS에서 `span:last-child`와 같은 제네릭한 구조적 선택자를 사용하면 의도치 않게 해당 숨겨진 텍스트(또는 그 이전 요소)에 스타일(예: 밑줄 등)이 적용되어 시각적 결함이 발생할 수 있습니다.
**Action:** `.visually-hidden`이나 ARIA 요소를 대화형 요소(`<a>` 등) 내부에 배치할 때, hover나 focus 스타일 적용을 위해 구조적 선택자를 사용하는 대신 변경이 필요한 콘텐츠에 `.entry-name`과 같은 구체적인 클래스를 부여하여 명확하게 스타일 대상을 지정해야 합니다.
