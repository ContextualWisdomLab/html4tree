...
## 2026-09-03 - 내비게이션 접근성 및 번역 호환성 개선
**Learning:** `<nav>` 블록 내에서 시각적으로 숨겨진 제목을 가리키는 `aria-labelledby`를 사용하는 것이 `aria-label`보다 우수합니다. 이는 스크린 리더 사용자가 '제목으로 탐색'할 수 있게 하며, 브라우저 번역기가 `aria-label` 속성을 번역하지 못하는 i18n 문제를 텍스트 노드를 정상적으로 처리하여 해결합니다.
**Action:** `<nav>` 요소에 `aria-label` 대신 항상 시각적으로 숨겨진 제목을 가리키는 `aria-labelledby`를 우선적으로 사용하십시오.
