...
## 2024-10-24 - 파일 및 디렉토리 이름의 양방향 텍스트(BiDi) 지원 개선
**학습:** 아랍어나 히브리어와 같은 Right-To-Left(RTL) 문자가 포함된 파일 또는 디렉토리 이름이 Left-To-Right(LTR) 레이아웃 내에 렌더링될 때, 텍스트 방향성이 지정되지 않으면 구두점이나 확장자의 위치가 잘못 표시되어 가독성을 심각하게 해칩니다.
**조치:** 파일 이름이나 디렉토리 이름과 같이 사용자 제공 텍스트가 렌더링되는 HTML 요소(`<h1>`, `<span>` 등)에 항상 `dir="auto"` 속성을 추가하여 브라우저가 해당 콘텐츠의 텍스트 방향을 양방향 텍스트(BiDi) 알고리즘에 따라 올바르게 렌더링하도록 보장하십시오.
