...
## 2026-07-31 - 빈 디렉토리 상태의 시각적 및 의미론적 접근성 개선
**Learning:** 상태 메시지(빈 디렉토리 등)는 시각적으로만 전달되면 스크린 리더 사용자에게 누락될 수 있으며, `role="status"`를 부여함으로써 보조 기기가 동적인 상태를 즉각 인지하도록 만들 수 있습니다. 또한 목록 내의 일반 항목과 동일한 시각적 계층(Flexbox 및 아이콘 적용)을 제공하면 일반 사용자에게도 일관된 UI/UX 경험을 제공합니다.
**Action:** 향후 정적 HTML 뷰나 상태가 변경되는 UI 요소에서 내용이 비어있거나 검색 결과가 없을 때 반드시 적절한 ARIA role(예: `role="status"` 또는 `role="alert"`)을 적용하고, 장식용 시각 요소는 `aria-hidden="true"`로 처리하여 접근성을 보장할 것입니다.
