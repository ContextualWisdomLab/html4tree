...
## 2024-08-20 - 혼합된 텍스트 방향(BiDi) 호환성 및 다국어 지원 향상
**학습:** 아랍어나 히브리어와 같은 우측에서 좌측으로 읽는(RTL) 언어가 좌측에서 우측으로 읽는(LTR) HTML 레이아웃에 나타날 때(예: 파일 및 디렉토리 이름), `dir="auto"` 속성이 없으면 브라우저가 문자를 올바르게 렌더링하지 못해 텍스트가 뒤섞이거나 혼란스럽게 표시될 수 있습니다.
**조치:** 다국어 환경에서 파일명이나 디렉토리명과 같이 텍스트가 포함된 동적 콘텐츠가 생성될 때 양방향 텍스트(BiDi) 호환성을 보장하기 위해 텍스트를 포함하는 요소(`<h1>`, `<span>` 등)에 항상 `dir="auto"`를 추가하십시오.
