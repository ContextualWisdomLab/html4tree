...
## 2026-08-09 - 모바일 브라우저 탭 및 상태 표시줄 테마 색상 지원
**Learning:** `color-scheme` 메타 태그와 다크 모드 CSS가 있어도 모바일 브라우저(예: 모바일 Safari, Chrome)의 주소창 및 상태 표시줄 영역은 기본 색상으로 남아 시각적 이질감을 줄 수 있습니다.
**Action:** 항상 `<meta name="theme-color" ... media="(prefers-color-scheme: ...)">` 메타 태그를 명시적으로 추가하여 브라우저 UI 색상이 웹 페이지의 배경색(다크/라이트 모드)과 자연스럽게 동기화되도록 하십시오.
