...
## 2024-06-18 - Fix broken hover states and add active feedback
**Learning:** Adding `.visually-hidden` for accessibility (e.g., hiding link types or aria-labels) can break CSS rules relying on `:last-child` if the hidden element is appended to the end of a list or link, causing visible interactions like text underlines to disappear.
**Action:** When updating HTML structure for a11y, verify and update fragile CSS pseudo-class selectors like `:last-child`. Use `:nth-last-child(2)` or explicit classes to ensure styles target the correct visible elements, and always include `:active` pseudo-class styles for tactile interaction feedback.
