...
## 2026-08-20 - 내비게이션 요소의 다국어 번역 및 화면 판독기 호환성 개선
**학습:** `<nav>` 요소에 `aria-label`을 사용하면 Chrome Translate 등 브라우저 번역 도구가 해당 텍스트를 번역하지 못하는 문제가 발생합니다. 이로 인해 문서의 언어가 번역되더라도 화면 판독기 사용자는 원본 언어로 내비게이션 설명을 듣게 되어 접근성이 저하됩니다.
**조치:** 내비게이션 요소의 설명을 제공할 때는 `aria-label` 대신 내부에 시각적으로 숨겨진 제목 요소(예: `<h2 id="nav-heading" class="visually-hidden">`)를 포함하고 `<nav aria-labelledby="nav-heading">`을 사용하여 화면 판독기 접근성('제목 단위 이동')과 다국어 번역 호환성을 동시에 확보하십시오.
