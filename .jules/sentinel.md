...
## 2024-07-16 - [LOW] glob 패턴 구문 분석 예외 처리 보안 강화
**Vulnerability:** 악의적이거나 잘못된 포맷의 glob 정규식에 의한 DoS
**Learning:** `java.nio.file.FileSystems.getDefault().getPathMatcher`는 잘못된 glob 패턴에 대해 `PatternSyntaxException`뿐만 아니라 `IllegalArgumentException`도 던질 수 있음을 배웠습니다. 하나만 잡을 경우 런타임 오류가 발생해 애플리케이션 충돌(DoS)을 유발할 수 있습니다.
**Prevention:** 광범위한 잘못된 정규표현식 입력을 안전하게 무시하기 위해 하위 클래스인 `PatternSyntaxException` 대신 `IllegalArgumentException`을 포괄적으로 잡아주어야 합니다.
