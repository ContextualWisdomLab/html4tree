...
## 2024-09-17 - [CRITICAL] Fix TOCTOU vulnerability in process_ignore_file
**Vulnerability:** A Time-of-Check to Time-of-Use (TOCTOU) vulnerability existed in `process_ignore_file`. The code checked `ignore_file.isFile`, `Files.isSymbolicLink(ignore_file.toPath())`, and `ignore_file.canRead()` before calling `ignore_file.useLines`. If the file was replaced by a symbolic link or became unreadable after the check but before `useLines`, it could lead to policy bypass or DoS.
**Learning:** Checking file attributes (stat) and then opening the file is fundamentally vulnerable to race conditions. Additionally, ignoring the return value or failure to read the policy file should lead to a fail-closed state, not a fail-open state where the directory contents are published.
**Prevention:** 1) Implement fail-closed behavior by throwing an exception (`IgnoreFileReadException`) if the policy file cannot be safely read, and 2) Ensure the calling code catches this exception and skips processing the directory entirely. Use robust I/O error handling (catching IOException during reads) to enforce the fail-closed policy.
## 2024-09-17 - [CRITICAL] Fix TOCTOU vulnerability in process_ignore_file
**Vulnerability:** A Time-of-Check to Time-of-Use (TOCTOU) vulnerability existed in `process_ignore_file`. The code checked `ignore_file.isFile`, `Files.isSymbolicLink(ignore_file.toPath())`, and `ignore_file.canRead()` before calling `ignore_file.useLines`. If the file was replaced by a symbolic link or became unreadable after the check but before `useLines`, it could lead to policy bypass or DoS.
**Learning:** Checking file attributes (stat) and then opening the file is fundamentally vulnerable to race conditions. Additionally, ignoring the return value or failure to read the policy file should lead to a fail-closed state, not a fail-open state where the directory contents are published.
**Prevention:** 1) Implement fail-closed behavior by throwing an exception (`IgnoreFileReadException`) if the policy file cannot be safely read, and 2) Ensure the calling code catches this exception and skips processing the directory entirely. Use robust I/O error handling (catching IOException during reads) to enforce the fail-closed policy. When checking attributes, evaluate them only after opening the file descriptor or within the context of the opened stream to avoid race conditions.

## 2024-09-17 - [CRITICAL] Prevent DoS from Named Pipes
**Vulnerability:** Moving static `isFile()` checks until after `useLines()` or `FileInputStream` is invoked exposes the application to Denial of Service via named pipes (FIFOs). Opening a named pipe blocks the thread indefinitely.
**Learning:** `isFile` correctly returns false for FIFOs. File attributes should be checked *before* opening the stream to prevent DoS. TOCTOU vulnerabilities resulting from checking attributes before opening must instead be handled by catching runtime exception failures (like `IOException`) while evaluating the stream.
**Prevention:** Always combine `!isFile()` pre-checks with `IOException` catching during I/O operations. Do not rely entirely on post-open checks for basic file types.
