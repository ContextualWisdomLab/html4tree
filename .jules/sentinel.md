...
## 2026-08-06 - [숨김 파일 검사 우회 방지 (유니코드 호모글리프)]
**Vulnerability:** 악의적인 사용자가 유니코드 호모글리프(예: `。`, `．`, `｡`)를 사용하여 숨김 파일과 유사한 이름의 파일을 생성할 경우, 기존의 `.startsWith(".")` 기반 숨김 파일 검사를 우회하여 해당 파일이 인덱싱되고 정보가 노출될 수 있는 취약점이 있습니다.
**Learning:** 숨김 파일이나 디렉토리를 필터링할 때, 일반적인 ASCII 점(.)만을 검사하는 것은 충분하지 않습니다. 다양한 언어와 문자 인코딩에서 유사하게 보이는 문자를 이용한 보안 우회 공격(Homoglyph attack)이 가능하므로 이에 대한 방어가 필요합니다.
**Prevention:** 파일 이름이 점(.)으로 시작하는지 검사할 때, ASCII 점 뿐만 아니라 의미적으로나 시각적으로 유사한 유니코드 문자(U+3002, U+FF0E, U+FF61)도 포함하여 숨김 파일로 간주하도록 필터링 로직을 확장해야 합니다.
