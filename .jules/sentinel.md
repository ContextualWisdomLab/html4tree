...
## 2024-05-24 - [Fail-Closed Security Posture Testing]
**Learning:** Testing fail-closed paths involving exceptions (like `IOException` from TOCTOU) in standard functions like `useLines` can be flaky if using thread-based concurrency. Injecting file I/O mocks via default lambda parameters (e.g., `processLines: (File, (Sequence<String>) -> Unit) -> Unit = { file, block -> file.useLines { block(it) } }`) provides a deterministic way to trigger these paths and maintain 100% test coverage without flakiness.
**Action:** Always refactor external I/O interactions that need exception testing into mockable lambda parameters with sensible defaults to avoid Groundedness Rule violations related to thread flakiness.
