...
## 2026-08-09 - 유니코드 유사 문자를 이용한 숨김 파일 보호 우회
**Learning:** 파일 이름이 점(.)으로 시작하는지 확인할 때 ASCII 점(0x2E)만 검사하면 U+FF0E, U+3002, U+FF61과 같은 유니코드 유사 문자(look-alike)로 시작하는 악성 숨김 파일이나 디렉토리가 필터링을 우회하여 민감한 정보가 노출될 수 있습니다.
**Action:** 숨김 파일 판별 로직을 구현할 때 ASCII 점뿐만 아니라 시각적으로 점과 유사한 유니코드 문자열을 포함하여 더 포괄적으로 검사하는 확장 함수(예: `isHiddenFile()`)를 사용하십시오.
