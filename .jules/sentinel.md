...
## 2024-07-26 - [html4tree] 파일 이름의 원시 양방향 제어 문자(BiDi Controls) 무력화
**Vulnerability:** 파일 이름 내에 악의적으로 삽입된 원시 양방향 제어 문자(예: PDI, RLO 등)가, 외부의 격리용 `&#x2068;`/`&#x2069;` 문자를 임의로 종결시키거나 무력화하여 의도치 않은 방향성 재정의(override)를 유발할 수 있습니다.
**Learning:** `&#x2068;`와 `&#x2069;`로 감싸는 것만으로는 충분하지 않습니다. 사용자가 입력한 문자열 내부에 직접 들어있는 원시 BiDi 제어 문자(예: `U+2069 PDI`, `U+202E RLO`)는 렌더링 시 브라우저에 의해 해석되어 외부의 격리를 탈출하고, 주변 UI의 텍스트 렌더링 방향을 왜곡시킬 수 있습니다(UI Spoofing).
**Prevention:** UI 스푸핑을 방지하기 위해 사용자 제어 텍스트(파일 이름 등)를 디스플레이용으로 변환할 때(`escapeHtml`), 해당 텍스트 내의 원시 양방향 제어 문자를 `\u2069`와 같은 가시적인 이스케이프 문자열로 변환하여 렌더링 시 브라우저가 제어 문자로 해석하지 못하도록 무력화(neutralize)해야 합니다.
