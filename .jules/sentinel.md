...
## 2026-08-01 - [html4tree] CSP 해시 불일치 방지를 위한 인라인 스타일 정제 및 성능 최적화
**Vulnerability:** Kotlin 다중 줄 문자열(multiline string)을 사용하여 인라인 스타일을 주입할 때, 의도치 않은 공백이나 줄바꿈이 포함되면 브라우저가 계산하는 CSP 해시값과 생성된 메타 태그의 해시값이 일치하지 않아(CSP Hash Mismatch), 보안 정책이 올바르게 적용되지 않거나 스타일이 차단되는 문제가 발생합니다.
**Learning:** 콘텐츠 보안 정책(CSP) 해시는 대상 콘텐츠의 정확한 내부 텍스트(inner text)를 기반으로 계산됩니다. 따라서 템플릿에 문자열을 삽입할 때 암시적인 패딩이나 여백이 포함되지 않도록 주의해야 하며, 매번 함수 호출 시 해시를 계산하는 것은 성능 저하를 일으킬 수 있습니다.
**Prevention:** 해시 계산 전 대상 문자열에 `.trimIndent()`를 적용하여 공백을 제거하고, HTML에 주입할 때 `<style>${exactStyleContent}</style>`와 같이 공백 없이 주입하십시오. 또한 고정된 정적 문자열과 해시 계산은 `private val` 형태의 최상위 상수로 끌어올려 중복 계산과 메모리 할당을 방지하십시오.
