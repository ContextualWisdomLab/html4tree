...
## 2024-09-19 - [Fix BiDi text spoofing]
**Vulnerability:** Bidirectional (BiDi) text spoofing (e.g., RTL injection) in the HTML generation. Attackers could trick users into executing malicious files by hiding real file extensions.
**Learning:** Raw Unicode Bidi_Control characters should be deterministically exposed as escaped strings. Additionally, user-controlled text within HTML should be isolated with First Strong Isolate (FSI) and Pop Directional Isolate (PDI) to prevent spoofing.
**Prevention:** Explicitly escape BiDi control characters in the HTML escaping logic and wrap dynamic content with `&#x2068;` and `&#x2069;`.
