...
## 2024-07-25 - [html4tree] 파일 교체 시 경쟁 조건(Race Condition) 방지
**Vulnerability:** 파일 교체 시 원자적 이동(ATOMIC_MOVE) 누락으로 인한 경쟁 조건.
**Learning:** `Files.move`를 사용하여 임시 파일을 최종 목적지(예: `index.html`)로 이동할 때 원자적 이동 옵션(`StandardCopyOption.ATOMIC_MOVE`)을 사용하지 않으면, 이동 과정에서 파일이 불완전하게 복사된 상태로 다른 프로세스나 스레드에 의해 읽힐 수 있는 경쟁 조건 취약점이 발생할 수 있습니다.
**Prevention:** 파일 교체 시 항상 `StandardCopyOption.ATOMIC_MOVE` 옵션을 먼저 시도하고, 파일 시스템에서 지원하지 않아 `AtomicMoveNotSupportedException`이 발생할 경우에만 `StandardCopyOption.REPLACE_EXISTING`으로 폴백(Fallback)하도록 구현하십시오.
