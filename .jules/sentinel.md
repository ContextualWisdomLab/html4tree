...
## 2026-08-04 - 호모그래프 공격(Homograph attack)을 통한 숨김 파일 노출 방지
**Vulnerability:** `.startsWith(".")`를 사용하여 숨김 파일을 필터링할 때, 유니코드 유사 문자(Homoglyphs, 예: U+3002, U+FF0E, U+FF61)를 사용하면 필터링을 우회하여 중요 파일이 노출될 수 있음.
**Learning:** 숨김 파일 또는 특수 파일을 필터링할 때 ASCII '.'뿐만 아니라 여러 유니코드 점(dot) 변형 문자들도 함께 필터링해야 함.
**Prevention:** 파일명 검사 시 점(dot)의 유니코드 변형 문자들(U+3002, U+FF0E, U+FF61)도 함께 포함되도록 검사 로직(isHiddenFile 등)을 강화하여 적용해야 함.
