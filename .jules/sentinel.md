...
## 2026-07-16 - 정보 노출 방지를 위한 robots 메타 태그 추가
**Vulnerability:** html4tree가 생성한 index.html 파일들이 웹 서버에 의해 호스팅될 때 검색 엔진에 의해 색인화되어 디렉토리 구조와 파일 목록(잠재적으로 민감한 정보)이 노출될 위험이 존재함.
**Learning:** 정적 사이트 생성기나 디렉토리 인덱스 도구는 기본적으로 생성된 산출물이 퍼블릭에 노출될 때 검색 엔진 크롤러가 민감한 디렉토리 구조를 크롤링하지 않도록 명시적으로 제어해야 함.
**Prevention:** 생성되는 HTML 템플릿의 <head> 영역에 항상 `<meta name="robots" content="noindex, nofollow">` 태그를 포함하여 검색 엔진 인덱싱을 기본적으로 차단해야 함.
