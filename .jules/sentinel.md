...
## 2026-08-05 - [html4tree] ReDoS Vulnerability in PathMatcher Glob Pattern Compilation
**Vulnerability:** The application was passing user-controlled glob patterns from `.html4ignore` directly into `java.nio.file.FileSystems.getDefault().getPathMatcher("glob:$pattern")` without validating the complexity of the pattern. Complex patterns with many overlapping wildcards (e.g., `*{*,*,*,*,*,*,*,*,*,*,*,*}*`) could trigger exponential backtracking and result in a Denial of Service (ReDoS).
**Learning:** Constructing a JDK glob matcher from unrestricted attacker-controlled patterns is unsafe. Even simple glob pattern to regex translation can result in pathologically slow regex engines matching. The standard length limitations (e.g. 100 characters) are not enough to protect against nested or repetitive wildcards.
**Prevention:** Apply a bounded-complexity budget before compiling glob patterns. Validate that the wildcard character count (`*`, `?`, `{`) does not exceed a reasonable limit (e.g., 5 total wildcards per line) and reject excessive overlapping wildcard structures to prevent JVM matcher exponential backtracking.
