...
## 2024-07-23 - [html4tree] 확장자 기반 민감 파일 동적 제외
**Vulnerability:** 민감한 확장자를 가진 파일(예: `.pem`, `.db`, `.log`, `.sqlite`, `.key` 등)이 기본 제외 목록에 명시되어 있지 않아 디렉토리 인덱스에 노출될 위험 (Information Exposure).
**Learning:** 기본 제외 목록에 고정된 특정 파일명(예: `id_rsa`)만 포함하는 것은 새로운 이름의 키나 데이터베이스 파일 등을 보호하지 못합니다. 숨김 파일 제외뿐만 아니라 특정 민감한 확장자를 가진 파일을 동적이고 대소문자 구분 없이 제외하는 것이 보다 강력한 심층 방어(defense-in-depth)를 제공함을 배웠습니다.
**Prevention:** 정적 목록을 유지하되, `toLowerCase().endsWith(ext)`와 같이 동적으로 확장자를 검사하여 민감할 수 있는 모든 파일을 사전 차단(Secure by Default)하십시오.
