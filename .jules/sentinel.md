...
## 2024-05-18 - BiDi Spoofing Prevention
**Vulnerability:** Bidirectional (BiDi) text spoofing (e.g., RTL injection) can hide malicious file extensions by reversing character display order.
**Learning:** `dir="auto"` is inadequate for isolating user-controlled text within HTML attributes or elements like `<title>`. Raw Unicode Bidi_Control characters (like `\u202E`) must be escaped.
**Prevention:** Deterministically expose raw Unicode Bidi_Control characters as escaped strings within HTML escaping functions. Additionally, isolate user-controlled text with First Strong Isolate (`&#x2068;`) and Pop Directional Isolate (`&#x2069;`).
