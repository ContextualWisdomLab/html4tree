...
## 2024-05-24 - DoS by Extremely Long Paths
**Vulnerability:** The application was missing input validation on the length of the `topDir` argument in `go()`. Passing excessively long strings (e.g., 4097+ characters) can lead to Denial of Service (DoS) and Out-Of-Memory (OOM) exceptions when attempting to resolve or manipulate the string as a file path.
**Learning:** File system methods are expensive. Validating input lengths bounds memory consumption and prevents unhandled exceptions from large inputs causing application crashes.
**Prevention:** Implement strict length limitations on user-provided path inputs before executing file system operations.
