...
## 2024-07-31 - [html4tree] CSP Hash Mismatch in Inline Styles
**Vulnerability:** Inline styles with incorrectly calculated Content-Security-Policy (CSP) hashes were being blocked by the browser. This weakens the security posture and causes styles to break or inline scripts to fail execution, negating the defense-in-depth provided by CSP.
**Learning:** The browser calculates CSP hashes based on the *exact* inner text content. When injecting multiline string templates in Kotlin, surrounding whitespace or newline paddings in the template wrapper implicitly alter the final string, causing a mismatch with the hash calculated on the unpadded content string.
**Prevention:** Always apply `.trimIndent()` to multiline content strings before hashing them, and inject them into the HTML structure without any implicit padding or extra newlines (e.g., `<style>${exactStyleContent}</style>`) to ensure perfect hash matching.
