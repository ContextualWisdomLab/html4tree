...
## 2024-08-29 - TOCTOU DoS via File.useLines()
**Vulnerability:** Unhandled `IOException` and `UncheckedIOException` during `File.useLines()` execution on user-modifiable files can crash the application, leading to a local Denial of Service (DoS) via Time-Of-Check to Time-Of-Use (TOCTOU) conditions.
**Learning:** Validating a file using `isFile`, `canRead`, and size prior to reading does not guarantee it will remain readable or prevent I/O errors during `useLines`.
**Prevention:** Always wrap file I/O operations (such as `File.useLines()`) in a `try-catch` block handling both `IOException` and `UncheckedIOException` to fail securely without process termination.
