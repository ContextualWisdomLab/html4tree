...
## 2024-05-24 - [TOCTOU in Policy File Reading]
**Vulnerability:** A TOCTOU (Time-of-Check to Time-of-Use) vulnerability existed when reading `.html4ignore` policy files. If an attacker replaced the file with a symbolic link or directory after it was enumerated but before it was processed, it could lead to exceptions or unintended behavior. More importantly, when exceptions occurred during policy reading, they were swallowed and the directory was processed with an empty exclusion list (fail-open), exposing files that were meant to be hidden.
**Learning:** Security policy files must enforce fail-closed behavior. If a policy file exists but cannot be securely read (e.g., due to permission errors, symlink attacks, or being a directory), the system should halt processing of that component rather than defaulting to permissive behavior.
**Prevention:** Implement explicit `isFile`, `isSymbolicLink`, and `canRead` checks on policy files. If validation fails, throw a specific exception (e.g., `IgnoreFileReadException`). Ensure the caller catches this exception and handles it by safely skipping the operation (fail-closed), rather than falling back to an insecure default.
