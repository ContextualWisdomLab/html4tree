...
## 2024-07-15 - [html4tree] 자동 생성된 HTML 디렉토리 인덱스의 정보 노출 방지
**Vulnerability:** 공개 웹 서버에 호스팅된 자동 생성 디렉토리 인덱스가 검색 엔진에 의해 무단으로 크롤링 및 색인화될 수 있는 정보 노출 (Information Exposure) 위험.
**Learning:** 기본 제공되는 메타 태그를 명시적으로 설정하지 않으면, 검색 엔진 봇은 로컬 환경을 위해 생성된 정적 디렉토리 파일 구조까지 수집하여 외부에 노출시킬 수 있습니다.
**Prevention:** HTML 기반의 파일 구조 브라우저를 렌더링할 때는 항상 `<meta name="robots" content="noindex, nofollow">` 태그를 삽입하여 검색 엔진 봇의 접근 및 색인을 차단하십시오.
