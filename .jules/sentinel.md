...
## 2026-07-28 - [html4tree] CSP 해시 불일치로 인한 인라인 스타일 차단 방지
**Vulnerability:** Kotlin 다중 줄 문자열에 포함된 암묵적 여백 및 줄바꿈이 삽입 시 HTML 콘텐츠와 일치하지 않아 CSP 해시를 무효화하는 문제
**Learning:** 브라우저는 인라인 스크립트 및 스타일의 정확한 내부 텍스트(공백 포함)를 기반으로 CSP 해시를 계산하므로, 템플릿에 삽입 시 양쪽 공백이 일치하지 않으면 유효하지 않은 해시가 됨
**Prevention:** 템플릿에 문자열을 삽입하기 전에 .trimIndent()를 적용하고 여백 없이 태그 사이에 정확히 주입(<style>${exactContent}</style>)하여 정확한 해시 매칭 보장
