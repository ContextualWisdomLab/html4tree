...
## 2026-07-29 - [MEDIUM] CSP 해시 불일치 (CSP Hash Mismatch)
**Vulnerability:** 인라인 스타일의 실제 내용과 생성된 CSP(Content-Security-Policy) 해시가 일치하지 않아 브라우저가 스타일 적용을 차단하거나 보안 정책을 제대로 검증하지 못하는 취약점이 발견되었습니다.
**Learning:** 멀티라인 문자열을 사용하여 HTML 템플릿에 스크립트나 스타일을 삽입할 때, 들여쓰기와 줄바꿈 등 주변 공백(Whitespace)이 최종 문자열에 포함되면서 원래 계산했던 콘텐츠 해시(SHA-256)와 달라지는 문제가 발생합니다. 브라우저는 `<style>`과 `</style>` 안의 정확한 텍스트로 해시를 계산하므로 단 하나의 공백 차이도 해시 불일치를 유발합니다.
**Prevention:** 템플릿 문자열을 삽입할 때 `.trimIndent()`를 적용하여 불필요한 들여쓰기를 제거하고, 주변 공백이나 줄바꿈 없이 정확히 일치하는 콘텐츠를 주입하여 생성된 해시와 실제 주입된 콘텐츠가 완벽히 동일하도록 유지해야 합니다. 또한, 변하지 않는 리소스와 해시는 매번 계산하지 않도록 `private val` 상수로 추출하여 안전하게 관리하십시오.
