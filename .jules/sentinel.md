...
## 2024-07-15 - [CRITICAL] 숨김 파일 유니코드 우회 취약점 방지
**Vulnerability:** 파일 이름 검증 시 유니코드 유사 문자를 통한 숨김 파일 우회 (Information Exposure)
**Learning:** `startsWith(".")` 방식만으로는 유니코드의 다른 형태 점 문자(예: `\u3002`, `\uFF0E`, `\uFF61`)로 조작된 악의적인 파일이 필터링을 우회하여 노출될 수 있음을 배웠습니다.
**Prevention:** 파일 필터링 로직에서 유니코드 등가 문자를 함께 처리하는 일관된 유틸리티 함수(`isHiddenFile`)를 사용하여 우회를 방지해야 합니다.
