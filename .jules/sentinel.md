...
## 2024-07-14 - [MEDIUM] TOCTOU 및 입출력 예외로 인한 크롤러 중단(DoS) 방지 및 Fail Securely 구현
**Vulnerability:** 파일 파싱 중 발생하는 IO 예외(`FileNotFoundException`, `AccessDeniedException` 등)나 잘못된 파일 이름 널바이트 등으로 인한 `InvalidPathException` 발생 시 전체 크롤러가 크래시(DoS)되는 문제. TOCTOU 상황에서도 파일 접근 오류로 크래시가 발생할 수 있습니다.
**Learning:** 파일 I/O나 경로 처리 중에는 다양한 예외가 발생할 수 있으며, 단일 파일이나 디렉토리에 대한 예외가 전체 프로세스의 실행을 막아선 안 됩니다. 예외를 안전하게 잡아내어 크래시를 방지하고, 기본 설정(예: 민감한 파일 무시)을 유지하여 진행하는 Fail Securely 전략이 필요합니다.
**Prevention:** 크롤러와 같은 재귀적 I/O 프로세스에서는 각 노드(파일/디렉토리) 처리 로직을 `try-catch`로 감싸 예상치 못한 예외가 발생하더라도 프로세스가 계속 실행될 수 있도록 방어적 프로그래밍을 적용하십시오.
