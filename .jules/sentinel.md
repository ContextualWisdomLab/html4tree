...
## 2026-07-15 - [동적 중요 파일 제외 로직 추가]
**Vulnerability:** `.pem`, `.key`와 같은 자격 증명 파일이나 `.sqlite`, `.db`와 같은 데이터베이스 파일이 `.html4ignore` 또는 기본 민감 파일 목록에서 명시적으로 무시되지 않을 경우 디렉토리 목록 조회 시 노출될 수 있었습니다.
**Learning:** 정적 목록만 사용하는 것은 동적으로 생성된 파일 이름이나 특정 확장자 유형을 모두 누락할 위험이 있습니다. 이러한 파일이 노출되지 않도록 디렉토리 내의 모든 파일에 대해 동적으로 확장자 단위로 검사해야 합니다.
**Prevention:** 파일 이름에 대해 동적인 검사(예: `.endsWith` 사용)를 적용하여 민감한 파일 유형의 노출을 정확하게 방지하고, 이를 통해 강력한 심층 방어(defense in depth) 수단으로 활용하십시오.
