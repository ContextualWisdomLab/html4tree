...
## 2026-07-25 - [MEDIUM] CSP 스타일 해시 불일치로 인한 정책 위반 및 보안 취약점 수정
**Vulnerability:** 정적 리소스용 CSP 생성 시 멀티라인 문자열의 여백(들여쓰기)으로 인해 실제 브라우저가 계산하는 해시값과 메타 태그의 해시값이 일치하지 않아 CSP 검증이 실패하는 취약점 발생.
**Learning:** 브라우저는 인라인 스타일이나 스크립트의 CSP 해시를 계산할 때 태그 내부의 공백과 줄바꿈을 모두 포함하여 계산합니다. Kotlin과 같이 템플릿 리터럴(멀티라인 문자열)을 사용할 때 의도치 않게 추가된 여백이 해시 불일치를 유발할 수 있습니다.
**Prevention:** `<style>` 또는 `<script>` 태그에 콘텐츠를 주입하여 해시 기반 CSP를 적용할 때는 `.trimIndent()`와 같은 기능을 사용하여 불필요한 여백을 제거하고, 해시 계산에 사용된 문자열이 정확히 일치하도록 템플릿의 여백 없이 주입(e.g., `<style>${exactContent}</style>`)해야 합니다.
