...
## 2026-08-18 - 루프 내 문자열 보간(String interpolation) 오버헤드 최적화
**Learning:** 디렉토리 항목을 렌더링할 때 반복문 내에서 문자열 보간(예: `"""...${...}..."""`)을 사용하면, 컴파일러가 매 루프마다 새로운 객체 생성과 문자열 빌딩 과정을 수행하게 되어 대규모 디렉토리 탐색 시 상당한 할당(Allocation) 오버헤드와 가비지 컬렉션(GC) 부하가 발생합니다.
**Action:** 핫 패스인 렌더링 루프(`index_middle`) 내부에서는 문자열 보간 대신 외부 루프의 `StringBuilder` 인스턴스에 직접 `.append()`를 체이닝하도록 수정하여 불필요한 중간 객체 생성을 원천 차단했습니다.
