...
**학습:** `isDirectory`, `!it.isDirectory()`, `isSymbolicLink` 3개의 개별적인 파일 시스템 I/O 호출을 수행하면 성능 저하가 큽니다. 이를 단일 `Files.readAttributes` 호출로 변경하여 메타데이터를 한 번에 조회함으로써 I/O 오버헤드를 대폭 줄일 수 있음을 확인했습니다.
**조치:** 디렉토리 순회 시 파일의 여러 속성을 확인할 때는 개별적인 stat 호출보다 `Files.readAttributes`를 사용하여 필요한 모든 속성을 한 번에 가져오는 방식을 우선적으로 고려해야 합니다.
## 2026-07-29 - 상수 및 무거운 연산의 전역 호이스팅 (Hoisting)
**학습:** `process_dir`과 같이 자주 호출되는 함수 내부에 정적 문자열(`cssContent`, `css`)이나 무거운 연산(SHA-256 해시 계산), 고정된 리스트(`defaultSensitiveFiles`)를 선언하면 함수가 호출될 때마다 불필요한 객체 할당과 계산이 발생하여 성능 저하 및 가비지 컬렉션(GC) 압력을 유발합니다.
**조치:** 불변하는 정적 문자열과 비용이 많이 드는 계산은 함수 외부의 파일 레벨 `private val` 상수로 끌어올려(Hoisting) 애플리케이션 수명 주기 동안 한 번만 할당 및 계산되도록 최적화합니다. `private val`을 사용하여 불필요한 getter 생성을 방지하고 테스트 커버리지를 유지합니다.
## 2026-08-05 - 마이크로 최적화의 범위 및 정책(Policy) 분리 실패\n**학습:** 여러 가지 최적화(예: SHA-256 호이스팅 및 민감 파일 목록 호이스팅)를 하나의 PR로 묶어서 제출하면, 각 최적화가 요구하는 테스트 및 보안 정책 기준이 달라 PR이 거절(Reject)되거나 중복(Duplicate) 처리될 위험이 있습니다.  호이스팅은 단순 성능 개선이 아닌 보안 정책 및 벤치마크 테스트와 연관된 별도의 마이크로 최적화로 취급되어야 합니다.\n**조치:** 앞으로 성능 개선 PR을 작성할 때는 서로 다른 도메인(예: 스타일시트 성능과 보안 제외 정책)의 변경 사항을 섞지 말고, 완전히 독립적인 단위로 분리하여 별도의 PR로 제출합니다. 벤치마크나 정책 테스트가 필요한 항목은 분리하여 충분한 근거(Evidence)와 함께 제안해야 합니다.
## $(date -I) - 마이크로 최적화의 범위 및 정책(Policy) 분리 실패
**학습:** 여러 가지 최적화(예: SHA-256 호이스팅 및 민감 파일 목록 호이스팅)를 하나의 PR로 묶어서 제출하면, 각 최적화가 요구하는 테스트 및 보안 정책 기준이 달라 PR이 거절(Reject)되거나 중복(Duplicate) 처리될 위험이 있습니다. `defaultSensitiveFiles` 호이스팅은 단순 성능 개선이 아닌 보안 정책 및 벤치마크 테스트와 연관된 별도의 마이크로 최적화로 취급되어야 합니다.
**조치:** 앞으로 성능 개선 PR을 작성할 때는 서로 다른 도메인(예: 스타일시트 성능과 보안 제외 정책)의 변경 사항을 섞지 말고, 완전히 독립적인 단위로 분리하여 별도의 PR로 제출합니다. 벤치마크나 정책 테스트가 필요한 항목은 분리하여 충분한 근거(Evidence)와 함께 제안해야 합니다.
