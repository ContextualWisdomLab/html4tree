...
## 2024-05-24 - 루프 내 할당 핫 패스 (toLowerCase)
**학습:** 파일 필터링 등의 반복(핫 패스) 영역 내에서 `toLowerCase()`와 같은 문자열 할당 작업을 무조건적으로 수행하면 가비지 컬렉션 부하를 높이게 됩니다.
**조치:** 값싼 불리언이나 속성 검사(`isHiddenFile()`, `endsWith()`)를 먼저 평가하고(short-circuit), 비용이 드는 문자열 할당 작업은 꼭 필요한 경우로 지연(lazy evaluation)시킵니다.
