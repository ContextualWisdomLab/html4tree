...
## 2026-08-11 - ArrayDeque 도입으로 인한 BFS 탐색 성능 개선 및 GC 오버헤드 최적화
**Learning:** 커스텀 Node(예: `Entry`, `LinkedListEntry`)를 할당하며 사용하는 `LinkedList` 방식은 BFS 탐색에서 큐의 push/pull 동작마다 래퍼 객체를 지속적으로 할당하므로 심각한 가비지 컬렉션(GC) 부하와 오버헤드를 발생시킵니다.
**Action:** BFS 트래버설을 위한 큐 구현은 `java.util.ArrayDeque`를 사용하도록 완전히 대체하여, 래퍼 노드 객체의 지속적인 할당을 방지하고 캐시 지역성을 높여 퍼포먼스를 향상시켜야 합니다.
