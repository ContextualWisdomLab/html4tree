...
## 2024-09-02 - 저렴한 검사 전 무조건적인 문자열 할당 지연
**학습:** 파일 순회 루프 내에서 숨김 파일 여부나 단순 접미사(`.endsWith("~")`) 검사와 같이 메모리 할당이 필요 없는 조건들을 먼저 평가하지 않고 매번 `toLowerCase()`를 호출하면, 처리할 파일이 많을 때 불필요한 중간 문자열 할당으로 인해 GC 부하가 크게 발생합니다.
**조치:** 무조건적인 문자열 할당을 유발하는 메서드(`toLowerCase()`, `replace()` 등)를 호출하기 전에 할당 오버헤드가 없는 저렴한 문자열 조건(예: 문자 순회 검사, `endsWith` 등)을 먼저 확인하도록 조건문을 분리하고 재배열합니다.
