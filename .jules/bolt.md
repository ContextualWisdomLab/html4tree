...
**학습:** `Files.isDirectory(it.toPath(), LinkOption.NOFOLLOW_LINKS)`, `!it.isDirectory()`, `!Files.isSymbolicLink(it.toPath())` 3개의 개별적인 파일 시스템 I/O 호출을 수행하면 성능 저하가 큽니다. 이를 단일 `Files.readAttributes` 호출로 변경하여 메타데이터를 한 번에 조회함으로써 I/O 오버헤드를 대폭 줄일 수 있음을 확인했습니다.
...
**조치:** `crawl_directories`에서 `Array(files.size)`로 파일명 배열을 직접 생성합니다. 순서, null 처리, ignore 입력과 파일 시스템 호출 정수를 변경하지 않습니다.
...
## 2024-05-24 - 중복된 I/O 오버헤드를 줄이기 위한 디렉토리 목록 캐싱
**학습:** 서로 다른 조건부 실행 블록 (예: `dirFilesNames ?: curr_dir.list()`)에서 `File.list()`가 여러 번 평가될 때, 파라미터가 null이면 값비싼 OS I/O 호출이 중복되어 발생합니다.
**조치:** 디렉토리 목록(`curr_dir.list()`)을 모든 조건부 실행 경로보다 상단에 지역 변수로 끌어올려(hoist) 캐싱함으로써 평가가 단 한 번만 이루어지도록 하여 성능상 이점을 확보해야 합니다.
