...
## 2024-09-04 - [Kotlin 문자열 이스케이프 성능 최적화: 조건 분기를 배열 조회로 대체]
**Learning:** Kotlin에서 hot path(예: HTML 이스케이프 등 빈번한 문자열 조작)의 문자 매핑 시, `when` 조건 분기(jump table) 대신 크기가 고정된 1차원 배열(`Array<String?>`) 기반의 직접 조회가 성능상 훨씬 유리합니다. 문자(char)의 정수형(`toInt()`)을 배열의 인덱스로 직접 활용하면, JIT 컴파일러가 효과적으로 최적화할 수 있어 분기 예측 비용 및 실행 시간을 크게 줄일 수 있습니다. 단, 자바 1.8 미만(또는 오래된 코틀린 버전)에서는 타입 추론 문제(Cannot use Nothing? as reified type parameter)를 피하기 위해 배열 초기화 시 명시적인 타입(`Array<String?>(128)`)을 제공해야 합니다. 또한, JvmField와 private object 패턴을 사용하여 자코코 커버리지 손실을 방지해야 합니다.
**Action:** 자주 호출되는 문자열 필터링/매핑 로직을 작성할 때는 O(1) 배열 조회를 우선 고려하고, 인덱스 바운드(bounds check) 처리에 유의하여 성능 저하 없이 최적화합니다.
