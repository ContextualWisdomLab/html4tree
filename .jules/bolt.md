...
## 2024-05-24 - Single-pass Directory Filtering
**Learning:** In Kotlin, separating filtering logic into multiple sequential `.forEach` iterations over a large list or array (like directory contents) introduces redundant traversal and condition-checking overhead. A single file might pass early inclusion criteria but still be unnecessarily processed by subsequent loops.
**Action:** Merge multiple iterations over the same collection into a single-pass loop with early returns (`return@forEach`) for exclusion conditions to minimize list traversal and short-circuit expensive string manipulations (like `toLowerCase` or path resolving).
