...
## 2026-09-12 - [escapeHtml 빠른 경로(Fast-path) 및 배열 조회 최적화]
**Learning:** `escapeHtml` 함수(파일 이름 및 aria 라벨 처리 등에서 빈번히 호출됨) 내에서의 `when` 문자 매칭 및 지속적인 `StringBuilder` 할당은 눈에 띄는 성능 오버헤드를 유발합니다. 배열 조회와 조기 종료가 가능한 빠른 경로(fast-path)를 도입하면, 이 핫 패스(hot path)에서 분기 예측 실패 및 조건부 점프를 방지하여 문자열의 HTML 이스케이프 성능을 2배 이상 향상시킬 수 있습니다.
**Action:** 긴밀한 루프에서 적용되는 문자 변환(예: HTML 인코딩)을 구현할 때는 일반적인 `when` 구문이나 반복적인 문자열 할당보다 사전 할당된 `StringBuilder` 사용, 조기 종료 검사, 그리고 O(1) 배열 매핑을 우선적으로 고려합니다.
