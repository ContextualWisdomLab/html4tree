...
## 2024-09-06 - HTML 이스케이프 성능 최적화 (배열 기반 룩업)
**학습:** Kotlin에서 `when` 조건문을 통한 문자 매핑은 핫 패스(hot path)에서 분기 예측 실패 및 점프 테이블 생성 등으로 성능 저하를 유발합니다.
**조치:** 예약된 문자의 아스키 코드 값을 인덱스로 사용하는 크기 128의 정적 `Array<String?>`을 직접 조회(`if (cInt < 128) array[cInt] else null`)하도록 변경하여 HTML 이스케이프 정규화의 성능을 크게 향상시켰습니다.
