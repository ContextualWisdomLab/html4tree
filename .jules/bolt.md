...
**학습:** Kotlin에서 `when` 조건문을 통한 문자 매핑(예: HTML 이스케이프)은 분기(jump) 예측과 여러 조건 검사로 인해 핫 패스에서 속도 저하를 일으킬 수 있습니다.
**조치:** `when` 조건 분기를 직접적인 배열 인덱스 기반 조회(예: `Array<String?>(128)`)로 교체하여 O(1) 시간에 빠르게 변환할 수 있도록 하였으며, `ArrayIndexOutOfBoundsException`을 피하기 위해 `cInt < 128` 검사를 추가했습니다.
