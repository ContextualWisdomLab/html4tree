...
## 2026-09-01 - BFS 큐 커스텀 LinkedList 할당 오버헤드 최적화
**Learning:** 커스텀 `LinkedList` 클래스는 항목을 큐에 삽입(push)할 때마다 내부 래퍼 객체(`Entry`)를 생성하고, 가져올(pull) 때마다 반환 객체 복사본을 만들어 과도한 메모리 할당과 가비지 컬렉션(GC)을 유발합니다. 이는 대규모 디렉토리를 순회할 때 심각한 성능 병목이 될 수 있습니다.
**Action:** 불필요한 중간 객체 할당을 방지하기 위해 큐나 스택으로 사용할 때는 항상 표준 라이브러리의 `java.util.ArrayDeque` 등을 활용합니다.
