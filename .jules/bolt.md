...
## 2026-08-11 - 중복된 escapeHtml 호출 최소화
**Learning:** 디렉토리 목록 렌더링 루프(hot path)에서 동일한 파일 이름에 대해 `escapeHtml()`이 여러 번(예: `ariaLabel` 설정 시 한 번, HTML 본문에 렌더링할 때 한 번) 호출되어 불필요한 문자열 탐색 및 할당이 발생합니다.
**Action:** 렌더링 루프 등 성능에 민감한 핫 패스에서는 `escapeHtml()`의 결과를 변수에 캐싱하고 이를 재사용하여 중복 연산과 추가적인 메모리 할당을 방지해야 합니다.
