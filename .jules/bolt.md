...
**학습:** `isDirectory`, `!it.isDirectory()`, `isSymbolicLink` 3개의 개별적인 파일 시스템 I/O 호출을 수행하면 성능 저하가 큽니다. 이를 단일 `Files.readAttributes` 호출로 변경하여 메타데이터를 한 번에 조회함으로써 I/O 오버헤드를 대폭 줄일 수 있음을 확인했습니다.
**조치:** 디렉토리 순회 시 파일의 여러 속성을 확인할 때는 개별적인 stat 호출보다 `Files.readAttributes`를 사용하여 필요한 모든 속성을 한 번에 가져오는 방식을 우선적으로 고려해야 합니다.
## 2026-08-04 - 디렉토리 처리 루프 내 정적 에셋 객체 추출
**Learning:** process_dir과 같이 반복적으로 호출되는 디렉토리 처리 루프 내부에 크기가 큰 상수 문자열(정적 CSS)과 결정론적인 계산(SHA-256 해시 생성 등)을 배치하면 불필요한 메모리 할당 및 CPU 오버헤드가 발생합니다. Kotlin에서는 이를 private object 및 const val/@JvmField를 사용하여 외부로 추출함으로써 커버리지 하락 없이 최적화할 수 있습니다.
**Action:** 반복적인 디렉토리/파일 순회 로직 내의 정적인 데이터와 불변의 연산 결과는 루프 바깥의 object 등에 선언하여 재사용합니다.
