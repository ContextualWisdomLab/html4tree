...
## 2024-10-24 - 루프 내 문자열 할당 지연 (toLowerCase)
**학습:** 디렉토리 항목을 반복하며 확장자 등을 검사할 때 루프 내에서 무조건 `toLowerCase()`를 호출하면 핫 패스에서 심각한 GC(Garbage Collection) 압력을 유발합니다.
**조치:** `isHiddenFile()`이나 `endsWith("~")`와 같이 저렴하게 평가할 수 있는 조건을 먼저 확인하고, 해당 조건을 통과하지 못한 경우에만 `toLowerCase()`를 호출하도록 지연 평가 구조(`if-else`)로 변경하여 불필요한 문자열 할당 비용을 최적화합니다.
