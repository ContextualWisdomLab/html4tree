...
## 2024-08-16 - HTML 이스케이프 문자 매핑 최적화
**학습:** `escapeHtml` 함수 내에서 매 문자마다 `when` 조건문을 사용하여 이스케이프 문자를 매핑하는 것은 불필요한 분기 처리를 유발합니다. O(1) 배열 기반의 룩업 테이블을 사용하면 이러한 분기 예측 실패 및 조건부 점프 오버헤드를 방지하여 성능을 최적화할 수 있습니다.
**조치:** `when` 조건 블록을 `Array<String?>` 룩업 테이블로 대체하여 `Char.toInt()`를 인덱스로 활용, 문자에 대한 O(1) 직접 매핑을 수행하도록 최적화했습니다.
