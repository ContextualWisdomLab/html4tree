...
**학습:** `process_dir`과 같은 재귀 함수 내부에 정의된 정적 데이터(예: `cssContent`) 및 비용이 많이 드는 암호화 연산(예: SHA-256)을 Kotlin의 파일 수준(top-level) 속성으로 추출하면, 디렉토리마다 동일한 작업이 반복적으로 평가되는 것을 방지하여 성능을 크게 향상시킬 수 있습니다.
**조치:** 함수 인자에 의존하지 않는 정적 데이터나 비싼 속성을 정의할 때는 이를 파일 수준으로 이동시킵니다. 100% 테스트 커버리지를 유지하려면 생성된 암묵적 getter에 대한 명시적 테스트 케이스를 추가해야 합니다.
