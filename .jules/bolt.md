...
## 2024-08-24 - 단일 StringBuilder를 사용한 문자열 결합 최적화
**Learning:** `process_dir`에서 `index_top`, `index_middle()`, `index_bottom`을 각각 생성하고 마지막에 `+` 연산자로 결합하면 불필요한 중간 문자열 객체가 생성되어 가비지 컬렉션(GC) 오버헤드가 발생합니다.
**Action:** 큰 문자열을 생성할 때는 중간 문자열 결합이나 추가적인 함수 할당(익명 함수 `val index_middle = fun():String`)을 피하고, 처음부터 단일 `StringBuilder` 인스턴스를 생성하여 모든 텍스트 청크를 해당 인스턴스에 직접 `append` 하도록 코드를 리팩토링합니다.
