...
## 2024-08-11 - 파일 목록 호출 캐싱으로 OS I/O 최적화
**학습:** `process_ignore_file` 함수에서 `.html4ignore` 파일의 패턴 일치 검사와 기본 민감 파일 이름 검사를 위해 `curr_dir.list()`를 여러 번 호출하면 디렉토리가 클 때 심각한 I/O 성능 저하가 발생합니다.
**조치:** 디렉토리 목록(`dirFilesNames ?: curr_dir.list()`)을 함수의 시작 부분에서 지역 변수로 한 번만 캐시(할당)하고 후속 작업에 재사용하여 불필요한 OS I/O 호출을 피해야 합니다.
