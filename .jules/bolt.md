...
## 2026-08-01 - 반복적인 디렉토리 처리 시 상수 및 해시 연산 외부 추출
**Learning:** process_dir과 같이 루프 성격의 함수 내부에 크기가 큰 문자열 상수나 SHA-256 해싱과 같은 결정론적인 연산이 포함되어 있으면 디렉토리를 순회할 때마다 불필요한 메모리 할당과 CPU 오버헤드가 발생한다.
**Action:** 불변하는 정적 자원(Static CSS 등)과 해시 계산을 private object 내부의 상수로 추출하여 한 번만 연산하도록 한다.
