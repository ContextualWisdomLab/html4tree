...
## 2024-09-22 - 첫 문자열 조회 시 박싱 오버헤드 최적화
**Learning:** Kotlin에서 문자열의 첫 문자를 확인하기 위해 `firstOrNull()`을 사용하면 반환값이 `Char?`로 박싱되면서 불필요한 객체 할당 오버헤드가 발생합니다. 핫 패스(hot path)에서 반복 호출될 경우 가비지 컬렉션(GC) 압력이 심해집니다.
**Action:** 시퀀스 구체화 및 박싱을 피하기 위해 `firstOrNull()` 대신 `isEmpty()` 체크와 직접 인덱스 접근(`this[0]`)을 조합하여 사용해야 합니다.
