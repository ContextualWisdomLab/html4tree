...
## 2026-08-11 - 중복된 디렉토리 목록 조회(I/O) 캐싱
**학습:** `process_ignore_file` 함수 내에서 `dirFilesNames ?: curr_dir.list()`를 여러 번 호출하면, `dirFilesNames`가 null일 때 불필요하게 `curr_dir.list()`가 중복 실행되어 파일 시스템 I/O 오버헤드가 발생합니다.
**조치:** `dirFilesNames ?: curr_dir.list()`의 결과를 지역 변수(`directory_files`)에 캐싱하여 여러 번의 파일 시스템 I/O 호출을 한 번으로 줄여 성능을 향상시킵니다.
