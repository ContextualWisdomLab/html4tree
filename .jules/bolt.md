...
## 2025-01-24 - LinkedList 최적화
**학습:** 커스텀으로 구현된 `LinkedList` 클래스는 디렉토리 순회 과정에서 노드 삽입 및 추출 시 불필요한 래퍼 객체 할당(`Entry`, `LinkedListEntry`)을 발생시킵니다.
**조치:** JDK의 `java.util.ArrayDeque`를 사용하여 래퍼 객체 할당 없이 효율적으로 큐를 관리하도록 리팩토링합니다. O(N) 삽입/삭제를 수행하는 커스텀 링크드리스트를 버리고 `ArrayDeque`로 교체하여 성능 향상을 이뤘습니다.
