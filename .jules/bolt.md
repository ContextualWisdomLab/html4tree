...
## 2024-08-11 - 루프 내 중복 문자열 변환 제거
**학습:** 반복문 내에서 동일한 변수에 대해 여러 번의 문자열 변환(예: `escapeHtml()`)을 수행하면 불필요한 연산과 객체 할당이 발생합니다. `process_dir` 내에서 파일명에 대한 이스케이프 처리가 두 번씩 발생하고 있었습니다.
**조치:** 반복문 내에서 무거운 문자열 변환 함수의 결과는 한 번만 계산하여 변수에 저장하고(`escapedFileName`) 이를 재사용하도록 수정하여 불필요한 중복 호출을 제거합니다.
