...
## 2026-08-11 - escapeHtml 배열 직접 조회 최적화
**학습:** Kotlin에서 HTML 이스케이프 처리를 위해 `Char.toInt()`를 변환하여 배열(예: `Array(128)`)에 접근하는 방식은 `when` 조건 분기 오버헤드를 줄여주며, 핫 패스에서 훨씬 빠릅니다.
**조치:** 성능 향상을 위해 `escapeHtml` 내의 `when(c)` 문을 배열 조회 `if (cInt < 128) Constants.htmlEscapeTable[cInt] else null`로 교체합니다. 100% 테스트 커버리지를 유지합니다.
