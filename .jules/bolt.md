...
## 2024-08-16 - 직접 배열 조회를 통한 HTML 이스케이프 성능 향상
**학습:** Kotlin에서 `when` 구문을 통한 문자 조건 비교는 핫 패스에서 비용이 발생합니다. 조건문을 직접 배열 기반 인덱스 조회로 대체하면 문자열 이스케이프 및 조건 평가 오버헤드가 현저히 줄어듭니다.
**조치:** `escapeHtml` 함수에서 문자를 이스케이프할 때 미리 계산된 `Array<String?>` 배열에 `c.toInt()`로 직접 접근하여 값을 가져옵니다.
