...
## 2026-07-25 - 정적 문자열 및 해시 계산 객체 분리를 통한 성능 최적화
**Learning:** 디렉토리 처리 루프 내부에서 변하지 않는 큰 문자열이나 결정론적 계산(예: SHA-256 해시)을 매번 수행하면 불필요한 메모리 할당과 CPU 오버헤드가 발생한다는 점을 발견함.
**Action:** 불변 상수 문자열 및 관련 해시 연산을 `private object` 로 추출하여 단 한 번만 평가하고 메모리에 재사용하도록 구현해야 함.
