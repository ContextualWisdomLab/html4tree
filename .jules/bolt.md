...
**학습:** 핫 패스 루프 내에서 예약된 바이트당 최대 3개의 문자열을 할당하는 `byte.toString(16).padStart(2, '0').toUpperCase()`는 상당한 GC 압력을 유발합니다. 이는 디렉토리 크롤러에서 대규모 문자열이나 수 편집 파일을 처리할 때 Kotlin에서 흔히 볼 수 있는 위험한 안티 패턴입니다.
...
**조치:** `crawl_directories`에서 `Array(files.size)`로 파일명 배열을 직접 생성합니다. 순서, null 처리, ignore 입력과 파일 시스템 호출 경계는 변경하지 않습니다.
...
## 2027-02-28 - 배열 기반 조회 매핑
**학습:** 핫 패스에서 `when` 조건문이나 점프 테이블 논리를 사용하면 조건 평가 오버헤드가 발생합니다. 모든 가능한 입력 범위가 한정되어 있을 때 배열 직접 조회를 사용하면 문자열 이스케이프처럼 자주 호출되는 함수에서 실행 속도를 크게 향상시킬 수 있습니다.
**조치:** `escapeHtml` 함수의 `when` 룩업을 직접 배열 룩업으로 대체했습니다.
