...
**학습:** 문자열 순회 중 각 문자에 대해 `when` 분기문을 사용해 대체 문자를 찾는 것은 핫 패스(예: HTML 이스케이프)에서 조건 평가 오버헤드를 일으킵니다.
**조치:** `when` 분기문 대신 미리 계산된 직접 배열 조회(Array based lookup)를 사용하여 문자 매핑 속도를 향상시켰습니다.
