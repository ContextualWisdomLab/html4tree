...
## 2024-08-13 - 파일 목록 이중 호출 및 람다 할당 방지
**학습:** `dirFilesNames ?: curr_dir.list()` 구문을 두 개의 개별 루프에서 각각 사용하면 `dirFilesNames`가 null일 때 비용이 큰 OS I/O 호출(`curr_dir.list()`)이 두 번 발생하며, `forEach`를 사용하면 핫 루프 내에서 불필요한 람다 객체가 할당됩니다.
**조치:** 디렉토리 탐색 함수의 맨 처음에 `curr_dir.list()` 결과를 단일 변수에 캐시하고, 람다 할당을 피하기 위해 `forEach` 대신 일반 `for` 루프와 `continue`를 사용합니다.
