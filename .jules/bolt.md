...
**Learning:** HTML 생성을 위해 루프 내에서 반복적으로 문자열을 추가할 때 StringBuilder를 사전 할당하지 않으면 O(N) 배열 재할당 오버헤드가 크게 발생합니다.
**Action:** 디렉토리 크기(항목 수)를 바탕으로 StringBuilder의 용량을 예상 항목 수와 예상 항목당 문자 수를 곱하여 사전 할당합니다.
