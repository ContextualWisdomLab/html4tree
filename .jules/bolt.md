...
## 2024-09-24 - HTML 이스케이프 문자 매핑 최적화
**학습:** Kotlin에서 HTML 이스케이프와 같이 핫 패스에서 문자를 매핑할 때, `when` 조건 분기 테이블을 사용하는 것보다 직접적인 배열 기반 조회를 사용하는 것이 훨씬 빠릅니다. 배열 조회를 사용하면 O(1) 시간 복잡도로 빠르게 문자를 치환할 수 있습니다.
**조치:** 128 미만의 ASCII 문자에 대한 이스케이프 맵을 `Array<String?>`으로 미리 정의하고, `when` 표현식 대신 배열 인덱싱을 통해 교체 문자열을 찾도록 최적화했습니다. `ArrayIndexOutOfBoundsException`을 방지하기 위해 배열 접근 전에 범위 검사를 수행합니다.
