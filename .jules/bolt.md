...
## 2024-05-18 - HTML 이스케이프 핫 패스에서 배열 기반 룩업 적용
**학습:** Kotlin에서 `when` 구문을 통한 문자 매핑은 핫 패스(hot path)에서 성능 저하를 일으킬 수 있습니다. 특히 HTML 이스케이프와 같이 빈번하게 호출되는 기능에서는 배열 기반 룩업을 사용하는 것이 조건문 평가 오버헤드를 피할 수 있어 훨씬 빠릅니다.
**조치:** `when` 조건 점프 테이블 대신, 미리 계산된 문자 매핑을 담은 배열(`Array<String?>(128)`)을 사용하여 문자 매핑 조회를 O(1)의 배열 인덱스 접근으로 최적화합니다.
