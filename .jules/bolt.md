...
## 2025-01-24 - 조건부 저렴한 문자열 검사 선행 (Short-Circuit)
**학습:** `toLowerCase()`와 같은 문자열 재할당 연산은 루프 내에서 큰 GC 오버헤드를 유발할 수 있습니다.
**조치:** 무시할 파일인지 확인할 때, `isHiddenFile()`이나 `endsWith("~")` 같은 비용이 적게 드는 검사를 먼저 수행하여 조건부 평가를 단축(short-circuit)시키고, 이러한 저렴한 조건에 맞지 않을 때만 `toLowerCase()`를 평가하여 불필요한 메모리 할당을 줄입니다.
