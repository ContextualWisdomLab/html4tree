...
## 2024-07-19 - [성능 최적화: 정적 변수 추출]
**Learning:** 반복적인 디렉토리 순회 시, 불필요한 문자열 할당 및 SHA-256 암호화 해시 연산이 각 디렉토리마다 발생하여 심각한 성능 저하를 초래할 수 있음을 확인했습니다.
**Action:** 불변인 정적 변수(CSS 내용, 해시 값)를 파일의 최상단(Top-level properties)으로 추출하여, 이 비싼 연산들이 프로세스 생명주기 동안 단 한 번만 실행되도록 수정합니다.
