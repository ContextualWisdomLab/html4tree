...
## 2026-08-24 - List 대신 Array 순회를 통한 이터레이터 오버헤드 제거
**Learning:** 빈번하게 호출되는 루프(예: 파일 확장자 검사) 내부에서 고정된 `List`에 대해 `.any {}` 또는 `forEach`를 호출하면 매번 이터레이터 객체가 할당되어 가비지 컬렉션(GC) 압력과 성능 저하를 유발합니다. 요소가 적을 때 `substring`을 이용한 O(1) `Set` 룩업을 시도하면 중간 문자열 생성 비용이 더 클 수 있습니다.
**Action:** 고정된 데이터 컬렉션을 순회할 때는 메모리 할당 없는 인덱스 기반 순회가 가능하도록 `.toTypedArray()`로 미리 변환된 `Array`를 사용하여 순회 오버헤드를 최적화해야 합니다.
