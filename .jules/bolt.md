...
**학습:** `isDirectory`, `!it.isDirectory()`, `isSymbolicLink` 3개의 개별적인 파일 시스템 I/O 호출을 수행하면 성능 저하가 큽니다. 이를 단일 `Files.readAttributes` 호출로 변경하여 메타데이터를 한 번에 조회함으로써 I/O 오버헤드를 대폭 줄일 수 있음을 확인했습니다.
**조치:** 디렉토리 순회 시 파일의 여러 속성을 확인할 때는 개별적인 stat 호출보다 `Files.readAttributes`를 사용하여 필요한 모든 속성을 한 번에 가져오는 방식을 우선적으로 고려해야 합니다.
## 2025-01-24 - process_dir의 정적 에셋 객체(Object) 추출
**Learning:** `html4tree/main.kt`의 `process_dir`과 같은 디렉토리 처리 루프에서, 정적 CSS 콘텐츠를 생성하고 SHA-256 해시를 매 반복마다 수행하면 대규모 디렉토리를 순회할 때 불필요한 문자열 할당 및 CPU 오버헤드가 발생하여 성능이 저하됩니다. 이를 `private object`를 활용하여 캡슐화하면 JVM 수명 주기당 한 번만 초기화되어 효율적입니다. 코틀린에서 컴파일러가 암시적으로 생성하는 getter로 인한 JaCoCo 커버리지 누락을 방지하기 위해 동적 해시에는 `@JvmField`를, 상수로 처리할 수 있는 문자열에는 `const val`을 사용하는 것이 필수적입니다.
**Action:** 재귀적이거나 빈번하게 호출되는 디렉토리 처리 함수 내부에서 대규모 상수 문자열 및 결정론적 계산을 수행할 때는 항상 이를 `private object` 싱글톤으로 추출하십시오. 코틀린의 암시적 getter로 인한 커버리지 감소를 방지하기 위해 `const val`이 불가능한 프로퍼티에는 `@JvmField` 어노테이션을 반드시 사용하십시오.
