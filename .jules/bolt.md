...
## 2024-08-23 - 배열 조회를 통한 이스케이프 문자 매핑 최적화
**학습:** 문자열에서 특정 문자를 찾아 치환할 때 (예: HTML 이스케이프 함수), `when` 조건문을 통한 점프 테이블(jump table) 방식보다 미리 캐싱된 배열에서 직접 O(1) 조회하는 것이 성능 오버헤드가 더 낮습니다.
**조치:** `escapeHtml` 핫 패스 내에서 문자를 이스케이프할 때 `when` 대신 ASCII 범위(`cInt < 128`)에 해당하는 크기의 배열(`HTML_REPLACEMENTS`)을 조회하여 할당 및 조건 분기 오버헤드를 최적화했습니다. 배열은 `Constants` 객체에 `@JvmField`와 함께 캐싱하여 중복 생성 비용을 제거하고 테스트 커버리지를 100% 유지했습니다.
