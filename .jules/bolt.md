...
**Learning:** `when (firstOrNull())`을 사용하면 핫 루프에서 문자를 확인할 때 객체 할당 오버헤드가 발생하며, HTML 문자 인코딩을 위해 `when` 분기문을 사용하는 것은 사전에 할당된 `Array` 룩업 테이블보다 성능이 떨어집니다.
**Action:** 핫 루프 내부에서 문자를 대체할 때 항상 Array를 활용한 O(1) 테이블 조회를 미리 할당하고, 빠른 불리언 평가를 위해 `firstOrNull()` 시퀀스 대신 길이를 명시적으로 검사하고 인덱스 0에 직접 접근합니다.
