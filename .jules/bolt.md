...
## 2024-09-18 - 문자열 첫 글자 검사 시 불필요한 객체 할당 오버헤드 최적화
**Learning:** Kotlin에서 문자열의 첫 글자를 확인할 때 `firstOrNull()`을 사용하면 핫 루프에서 문자를 `Char?`로 박싱하여 불필요한 객체 할당(overhead)이 발생합니다.
**Action:** `isEmpty()`와 직접 인덱스 접근(`this[0]`)을 조합하여 시퀀스 구체화를 방지하고 성능을 개선합니다.
