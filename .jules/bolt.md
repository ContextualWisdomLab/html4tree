...
## 2024-08-15 - 지연 평가를 통한 핫 루프 문자열 할당 최소화
**Learning:** 핫 루프 내에서 저렴하게 평가 가능한 조건(`isHiddenFile()`, `endsWith()`) 이전에 무거운 문자열 할당(`toLowerCase()`)을 수행하면 불필요한 가비지 컬렉션(GC) 부하가 발생합니다.
**Action:** 비용이 적게 드는 단락(Short-circuit) 검사를 먼저 평가하고, 해당 조건을 통과한 경우에만 메모리 할당이 발생하는 작업을 수행하도록 `if-else` 또는 지연 평가(lazy evaluation)를 활용하여 최적화해야 합니다.
