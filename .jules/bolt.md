...
## 2024-05-19 - [arrayOf를 사용한 확장자 검사 최적화]
**Learning:** Kotlin에서 List에 대해 `.any {}`를 직접 호출하면 매번 새로운 Iterator 객체가 할당됩니다. 정적인 List 대신 배열(`arrayOf()`)을 정의하고 `.any {}`를 호출하면 할당을 방지할 수 있습니다. `Array.any` 인라인 함수는 원시 인덱스 기반 루프로 컴파일되기 때문입니다. 또한 `listOf(...).toTypedArray()`보다 처음부터 `arrayOf(...)`를 사용하는 것이 불필요한 중간 리스트 생성을 막아 더 효율적입니다.
**Action:** 고차 함수(예: `.any {}`)를 사용하여 자주 순회하는 정적 컬렉션에는 `arrayOf()`를 사용하세요.
