...
## 2025-02-12 - 값싼 검사 먼저 수행하여 문자열 할당 지연 평가
**학습:** 반복문 내에서 `toLowerCase()`와 같이 문자열 객체를 새로 할당하는 무거운 연산을 매번 수행하면 GC 부하가 가중됩니다. 파일명 검사 시, `isHiddenFile()`나 `endsWith("~")` 같은 단순하고 저렴한 문자열 검사를 먼저 수행하도록 조건문을 분리하면 많은 경우 무거운 연산을 피할 수 있습니다.
**조치:** 핫 패스 루프 내에서는 비용이 적게 드는 조건(`isHiddenFile`, 문자열 길이 비교나 문자 검사 등)을 최상단에서 먼저 평가하고, 값비싼 문자열 변환이나 할당 연산은 이후로 지연(Deferred)시켜 불필요한 메모리 할당을 줄입니다.
