...
## 2026-08-11 - List.any() 호출로 인한 루프 내부 할당 오버헤드 제거
**학습:** Kotlin에서 `List`에 대해 `.any {}`를 직접 호출하면 호출 시마다 새로운 `Iterator` 객체가 할당되어, 파일 필터링과 같은 핫 패스 루프에서 GC 오버헤드를 증가시킵니다.
**조치:** 정적인 `List`를 `.toTypedArray()`를 사용해 Typed Array로 변환하여 사용하면, `Array.any` 인라인 함수가 호출되어 원시 인덱스 기반 루프로 컴파일되므로 Iterator 객체 할당을 완벽히 방지할 수 있습니다. 이를 `Constants.defaultSensitiveExtensionsArray` 필드로 추가하여 성능을 최적화했습니다.
