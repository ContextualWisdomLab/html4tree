...
## 2024-05-18 - [Kotlin HTML escaping 성능 최적화]
**학습:** `String.escapeHtml`은 핫 패스이며, 단순 문자 매칭을 위한 `when` 조건부 분기 테이블은 불필요한 분기 오버헤드를 발생시킵니다.
**조치:** 핫 패스의 분기문을 O(1) 배열 조회(`Constants.htmlEscapes`)로 대체하여 성능을 개선합니다. 배열 인덱스 초과를 방지하기 위해 경계 검사(`cInt < 128`)를 추가하고, 암시적 getter 오버헤드 없이 100% 테스트 커버리지를 유지하기 위해 조회 배열에 `@JvmField`를 사용합니다. 또한 레거시 호환성을 위해 `Char.code` 대신 `Char.toInt()`를 유지하거나 환경에 맞게 사용합니다.
