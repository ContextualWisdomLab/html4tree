...
## 2026-08-28 - List.any() Iterator 할당 오버헤드
**학습:** Kotlin에서 `List`에 대해 직접 `.any {}`를 호출하면 매 호출마다 새로운 `Iterator` 객체가 할당되어, 디렉토리 반복과 같은 핫 패스에서 GC 부하를 증가시킵니다.
**조치:** 정적인 `List`를 타입이 지정된 `Array` (예: `.toTypedArray()`)로 변환하고, 배열에 대해 `.any {}`를 사용하여 이 반복자 할당을 방지하면서 O(N) 최악의 경우 안전성을 유지합니다. API 호환성을 유지하기 위해 원래의 공용 `@JvmField` 속성의 타입을 변경하지 마십시오.
