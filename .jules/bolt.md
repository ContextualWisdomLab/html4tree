...
## 2024-05-30 - 지연 평가를 통한 GC 부하 완화 및 중복 I/O 제거
**Learning:** 핫 루프 내에서 저렴한 검사(`isHiddenFile()`, `endsWith()`) 전에 비싼 문자열 할당(`toLowerCase()`)을 수행하면 불필요한 가비지 컬렉션 부하가 발생합니다. 또한 동일 함수 내에서 `curr_dir.list()`와 같은 비싼 OS I/O 호출이 여러 조건 블록에 걸쳐 중복으로 발생하면 성능이 크게 저하될 수 있습니다.
**Action:** 조건문에서 짧은 회로(short-circuit) 평가를 활용해 문자열 할당을 지연시키고, I/O 호출 결과를 상위 스코프의 지역 변수에 캐싱하여 중복 호출을 제거합니다.
