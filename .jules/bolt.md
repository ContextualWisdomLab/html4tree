...
## 2024-08-28 - 디렉토리 목록 조회(OS I/O) 중복 호출에 따른 성능 저하 방지
**Learning:** `curr_dir.list()`와 같이 디렉토리 내용을 조회하는 함수가 한 함수 내에서 조건에 따라 여러 번 호출될 경우 매번 무거운 OS I/O 작업이 발생하여 애플리케이션의 성능 저하를 초래할 수 있음을 확인했습니다.
**Action:** 디렉토리 목록과 같은 OS I/O 작업의 결과는 함수의 최상단에서 한 번만 호출하여 지역 변수에 캐싱(`cachedDirFilesNames`)한 뒤, 이후의 조건문이나 반복문에서 해당 캐시 변수를 재사용하도록 코드를 작성해야 합니다.
