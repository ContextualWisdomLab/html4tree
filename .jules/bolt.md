...
## 2024-09-20 - 문자열 첫 문자 확인 시 불필요한 객체 할당(Boxing) 방지
**Learning:** Kotlin에서 `String.firstOrNull()`을 사용하면 `Char?`로 반환되면서 내부적으로 객체 할당(Boxing)이 발생합니다. 반복문이나 hot path 내부에서 사용될 경우 가비지 컬렉션(GC) 부하를 유발합니다.
**Action:** `firstOrNull()` 대신 `isEmpty()` 확인 후 `this[0]`을 직접 인덱스로 접근하여 불필요한 박싱 및 할당 오버헤드를 방지합니다.
