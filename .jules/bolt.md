...
## 2024-09-12 - 싼 조건문 평가 후 비싼 문자열 할당 지연 최적화
**학습:** 디렉토리 순회 루프 안에서 문자열 할당(`toLowerCase()`)과 같은 비싼 작업을 무조건 먼저 실행하면 불필요한 가비지 컬렉션(GC) 압력이 발생합니다.
**조치:** `isHiddenFile()`과 `endsWith("~")` 같은 싼 short-circuit 조건을 먼저 평가하고, 필요한 경우에만 비싼 문자열 객체 할당 연산을 수행하도록 조건문을 재배치해야 합니다.
