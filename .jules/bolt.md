...
## 2024-08-20 - 조건부 블록 전 디렉토리 목록 호이스팅
**Learning:** `process_ignore_file`과 같은 함수에서 `curr_dir.list()`를 여러 조건문 블록에서 반복적으로 호출하면 비용이 큰 OS I/O 작업이 중복으로 발생합니다. 단일 사용 위치 근처에서 캐싱하는 것은 조건부 실행을 포괄하지 못합니다.
**Action:** 동일한 디렉토리에 대해 파일 시스템 호출 횟수를 최소화하기 위해 항상 모든 조건문 경로 상단으로 디렉토리 목록(`curr_dir.list()`) 캐시 변수를 호이스팅하여 중복 평가를 제거해야 합니다.
