...
## 2026-08-30 - Array의 any 인라인 함수를 활용한 Iterator 할당 최적화
**Learning:** Kotlin에서 List와 같은 컬렉션에 대해 `.any {}`를 호출하면 내부적으로 매번 새로운 Iterator 객체가 할당되어 가비지 컬렉션(GC) 부하를 유발합니다. 이는 디렉토리 내의 수많은 파일을 반복적으로 검사하는 핫 패스에서 심각한 성능 저하를 초래할 수 있습니다.
**Action:** 자주 참조되는 정적 컬렉션(예: 제외할 파일 확장자 목록)은 배열(`Array`)로 변환(예: `toTypedArray()`)하여 사용해야 합니다. 배열의 `.any {}` 확장 함수는 인라인(inline)으로 처리되어 기본 인덱스 기반 루프로 컴파일되므로 Iterator 객체 할당을 방지할 수 있습니다.
