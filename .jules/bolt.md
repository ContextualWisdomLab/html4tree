...
## 2024-09-24 - 루프 내 템플릿 문자열 보간 오버헤드 최적화
**Learning:** 핫 패스 루프 내에서 복잡한 문자열 보간(string interpolation)과 중복된 변환(`escapeHtml()` 등)을 사용하면 불필요한 중간 객체가 생성되어 가비지 컬렉션 부하를 유발하고 성능이 저하됩니다.
**Action:** 루프 내에서 문자열을 구성할 때는 여러 번의 할당을 발생시키는 문자열 보간 대신, 이미 존재하는 `StringBuilder` 인스턴스에 직접 `.append()`를 체이닝하여 오버헤드를 줄입니다.
