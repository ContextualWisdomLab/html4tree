...
## 2026-08-11 - HTML 이스케이프 문자 매핑 최적화
**학습:** Kotlin에서 HTML 이스케이프와 같이 빈번하게 호출되는 hot path에서 `when` 조건문을 통한 문자 매핑은 각 문자마다 조건 평가 오버헤드를 발생시킵니다.
**조치:** `when` 조건 점프 테이블을 배열 기반 직접 조회(`Array<String?>(128)`)로 교체하여 O(1) 시간 복잡도의 빠른 매핑으로 최적화했습니다.
