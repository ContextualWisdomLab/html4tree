...
## 2024-05-18 - [디렉토리 탐색 시 컬렉션 Iterator 할당 제거]
**Learning:** `Constants.defaultSensitiveExtensions.any { ... }`와 같이 `List`에 대해 `.any {}`를 직접 호출하면 반복마다 새로운 `Iterator` 객체가 생성되어 가비지 컬렉션(GC) 압력이 증가합니다. 이 코드는 디렉토리 내의 모든 파일에 대해 평가되므로 성능 병목이 됩니다.
**Action:** `List`를 미리 복사한 `TypedArray` (`.toTypedArray()`)를 만들어 사용하고, 배열에서 `.any {}`를 호출하게 하여 컴파일러가 원시 인덱스 기반 루프로 최적화할 수 있도록 합니다. 이렇게 하면 추가적인 객체 할당을 방지하면서도 코드 가독성을 유지할 수 있습니다.
