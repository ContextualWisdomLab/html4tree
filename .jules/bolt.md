...
## 2026-10-18 - 커스텀 LinkedList 래퍼 객체 할당 오버헤드
**Learning:** 커스텀 `LinkedList` 구현체는 큐 연산(push/pull) 시마다 래퍼 객체(`Entry`)를 반복적으로 생성해야 하므로, 디렉터리 순회(BFS) 과정의 핫 패스에서 심각한 가비지 컬렉션(GC) 오버헤드를 유발합니다.
**Action:** BFS 순회 구현 시 불필요한 객체 생성을 방지하기 위해 커스텀 큐 자료구조를 버리고 표준 라이브러리인 `java.util.ArrayDeque`를 사용합니다.
