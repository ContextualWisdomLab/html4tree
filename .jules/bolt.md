...
## 2024-08-14 - HTML 이스케이프 문자 매핑 최적화
**학습:** Kotlin에서 HTML 이스케이프와 같은 핫 패스 루프 내 문자를 매핑할 때, `when` 조건부 점프 테이블을 사용하는 것보다 직접적인 배열 기반 조회(예: `Array<String?>(128)`)로 교체하면 문자당 조건 평가 오버헤드를 방지하여 성능이 크게 향상됩니다.
**조치:** `escapeHtml` 함수의 `when` 조건문을 크기가 128인 `Array`를 미리 초기화하고 `if (cInt < 128) ESCAPE_MAP[cInt] else null`을 통해 조회하도록 변경했습니다. 최상위 private 속성의 경우 100% JaCoCo 커버리지를 유지하기 위해 리플렉션 테스트를 추가해야 합니다.
