...
**조치:** `crawl_directories`에서 `Array(files.size)`로 파일명 배열을 직접 생성합니다. 순서, null 처리, ignore 입력과 파일 시스템 호출 정답은 변경하지 않습니다.
...
## 2026-09-03 - List의 any 확장 함수로 인한 Iterator 할당 최적화
**Learning:** Kotlin에서 핫 패스 루프 내에서 정적인 List에 .any {}를 호출하면 매 호출마다 새로운 Iterator 객체가 할당되어 약간의 오버헤드를 유발합니다.
**Action:** 자주 호출되는 반복문 안에서 컬렉션의 .any {}를 사용해야 할 경우, List를 .toTypedArray()로 변환한 Array에 대해 호출하여 인덱스 기반 루프로 컴파일되도록 함으로써 불필요한 객체 할당을 줄입니다. (JMH 측정 결과 List(0.599 ops/ms, 227 B/op) 대비 Array(0.664 ops/ms, 142 B/op)로 Throughput과 Allocation 모두 개선됨을 확인했습니다.)
