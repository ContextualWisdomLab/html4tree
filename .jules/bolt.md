...
## 2024-09-10 - 값비싼 문자열 할당(toLowerCase)을 조기 종료 조건 이후로 지연
**Learning:** 핫 패스(hot path) 파일 반복 루프에서 모든 파일에 대해 `toLowerCase()`를 무조건 호출하면 불필요한 문자열 객체 할당과 GC 압력이 크게 증가합니다. 특히 `isHiddenFile()`나 `endsWith("~")`와 같은 빠르고 저렴한 조건 검사로 조기 종료가 가능한 경우, 값비싼 변환 연산을 조건문 이후로 미루는 것이 효율적입니다.
**Action:** 조건문에서 빠른 평가가 가능한 저렴한 연산(`isHiddenFile`, `endsWith`)을 먼저 배치하고, 값비싼 객체 할당 및 변환 연산(e.g., `toLowerCase()`)은 단축 평가(short-circuit) 이후 시점까지 지연(defer)시키도록 최적화합니다.
