...
## 2024-05-20 - 지연 평가를 통한 GC 오버헤드 감소
**Learning:** 반복문 내에서 `toLowerCase()`와 같은 문자열 할당을 무조건적으로 수행하면 불필요한 가비지 컬렉션(GC) 부하가 발생합니다.
**Action:** `isHiddenFile()`이나 `endsWith()` 같은 비용이 적고 단축 평가(short-circuit) 가능한 속성 검사를 먼저 수행하여 무거운 작업의 빈도를 줄입니다.
