...
## 2024-08-17 - 디렉토리 목록 I/O 중복 호출 캐싱
**Learning:** `process_ignore_file` 내부의 여러 블록에서 `curr_dir.list()`가 중복 평가되어 불필요한 OS 레벨의 파일 시스템 I/O 오버헤드를 유발함을 발견했습니다.
**Action:** 함수 최상단에 `val cachedDirList = dirFilesNames ?: curr_dir.list()`를 선언하여 한 번만 평가되도록 캐싱하고, 함수 내의 모든 조건부 블록에서 이를 재사용하여 반복적인 I/O를 방지합니다.
