...
## $(date +%Y-%m-%d) - ArrayDeque 도입을 통한 BFS 성능 최적화
**Learning:** 기존 `LinkedList` 구현은 큐 조작 시마다 래퍼 객체를 생성하여 디렉토리 크롤링(BFS) 과정에서 높은 가비지 컬렉션(GC) 비용을 유발했습니다. 이를 해결하기 위해 `java.util.ArrayDeque`를 내부적으로 사용하면 객체 할당을 극적으로 줄일 수 있습니다. 하지만, 낡은 코드베이스의 공개 API(`first`, `last`)를 하위 호환성 및 테스트 통과 목적으로 억지로 유지하는 것은 O(N) 래퍼 객체 생성이나 상태 불일치(Null 포인터 회귀)를 야기할 수 있으므로, 해당 속성을 완전히 제거하고 의존하는 테스트도 함께 정리하는 것이 가장 안전한 성능 최적화 패턴입니다.
**Action:** 내부 구현을 `ArrayDeque`로 교체할 때 불필요해진 구형 속성(`first`, `last` 등)을 유지하려 하지 말고 완전히 제거하여 상태 불일치 문제를 예방하십시오.
