...
## 2024-08-19 - 문자 매핑을 위한 직접 배열 조회
**학습:** Kotlin에서 `when` 조건부 점프 테이블을 사용하는 것은 핫 패스(예: HTML 이스케이프)에서 문자 매핑을 수행할 때 조건 평가 오버헤드를 발생시킵니다.
**조치:** `when` 문 대신 크기가 128인 배열 기반 조회 테이블(Array<String?>)을 만들어 직접 인덱스로 접근하도록 하여 조건 평가 오버헤드를 줄입니다.
