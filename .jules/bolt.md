...
## 2026-08-24 - O(N) 확장자 검색을 O(1) Set 룩업으로 최적화
**Learning:** 파일 목록 처리 시 다수의 확장자를 `.any { string.endsWith(...) }`로 선형 탐색하면 확장자 개수와 파일 개수에 비례하여 O(N*M) 성능 오버헤드가 발생합니다.
**Action:** 긴 리스트에 대해 문자열 접미사를 확인할 때는, 마지막 점(.)을 기준으로 확장자를 추출(`substring`)한 후 `Set`을 사용해 O(1) 룩업을 수행하여 성능을 최적화해야 합니다.
