...
## 2024-08-17 - ArrayDeque 도입으로 성능 개선
**Learning:** 커스텀 LinkedList(현재 `LinkedList` 클래스 및 `Entry` 데이터 클래스)는 각 요소 추가 시마다 새로운 객체를 할당해야 하므로 GC 오버헤드와 힙 메모리 할당 비용이 큽니다. 반면 Java의 `ArrayDeque`를 사용하면 객체 재사용과 캐시 로컬리티가 향상되어 수백만 건의 파일 탐색 작업에서 실행 시간이 현저히 줄어듭니다.
**Action:** 단순한 순차적 탐색 큐가 필요할 때는 수동으로 구현한 LinkedList보다 `java.util.ArrayDeque`를 도입하여 성능을 최적화할 수 있습니다.
