...
## 2024-08-15 - escapeHtml Fast Path Optimization
**Learning:** `escapeHtml()` 함수에서 이스케이프가 전혀 필요 없는 문자열에 대해서도 매번 반복문을 돌면서 조건 검사를 수행하고 `null` 할당을 지연하더라도 객체 참조와 비교 연산 등 불필요한 오버헤드가 발생합니다.
**Action:** `escapeHtml`처럼 빈번하게 호출되지만 대부분 변경이 없는 함수는 먼저 짧은 루프로 이스케이프 필요 여부(fast-path)를 검사하여 불필요한 객체 할당과 로직을 생략하고 원본 문자열을 즉시 반환(조기 종료)하는 것이 성능에 유리합니다.
