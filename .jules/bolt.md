...
## 2024-11-20 - Avoid List.any allocations
**Learning:** `List`에 대해 `.any {}`와 같은 인라인 함수를 빈번하게 호출하면 매번 `Iterator` 객체가 할당됩니다.
**Action:** 자주 검사되는 정적 설정 컬렉션에는 `List` 대신 `Array`를 사용하여 인라인 함수 호출 시 불필요한 객체 할당을 방지해야 합니다.
