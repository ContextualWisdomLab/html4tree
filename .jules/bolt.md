...
## 2026-08-31 - List의 any 확장 함수로 인한 Iterator 할당 최적화
**Learning:** Kotlin에서 핫 패스 루프 내에서 정적인 List에 .any {}를 호출하면 매 호출마다 새로운 Iterator 객체가 할당되어 상당한 GC 오버헤드를 유발합니다.
**Action:** 자주 호출되는 반복문 안에서 컬렉션의 .any {}를 사용해야 할 경우, List를 .toTypedArray()로 변환한 Array에 대해 호출하여 인덱스 기반 루프로 컴파일되도록 함으로써 불필요한 객체 할당을 방지합니다.
