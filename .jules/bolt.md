...
## 2026-08-20 - 반복 호출되는 빈 배열 할당 최적화
**Learning:** 함수 내부에서 `emptyArray()`를 반복적으로 호출하면 매번 새로운 빈 배열 인스턴스를 할당하여 메모리 오버헤드를 유발할 수 있습니다.
**Action:** 자주 호출되는 함수에서 폴백 값으로 사용되는 빈 배열 등은 `emptyArray()` 호출 대신 최상위 레벨의 빈 배열 상수(예: `val EMPTY_ARRAY = emptyArray<File>()`)를 정의하고 재사용하여 불필요한 인스턴스 생성을 방지해야 합니다.
