...
## 2024-09-14 - 문자열 변환 지연 평가
**Learning:** 반복문 내에서 `toLowerCase()`와 같은 비용이 큰 문자열 할당 작업을 수행하기 전에, 가벼운 단락 평가(예: `isHiddenFile()`, `endsWith()`)를 먼저 수행하면 불필요한 GC(가비지 컬렉션) 압박을 크게 줄일 수 있습니다.
**Action:** 향후 문자열 변환이나 비용이 큰 연산은 반드시 가벼운 조건문 뒤로 지연 평가(lazy evaluation)하여 성능을 최적화해야 합니다.
