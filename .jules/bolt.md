...
## 2026-08-12 - HTML 이스케이프 문자 매핑 최적화
**학습:** Kotlin에서 HTML 이스케이프와 같이 문자를 순회하는 핫 패스 루프 내에서 `when` 조건문 점프 테이블을 사용하면 각 문자마다 조건 평가 오버헤드가 발생합니다.
**조치:** `when` 조건문을 직접 인덱싱이 가능한 배열 기반 룩업(`Array<String?>(128)`)으로 교체하여 브랜치 예측 실패를 줄이고 O(1) 시간 복잡도로 매핑 속도를 향상시켰습니다.
