...
**학습:** `isDirectory`, `!it.isDirectory()`, `isSymbolicLink` 3개의 개별적인 파일 시스템 I/O 호출을 수행하면 성능 저하가 큽니다. 이를 단일 `Files.readAttributes` 호출로 변경하여 메타데이터를 한 번에 조회함으로써 I/O 오버헤드를 대폭 줄일 수 있음을 확인했습니다.
**조치:** 디렉토리 순회 시 파일의 여러 속성을 확인할 때는 개별적인 stat 호출보다 `Files.readAttributes`를 사용하여 필요한 모든 속성을 한 번에 가져오는 방식을 우선적으로 고려해야 합니다.
## 2026-07-31 - 불변 문자열 및 해시 연산 오버헤드
**학습:** `process_dir`와 같이 빈번하게 호출되는 함수 내에 고정된 문자열과 비용이 큰 연산(예: SHA-256 해싱)을 두면, 디렉토리를 처리할 때마다 불필요한 객체 할당과 연산 오버헤드가 발생합니다.
**조치:** 불변 정적 문자열(`cssContent`, `css`, `index_bottom`)과 연산(`styleHash`)을 파일 최상단 `private val` 상수로 추출(hoist)하여 중복 할당 및 연산을 방지합니다. `private val`을 사용하여 불필요한 getter 생성을 막아 테스트 커버리지를 보존합니다.
