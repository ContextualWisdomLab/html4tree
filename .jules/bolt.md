...
## 2024-08-25 - HTML 이스케이프 핫패스 내 조건문 분기 최적화
**학습:** Kotlin에서 `when` 구문을 통한 문자 조건 비교는 특히 문자열을 구성하는 각 문자에 대해 루프 내에서 수행될 때 불필요한 분기 처리를 발생시킵니다.
**조치:** 핫 패스 (Hot Path)에서의 문자 매핑은 분기문을 O(1) 배열 조회를 통해 (예: `Array<String?>(128)`) 교체함으로써 가비지 컬렉션(GC) 및 명령어 사이클 오버헤드 최적화를 이룰 수 있습니다. 배열 조회 전 경계 확인 (`cInt < 128`) 을 통해 안전성 역시 유지합니다.
