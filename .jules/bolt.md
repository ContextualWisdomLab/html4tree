...
## 2024-06-03 - 지연 평가를 통한 GC 압력 감소
**Learning:** 파일 목록 루프에서 `.toLowerCase()` 같은 비용이 큰 문자열 할당 연산을 미리 수행하면, 조건문에서 조기 종료될 항목들에 대해서도 불필요한 메모리 할당과 GC 압력이 발생합니다.
**Action:** 조건문 내에서 값이 변하지 않고 검사가 빠르고 간단한 속성(예: `isHiddenFile()`, `endsWith()`)을 먼저 평가한 후, 이를 통과한 항목에 한해서만 고비용 연산(예: `toLowerCase()`)을 수행하도록 평가 순서를 재배치합니다.
