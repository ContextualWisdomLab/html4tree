...
## 2026-08-11 - 핫 패스 내 값비싼 문자열 할당 지연 평가
**학습:** 디렉토리 순회 루프처럼 반복 횟수가 많은 핫 패스에서 `toLowerCase()`와 같은 문자열 할당 연산을 무조건 수행하면, 단순한 조건(`isHiddenFile()`, `endsWith()`)으로 일찍 제외될 수 있는 항목들에 대해서도 불필요한 가비지 컬렉션(GC) 부하가 발생합니다.
**조치:** 새로운 문자열을 할당하는 값비싼 연산은 가장 저렴하고 빠르게 확인할 수 있는 조건(Short-circuit properties)을 먼저 평가한 이후, 실제로 필요할 때만 지연해서 수행하도록 조건문을 재배치해야 합니다.
