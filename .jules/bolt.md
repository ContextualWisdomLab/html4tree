...
## 2024-09-26 - 확장자 필터링 O(1) Set 조회로 최적화
**학습:** 긴 확장자 목록을 검사할 때 디렉토리의 모든 파일에 대해 `List.any { endsWith(...) }`를 호출하면 선형 탐색(O(N)) 및 문자열 연산으로 인해 심각한 CPU 오버헤드와 가비지 컬렉션 부하가 발생합니다. 특히 숨김 파일이나 임시 파일과 같은 가벼운 조건보다 늦게 평가되거나, 매번 `toLowerCase()`를 호출하면 낭비가 큽니다.
**조치:** 확장자 검사 시 파일 이름에서 `lastIndexOf('.')`를 이용해 확장자 부분만 추출한 후, 미리 만들어둔 $O(1)$ 조회용 `Set`을 활용합니다. 또한 저비용의 조건(`isHiddenFile()`, `endsWith('~')`)을 먼저 평가하여 불필요한 객체 할당(Short-circuit)을 방지합니다.
