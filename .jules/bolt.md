...
## 2024-05-20 - 핫루프 내 비용이 큰 연산 지연
**학습:** `toLowerCase()`와 같은 문자열 할당 연산은 비용이 크며, 이를 핫루프 내에서 무조건 실행하면 불필요한 가비지 컬렉션(GC) 압력이 발생합니다.
**조치:** `isHiddenFile()`이나 `endsWith()` 같이 저렴하고 단락(short-circuiting) 가능한 조건들을 먼저 평가한 후에 비용이 큰 연산을 지연 실행(lazy evaluation)하여 불필요한 할당을 방지해야 합니다.
