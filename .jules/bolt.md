...
## 2024-05-24 - [Kotlin 문자열 이스케이프 성능 최적화]
**Learning:** Kotlin에서 문자열 처리를 할 때 `when` 절을 이용한 분기 처리보다, 직접 배열(Array)를 통한 조회가 훨씬 빠릅니다. 특히 HTML 이스케이프와 같이 반복적으로 자주 호출되는 핫 패스(hot path)에서는 이러한 조건 분기를 제거하고 배열 조회를 사용하는 것이 성능 향상에 큰 도움이 됩니다. 배열 초기화 시 `Nothing?` 타입 추론 문제를 피하기 위해 `Array<String?>(128)`과 같이 명시적으로 타입을 지정해야 합니다.
**Action:** 자주 발생하는 문자 치환(예: HTML 인코딩 등)의 경우 `when`이나 `if-else` 대신 미리 정의된 크기의 배열을 이용해 O(1) 복잡도로 직접 접근하여 처리하도록 합니다.
