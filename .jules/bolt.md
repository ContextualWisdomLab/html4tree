...
## 2024-05-15 - 문자열 변환 지연
**Learning:** 디렉토리 순회와 같이 자주 호출되는 루프에서 무거운 문자열 변환(예: `toLowerCase`)을 반복적으로 수행하면 불필요한 가비지 컬렉션 압력이 발생하여 성능 저하의 원인이 될 수 있음을 확인했습니다.
**Action:** 비용이 저렴하고 즉시 판단 가능한 조건(예: `isHiddenFile()`, 문자열 접미사 검사)을 먼저 평가하여 빠른 반환(Short-circuiting)을 유도하고, 불가피한 경우에만 문자열 변환 등 무거운 객체 할당 연산을 수행하도록 평가 순서를 재조정해야 합니다.
