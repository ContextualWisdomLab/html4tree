...
## 2024-05-18 - Caching directory list to reduce duplicate I/O OS calls
**Learning:** Repeated calls to `curr_dir.list()` inside identical context paths without caching leads to redundant OS I/O overhead. This is heavily exposed when conditionally branching into blocks that separately trigger the listing.
**Action:** Always hoist invariant directory enumerations out of execution loops or multiple code branches to a local immutable cached variable, eliminating redundant allocations and expensive file system OS calls.
