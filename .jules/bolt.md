...
## 2024-09-07 - StringBuilder 용량 사전 할당 최적화
**학습:** `process_dir` 내에서 디렉토리의 파일 목록을 기반으로 HTML을 렌더링할 때, 크기가 큰 디렉토리의 경우 `StringBuilder`가 반복적으로 크기를 늘리며 불필요한 배열 복사와 메모리 할당(O(N) 리얼로케이션)을 발생시킵니다.
**조치:** `StringBuilder` 생성 시 디렉토리 파일 수(각 항목당 약 256바이트 예상)를 기반으로 초기 용량(capacity)을 사전에 계산하여 전달(예: `filesList.size * 256`)함으로써 반복적인 크기 조정(resizing)을 방지합니다.
