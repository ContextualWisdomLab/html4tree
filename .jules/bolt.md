...
**학습:** `isDirectory`, `!it.isDirectory()`, `isSymbolicLink` 3개의 개별적인 파일 시스템 I/O 호출을 수행하면 성능 저하가 큽니다. 이를 단일 `Files.readAttributes` 호출로 변경하여 메타데이터를 한 번에 조회함으로써 I/O 오버헤드를 대폭 줄일 수 있음을 확인했습니다.
**조치:** 디렉토리 순회 시 파일의 여러 속성을 확인할 때는 개별적인 stat 호출보다 `Files.readAttributes`를 사용하여 필요한 모든 속성을 한 번에 가져오는 방식을 우선적으로 고려해야 합니다.
## 2025-01-24 - 불필요한 객체 및 문자열 재할당 (Top-level Properties 최적화)
**학습:** Kotlin에서 `process_dir` 등 특정 디렉토리를 처리하는 반복적인 함수 안쪽에 큰 크기의 정적 문자열(`cssContent`)이나 많은 비용이 드는 암호화 해시 계산(`styleHash`)을 위치시키면, 매 디렉토리를 방문할 때마다 메모리 할당 및 가비지 컬렉션(GC) 압력이 발생합니다.
**조치:** 디렉토리마다 고정되어 변경되지 않는 문자열이나 객체는 파일 최상단 프로퍼티(`top-level property`)로 추출하여, 프로그램이 실행될 때 단 한 번만 계산 및 할당되도록 최적화합니다. 이는 디렉토리 트리를 순회할 때 심각한 성능 병목을 예방합니다.
