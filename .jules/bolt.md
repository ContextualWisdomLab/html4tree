...
## 2026-07-25 - 자주 호출되는 함수에서 불변 정적 문자열 및 무거운 연산 호이스팅
**Learning:** 자주 호출되는 함수(예: `process_dir`) 내에서 불변 정적 문자열과 비용이 많이 드는 연산(예: SHA-256 해싱)을 매번 수행하면 불필요한 할당 및 처리 오버헤드가 발생합니다.
**Action:** 이러한 불변 상수와 무거운 연산을 최상위 `private val` 상수로 호이스팅(hoisting)하여 중복 할당과 연산 오버헤드를 방지합니다. Kotlin에서 `private val`로 선언하면 public getter 생성을 방지하여 Jacoco 커버리지 지표도 보존할 수 있습니다.
