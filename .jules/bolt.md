...
## 2026-08-11 - HTML Escape 배열 매핑 최적화
**학습:** Kotlin에서 `when` 구문을 사용한 문자 기반 분기 처리(조건부 점프 테이블)는 디렉터리 내 수많은 파일과 큰 문자열이 처리되는 hot path(예: HTML 이스케이프)에서 성능 부하를 일으킵니다.
**조치:** `when` 조건문 점프 테이블 대신, 미리 계산된 O(1) 시간 복잡도의 인덱스 접근이 가능한 `Array<String?>(128)` 조회를 사용하여 문자 변환 시 불필요한 분기 평가를 줄였습니다.
