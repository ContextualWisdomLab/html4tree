...
## 2024-05-24 - [지연 평가를 통한 GC 부하 감소]
**Learning:** 루프 내에서 무거운 연산(예: `toLowerCase()`를 통한 문자열 할당)을 수행하기 전에, 가벼운 단락 평가(short-circuit) 조건(예: `isHiddenFile()`, `endsWith()`)을 먼저 검사하면 불필요한 객체 생성을 막고 GC 부하를 크게 줄일 수 있습니다.
**Action:** 여러 조건이 결합된 검사에서 리소스 소모가 많은 연산은 항상 가벼운 조건들이 실패한 이후에만 실행되도록(Lazy evaluation) 지연시켜야 합니다.
