...
## 2025-02-14 - 불필요한 문자열 할당(toLowerCase) 지연 최적화
**학습:** 디렉토리 파일을 순회하며 무시할 파일인지 확인할 때, 모든 파일에 대해 무조건 `toLowerCase()`를 호출하면 불필요한 String 객체 할당과 GC 부하가 발생합니다. 대부분의 파일은 히든 파일이거나 임시 파일(`~`)이 아니므로, 먼저 저렴한 boolean 검사로 걸러내야 합니다.
**조치:** `isHiddenFile()` 및 `endsWith("~")`와 같이 저렴한 검사를 먼저 수행하도록 조건문을 단축(short-circuit)하고, 해당 조건을 통과한 파일에 대해서만 `toLowerCase()`를 호출하도록 최적화하여 O(N) 문자열 할당 오버헤드를 줄였습니다.
