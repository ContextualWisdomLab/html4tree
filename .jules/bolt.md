...
## 2026-09-24 - 확장자 목록에 배열을 사용한 Iterator 할당 회피
**Learning:** Kotlin에서 요소 컬렉션에 대해 `.any {}` 등 인라인 람다를 호출할 때, 컬렉션이 `List`이면 파일마다 새로운 `Iterator` 인스턴스가 생성되어 가비지 컬렉션(GC) 부하를 유발합니다. 반면 `Array`에 대해 `.any {}`를 호출하면 내부적으로 Iterator 할당 없이 인덱스 기반 루프로 최적화됩니다.
**Action:** 디렉터리 내 수많은 파일들을 순회하면서 고정된 요소 집합에 대해 검사하는 경우, `List` 대신 `Array`를 선언하여 불필요한 Iterator 할당을 회피합니다.
