...
## 2026-07-30 - 루프 내 정적 문자열 및 해시 계산 추출
**Learning:** process_dir과 같이 루프 내에서 반복적으로 호출되는 함수에서, 큰 정적 문자열이나 SHA-256 해싱과 같은 계산을 매번 할당하고 실행하는 것은 메모리와 CPU 자원을 낭비합니다. 특히 디렉토리가 많은 환경에서는 이 오버헤드가 누적됩니다.
**Action:** 이런 정적인 데이터와 결정론적 계산은 private object로 추출하여 프로그램 실행 시 한 번만 초기화되도록 하여 성능을 최적화합니다.
