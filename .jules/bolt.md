...
## 2024-09-21 - firstOrNull 객체 할당 오버헤드
**Learning:** Kotlin에서 문자열의 첫 글자를 확인할 때 `firstOrNull()`을 사용하면 핫 루프에서 boxing(`Char?`)으로 인한 객체 할당 오버헤드가 발생합니다.
**Action:** 성능에 민감한 루프에서는 문자열의 첫 문자를 검사할 때 시퀀스 구체화(sequence materialization) 및 박싱을 피하기 위해 `isEmpty()`와 직접 인덱스 접근(`this[0]`)을 조합하여 사용합니다.
