...
## 2024-05-20 - HTML 이스케이프 문자열 매핑 최적화
**Learning:** `when` 조건문을 사용하여 문자를 매핑하면 조건 분기(jump) 오버헤드가 발생합니다. HTML 렌더링 시 빈번하게 호출되는 `escapeHtml`과 같은 핫 패스에서는 성능 저하의 원인이 될 수 있습니다.
**Action:** `when` 조건문을 크기가 128인 배열 기반의 직접 조회(Direct Array Lookup)로 교체하여 분기 오버헤드를 줄이고 빠르게 치환 문자를 찾도록 최적화했습니다.
