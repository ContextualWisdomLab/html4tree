...
## 2026-08-27 - List의 any 호출 시 Iterator 할당 오버헤드 최적화
**Learning:** Kotlin에서 정적 List에 대해 직접 .any {}를 호출하면 호출될 때마다 새로운 Iterator 객체가 할당되어 핫 패스에서 불필요한 GC 오버헤드를 유발합니다. Array로 변환하여 사용하면 이러한 Iterator 할당을 피할 수 있습니다.
**Action:** 고정된 값 목록을 조회할 때 .any {} 등을 반복적으로 사용해야 한다면, List 대신 arrayOf()를 사용하여 불필요한 메모리 할당을 줄입니다.
