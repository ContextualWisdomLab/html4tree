...
## 2026-09-08 - 최적화 근거 부재에 따른 거절 (학습)
**학습:** 128-slot 배열을 사용한 핫 패스 최적화 제안이 정식 측정(JMH/release-profile 벤치마크, GC 프로파일링, 대표적 부하 테스트) 부족으로 반려되었습니다. `when`과 배열 조회 모두 O(1)이지만, 배열 접근은 인덱싱 및 변환 오버헤드와 사전 할당 비용이 추가되므로 벤치마크 결과 없이 최적화라고 속단할 수 없습니다.
**조치:** 앞으로 성능 개선 PR을 작성할 때는 100% 테스트 커버리지뿐만 아니라 구체적인 JMH/릴리즈 벤치마크 수치 및 메모리/GC 프로파일 지표를 확보하고 포함해야 합니다.
