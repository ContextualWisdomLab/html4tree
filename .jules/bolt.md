...
## 2024-05-18 - 문자열 할당의 지연 평가를 통한 GC 부하 감소
**Learning:** 핫 패스 루프에서 `toLowerCase()`와 같은 비싼 문자열 할당 연산을 가장 먼저 수행하면, 이후의 간단한 조건(`isHiddenFile()`, `endsWith()`)으로 인해 버려지는 객체들로 인해 불필요한 가비지 컬렉션(GC) 압력이 발생합니다.
**Action:** 빠른 반환이 가능한 저렴한 조건들을 먼저 평가하여, 실질적으로 변환이 필요한 경우에만 비싼 문자열 할당이 일어나도록 조건문을 재배치해야 합니다.
