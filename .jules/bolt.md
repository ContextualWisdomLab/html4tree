...
## 2026-08-15 - 불필요한 반복 계산 제거
**Learning:** HTML 인덱스 생성 시 동일한 파일명에 대해 `escapeHtml()` 및 `urlEncodePath()` 같은 비용이 큰 변환 함수가 여러 번 호출되는 비효율성이 발견되었습니다. 특히 디렉토리 내 파일 수가 많을 때 루프 내에서의 이러한 중복 할당 및 계산은 눈에 띄는 오버헤드를 발생시킵니다.
**Action:** 루프 내에서 비용이 많이 드는 함수의 결과를 지역 변수로 캐싱하여 재사용함으로써 중복 할당과 계산을 방지합니다.
