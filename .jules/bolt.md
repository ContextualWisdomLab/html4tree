...
## 2024-07-20 - HTML 디렉토리 순회 시 정적 에셋 객체 추출 최적화
**Learning:** 디렉토리를 순회하면서 처리하는 재귀(recursive) 혹은 반복 루프(`process_dir`) 내부에 고정된 크고 정적인 문자열(예: CSS)이나 결정론적 계산(예: SHA-256 해시)이 위치하면, 디렉토리가 많을수록 불필요한 메모리 할당과 CPU 오버헤드가 선형으로 증가한다는 것을 알게 되었습니다.
**Action:** 동일한 값을 반환하는 대용량 상수 문자열과 해시 계산은 루프 바깥의 `private object`로 추출(extract)하여, 클래스가 로딩될 때 단 한 번만 초기화(`const val` 및 `@JvmField` 사용)되도록 최적화함으로써 반복문마다 발생하는 불필요한 비용을 최소화해야 합니다.
