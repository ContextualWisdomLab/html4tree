...
## 2024-05-24 - 단일 StringBuilder를 통한 문자열 연결 최적화
**학습:** `index_top + index_middle() + index_bottom`과 같이 여러 문자열 및 함수 결과를 연결할 때, 내부적으로 여러 중간 문자열 객체가 할당되어 불필요한 메모리 할당 및 가비지 컬렉션 부하를 유발합니다. 특히 디렉토리를 순회하면서 이 작업이 반복되면 성능이 저하될 수 있습니다.
**조치:** 여러 개의 문자열 조각과 반복문에서 생성되는 문자열을 연결할 때, 단일 `StringBuilder` 인스턴스를 초기화하고 모든 문자열 조각(`index_top`, 루프 결과, `index_bottom`)을 `append`하여 중간 문자열 할당을 완전히 제거합니다.
