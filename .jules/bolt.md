...
**Learning:** Kotlin 애플리케이션에서 디렉토리를 재귀적으로 순회하는 `process_dir` 함수 내부에 고정된 CSS 문자열 선언과 SHA-256 해시 연산이 포함되어 있으면, 처리하는 디렉토리 수에 비례하여 불필요한 메모리 할당과 무거운 암호화 해시 연산이 매번 발생합니다.
이러한 상수는 파일 최상위 레벨(Top-level properties)로 추출하여 한 번만 초기화하도록 최적화해야 합니다. 또한 Kotlin 컴파일러는 최상위 레벨 변수에 대해 암묵적인 getter를 생성하므로, JaCoCo 100% 명령어 커버리지를 유지하려면 테스트에서 이러한 변수들의 값을 읽어 명시적으로 호출해야 한다는 점이 중요합니다.
**Action:** 디렉토리나 파일을 반복 처리하는 핵심 루프 내에 불필요한 상수 객체 생성이나 무거운 연산(특히 암호화 해시 등)이 존재하지 않는지 먼저 확인하고, 발견 즉시 최상위 레벨이나 companion object로 추출(캐싱)하여 성능을 개선합니다. 이 때 테스트 커버리지가 떨어지지 않도록 추출된 상수에 대한 접근 테스트도 반드시 추가합니다.
