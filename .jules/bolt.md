...
## 2026-08-11 - 문자열 이스케이프 핫 패스 최적화
**학습:** Kotlin에서 `escapeHtml`과 같이 디렉토리 및 파일 이름마다 호출되는 핫 패스에서 문자를 이스케이프할 때, `when` 조건문을 통한 점프 테이블 평가를 각 문자마다 수행하는 것은 성능에 영향을 줄 수 있습니다.
**조치:** `when` 조건문을 직접 배열 인덱스 기반 조회(`Array<String?>(128)`)로 교체하여 분기 예측 오버헤드와 조건 평가 비용을 최소화하고 성능을 향상시켰습니다.
