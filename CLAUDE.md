...
- `src/main/kotlin/html4tree/main.kt` — everything of substance:
  ...
  - `process_dir(dir)` builds the HTML listing (sorted entries, folder/file icons, aria-labels, CSP meta tag, empty-state message) and writes it via `write_index_file`. Absent targets use exclusive publish (`Files.createLink`; on `UnsupportedOperationException` or `IOException` except `FileAlreadyExistsException`, a create-only move with no `ATOMIC_MOVE` / `REPLACE_EXISTING`). Owned or `--force-overwrite` targets are backed up, reclassified, then replaced. Failed replace restores only after another classify (`ABSENT`/`OWNED`); a late customer page keeps the occupant and a `backup-retained:` path. Unowned, symlink, and directory occupants are preserved.
