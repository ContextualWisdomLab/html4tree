...
- `src/main/kotlin/html4tree/main.kt` — everything of substance:
  ...
  - `process_ignore_file(dir)` returns the exclusion set: `.html4ignore` globs (`PathMatcher` `glob:` syntax) plus `index.html` plus a hardcoded sensitive-file list (`.git`, `.env`, `.ssh`, etc.). Pass the same name snapshot used to render the page so the crawl does not call `File.list()` again.
